# BRIEF — #631 increment 5: the server-derived session catalogue, still dark

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/glm-631i5
Branch: glm-631i5
Base sha: 911b6ab7
...
Lane-owns: session_source.py, test_session_source.py


**Task:** `python3 dev/ledger.py get 631 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry and all its notes. Increments 1–4 are landed.

**Design:** `.dreamwork/docs/plans/session-log-view.md`, **§12 increment 5**, which specifies this increment including its two injections. That spec is authoritative; this brief adds only what it does not say.

> **Server-derived session catalogue, still dark.** Extend the existing active-session resolver with the switcher catalogue: strict UUID JSONL discovery under the measured client root, target-cwd classification, mtime/size/liveness metadata, and no browser-supplied path. The recorded `agent_session` remains the only active identity; newest-mtime is never promoted to identity.

**Files:** `session_source.py` and `test_session_source.py`. Existing callers of `resolve_target` stay `session_source.main` and its focused tests; the new catalogue is test-only until `SessionService` calls it in increment 6.

**Explicitly later:** opening or scanning any catalogue entry. That is increment 6 onward.

## The two rules that carry the security weight, and they are not decoration

**1. No browser-supplied path.** The catalogue is *server-derived*. A path that arrives from the client and is then used to open a file is a directory-traversal primitive, and this reads real session transcripts containing real conversation content. The wire shape must carry an opaque identifier the server can resolve, never a path the browser can choose. **Return no absolute paths in the wire catalogue at all** — the design names this as an injection: *returning an absolute path in the wire catalogue must red the confinement assertion.*

**2. Newest-mtime is never promoted to identity.** The recorded `agent_session` is the only active identity. The design's other injection: *making newest mtime the active choice must red when the recorded id names the older live file.* This is the interesting one, because newest-mtime is what a reasonable implementer reaches for and it is right almost all the time — which is exactly what makes it dangerous. **Run both injections.**

## Direction 2

**Strict UUID discovery is where a "safe" filter quietly is not.** Construct names that a loose check accepts: a valid UUID with a path separator or `..` embedded, a UUID-shaped prefix followed by anything, a symlink whose name is a clean UUID but whose target leaves the root. The design already puts a **symlinked projects root** in the fixtures, so the symlink case is in scope by construction — say whether you resolve links before or after confinement, because the order is the bug.

Second: the catalogue **passes vacuously when it finds nothing** — an empty catalogue over an empty root and an empty catalogue over a root the resolver failed to measure are different facts that must not render identically (`#136`, `#671`). Assert the fixture population is non-empty as a precondition, derived at runtime.

Third, on liveness: "live" is a claim about the world at read time, and it goes stale between the scan and the render. Say what your metadata means and over what window — `#765` measured that a recorded hold keeps reading as current long after its condition expired, and liveness is the same shape one level down.

## What increments 1–4 established that binds you

- The node/state/event vocabularies are **closed**; `session_log.model` raises `ModelError` on an unknown kind. Do not widen them.
- The three classifier outcomes — a node, `suppressed`, `unclassifiable` — must stay distinguishable (`#702`).
- Increment 4 landed `Frontier` and `scan_incremental` as **the one supported way** to resume. Do not add a second entry point.
- The wire shape is exact: `{ev, node}`, and `ref.len` is spelled `len`.

## Where to get real input

Real Claude Code JSONL transcripts live under `~/.claude-p/projects/`. **Read them to check your assumptions against reality — do not copy one into the repo**; they contain session content. Hand-author the fixtures. If you find a directory shape §2 does not describe, report it rather than extending the resolver to swallow it.

## Scope

A **~15–20 minute landing**. Increment 4 came in at +12 collected tests. If this grows past a coherent landing, stop at a committable point and say what remains.

## Constraints

- **`#671`**, **`#136`**, **`#702`**, **`#755`**, **`#440`** as usual.
- This increment is dark and server-only. If you are touching `watch.py` or any route, you have left it.
...
- `just pytest test_session_source.py <anything else you touched>` — the one supported form. **Do not run the bare full suite**; I own the single merged-tree sweep.
- **Count tests by collecting, not by grepping `def test_`.** Tell me the collected count before and after so my gate arithmetic is right.
...
`.dreamwork/lane-631i5-report.md`, with a DOGFOOD REPORT section. Lead with what your confinement check decides for the adversarial names above, since that is the assertion protecting real session content.
