...
# BRIEF — #766 Half B: persist the brief inside the dispatch wrapper

**Task:** `python3 dev/ledger.py get 766 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry **and all its notes**, especially the last one. That note is my adjudication of a previous lane's Half B and it is the reason this task is dispatched again. Read `#768` too; it is the incident this design has to survive.

Half A landed. The four brief checks now say `HISTORICAL ONLY` with the id gap. **The freeze itself is still unfixed** — nothing persists briefs, so that gap grows every dispatch and the new qualifier just watches it happen.

## The previous lane's answer, and why I refused it

A prior lane (branch `cx-766brief`, kept — read `.dreamwork/lane-766-report.md` on it, §"Half B — IGC decision") concluded: **the lane persists its own untouched `BRIEF.md` as its first increment.** It rejected coordinator persistence on a base-SHA circularity it called G5, and **that catch is correct**: commit the brief before creating the worktree and the brief cannot cite the sha of the commit containing itself; create the worktree first and commit second, and master's tip is no longer the lane's branch point at dispatch. Do not dismiss it — it is real, and it constrains you.

I refused lane-first for a reason its goal set never names: **a lane cannot persist a brief it never received.** That is `#768`'s incident exactly — a malformed dispatch handed the runner 24 literal characters and no brief at all. Nor can a lane persist one it died before committing, nor one on a branch that gets abandoned; abandoned lanes are precisely the ones whose briefs are most diagnostic. In all three the corpus stays frozen while Half A's qualifier reports the gap growing.

## What to build

`dev/dispatch_lane.py` already validates the prompt and then `exec`s the runner. **Make it also write the validated prompt into `.dreamwork/docs/briefs/` at validation time, uncommitted.** The coordinator commits it at the merge gate.

Nothing is committed at dispatch, so master's tip never moves and no content-addressed sha is self-referential — G5's circularity dissolves rather than being traded against. And persistence lands inside the already-mandatory single route (`#440`) instead of being a second thing a lane must remember.

**This is a design I am handing you, not a conclusion you must reach.** If you find it wrong, say so with the measurement or the argument that kills it, and build what survives. What you may not do is ship lane-first without answering the three-scenario objection above.

## The cost you must state, not hide

This is **weaker** than lane-first on the prior lane's G2 (*"enters git before any task change can obscure the evidence"*). A brief written at dispatch but committed at the gate sits uncommitted in my working tree for the lane's entire lifetime, where it can be edited or deleted with no trace.

**You own answering that**: does the window matter, and what detects a pending brief that changed between dispatch and commit? A content hash recorded at write time is one option; there are others. Whatever you choose, `#671` applies — a brief whose integrity was never checked must not read the same as one that was checked and matched.

Do not let the wrapper's name or message imply it guarantees more than it does (`#651`, `#768`'s own limit paragraph in `SKILL.md`).

## Naming and collision

The filename must let Half A's `brief_corpus_reach` order it — that function takes the **leading integer** of the filename (`lint.py`, `brief_corpus_reach`; find it by symbol). A brief that cannot be ordered increments the `unnumbered brief(s) cannot be ordered` count, which is the honest-but-useless outcome. Two dispatches for the same task id must not collide destructively; the prior lane used `<task>-<lane>.md` for this and that part of its design is sound.

## Fix what the refusal made false

Branch `cx-766brief` is kept and **not merged beyond Half A**. Its report says *"Both halves shipped"* and describes a lane-first `SKILL.md` paragraph that does not exist on master. Landing this task means the repo's record matches what shipped: carry the prior lane's Half B IGC and measurements forward (they are good work and `#764` says a citation carries its own evidence), and correct the verdict. **Do not silently delete its reasoning** — the four-option IGC and the G5 catch are the reason this design exists.

## Constraints

- **`#755`: silent on a healthy dispatch.** The wrapper is currently silent at `rc=0`; adding a write must not add output. Measure it against a real correct dispatch.
- **`#671`: a check that examined nothing must not read as passing.** A write that silently failed (unwritable dir, full disk) must not look like a clean launch.
- **`#136`: "brief persisted" and "could not persist the brief" must not render identically.**
- **`#702`: report what you cannot classify.**
- **`#440`: one supported way.** Exactly one writer, one time, one destination.
- Do NOT rewrite historical briefs or lane reports other than `cx-766brief`'s own, and do not disturb grandfathering — it has been upheld five times.
- Do NOT modify `ccc` or anything outside this repository.

## Verification

- `python3 lint.py` from the worktree — the lane bar is **6 warnings** (`#611`/`#667`). Do not compare against the main checkout's number. **Your change adds a brief to the corpus; say exactly how the four brief lines move, before and after.** If your own persisted brief changes the reach qualifier, that is the feature working — report the new line.
...
- Red-proof both directions. Direction 1: make the write fail and require a message naming what could not be persisted. **Direction 2 is the one that decides this task** — construct a dispatch where the brief appears persisted but the artifact is wrong or absent, and show what your check decides. The prior lane's direction 2 for Half A is the bar: it found an honest false-green and stated it rather than claiming completeness.
...
- Use `dev/lane_scratch.py` for any filesystem measurement; the scratchpad is tmpfs (`#634`).
- No browser guards, no port binding, nothing near `:35110` / `:35113`.

## Report

`.dreamwork/lane-766b-report.md`, with a DOGFOOD REPORT section. State plainly whether the wrapper route escapes the remember-to-run problem or merely moves it, and what the uncommitted window costs.
