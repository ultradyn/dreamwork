...
Worktree:   /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-645i6
Branch:     cx-645i6
...
Lane-owns:  dreamwork_db/, its migrations and tests, .dreamwork/lane-645i6-report.md

Your brief is at BRIEF.md in your worktree and reproduced below.
Read the ledger entry and its notes before planning.

# BRIEF — #645 increment 6: the schema-v3 unit

**Task:** `python3 dev/ledger.py get 645 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry **and all its notes**. Increments 1–5 are landed; their notes carry the precedents and my recorded brief errors. Do not re-derive them.

**Design:** `.dreamwork/docs/cx-645-db-api-design.md`, the numbered landing sequence at its tail. Increment 6 reads:

> **Schema-v3 unit.** Add questions/reviews/issues/links in scratch stores and migrate the empty live-shape `review_decision`; no live import or dispatch.

Increment 5 (merged `64b9eb2b`) enabled the no-production-raw-connect guard specifically so this increment could not add a new schema behind a raw connection. That guard is now live in `test_no_raw_connect.py`; **your new code must route through `dreamwork_db.core`** or it goes red.

## What "no live import or dispatch" means, and why it is the whole discipline

The new tables exist in **scratch stores**. Nothing reads them, nothing writes them from a live path, and `review_decision`'s migration is of an **empty live-shape** table. If you find yourself importing real questions or wiring a route, you have left the increment — that is increments 7–11 and they are already sequenced.

State in your report what would break if someone shipped a live import on top of this, so the next increment's lane knows what it inherits.

## Precedents from increments 1–5 that bind you

- **The version fact is `meta.schema_version`**, not `PRAGMA user_version`.
- READ specs need no initializer; core ignores initializers on READ handles.
- Repositories return the **exact compatibility DTO shapes** the facades already returned, not `sqlite3.Row`.
- Facades keep their signatures and soft-failure behaviour and hold no SQL.
- **A parity or migration proof must load the "before" state from a frozen source**, not from the code doing the migrating (`#759`). Increment 4 used a git rev for this; a migration has a natural equivalent in a fixture store built at v2. Say which you used.

## Direction 2 — where this increment is won

A migration test that builds a v2 store, migrates it, and asserts the v3 shape **passes just as well when the migration is a no-op and the fixture was already v3**. Construct that case and show what your check decides. If it passes, the test proves nothing about the migration.

The same trap one level over: an empty table migrates "successfully" under almost any implementation, and `review_decision` is empty by construction here. Say explicitly what an empty-table migration can and cannot establish — this is `#651`'s rule, and increment 5's own direction-2 (an aliased connect evading a lexical guard) is the standard to match.

## The call-graph rule — because I have now got this wrong three times

My file lists have twice come from the diff I expected rather than the call graph, and once — at increment 5 — I wrote "exactly three paths" when the real inventory was **7 sites in 6 files**, because my grep was `--include="*.py"` and could not see the two extensionless CLIs. That lane re-measured and corrected me.

**Find callers by searching and name them.** Your verification list must include caller fallout, not just the files you change. If my framing here is wrong, say so — being corrected is cheaper for both of us than a red merge gate.

## Constraints

- **`#755`: silent on healthy input.** A migration that runs on an already-current store must not warn.
- **`#671`: a check that examined nothing must not read as passing.** A migration verifier over zero rows must say so.
- **`#136`: "migrated" and "could not determine the schema version" must not render identically.**
- **`#702`: report what you cannot classify.**
- **`#440`: one supported way** to reach a version — do not add a second migration entry point beside the existing one.
- Do NOT touch the live `.dreamwork/ledger.sqlite3`, the live `questions.md`, or `status.json`.
- Do NOT rewrite historical briefs or lane reports.

## Verification

- `python3 lint.py` from the worktree — **the lane bar is 5 warnings.** I measured this at `bc7aab6b` on a throwaway worktree today; it was 6 until `#767` retired a false one this morning. Do not compare against the main checkout's number (`#611`/`#667`).
- `just pytest <files you touched, plus caller fallout>` — the one supported form. **Do not run the bare full suite**; I own the single merged-tree sweep. (Standing contract, not an override.)
- **`just pytest test_no_raw_connect.py` as well** — increment 5's guard is the thing most likely to catch a new schema opening its own connection, and it is cheap.
- Prefer an exact pytest node id over a `-k` expression containing spaces; a prior lane measured that the recipe splits `and` into a path argument.
...
`.dreamwork/lane-645i6-report.md`, with a DOGFOOD REPORT section. Lead with what the empty-table migration proof does and does not establish.
