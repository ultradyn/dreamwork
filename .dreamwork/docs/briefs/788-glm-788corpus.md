# BRIEF — #788: the brief corpus is written, receipted, and never committed — so every audit of it is silently truncated

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/glm-788corpus
Branch: glm-788corpus
Base sha: 8d60bbae
...
Lane-owns: dev/citation_audit.py, test_citation_audit.py, .dreamwork/lessons.md (the two #786
provenance clauses ONLY), and a corpus-commit mechanism wherever you decide it belongs.
NOT dev/dispatch_lane.py without saying why in your report first — see the care note below.


**Task:** `python3 dev/ledger.py get 788 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` —
read the entry and its note. The note is the specification.

## This was measured today, and it already produced one wrong answer

`#766` landed brief persistence: `dev/dispatch_lane.py` writes every brief to
`.dreamwork/docs/briefs/<id>-<lane>.md` with a `.sha256` integrity receipt, and refuses a receiptless
file. That half works. **Nothing ever commits the result.**

Measured in the main checkout at `8e3bfc10`:

```
251  .md files on disk under .dreamwork/docs/briefs/
220  tracked by git
 64  untracked entries (.md + .sha256 pairs)
```

`git check-ignore` returns nothing for them — they are not ignored, they are simply never added.

**The consequence is not hypothetical.** The `#786` lane audited this corpus from its worktree,
correctly found 219 briefs and **zero** carrying the phrase *"a check that fires on a healthy
input"*, and concluded the miscitations were *"ephemeral session prompts that were never
persisted."* That conclusion is **false**. The main checkout holds **22** briefs carrying the
phrase, and **all 22 are untracked** — a clean partition, because the miscitations are recent and
everything recent is uncommitted. The lane measured honestly for exactly what it could see. What it
could not see was 12% of the corpus, and that 12% was the entire subject of its audit.

**Re-derive all of these numbers yourself before building on them.** I am telling you how I measured
(`ls -1 .dreamwork/docs/briefs/*.md | wc -l` against `git ls-files .dreamwork/docs/briefs/ | wc -l`,
then `grep -rl` for the phrase and `git ls-files --error-unmatch` per hit) so you can audit the
method rather than inherit the number. This matters here more than usual: this task exists *because*
a measurement was inherited from a truncated input.

## Two halves, and they are separable

### Half 1 — commit the corpus

`dispatch_lane.py` writes the brief; something must add it. **Weigh where that belongs and say why:**

- **At write time in `dispatch_lane.py`.** Closest to the write, so the file is never unversioned.
  But `dispatch_lane.py` ends in `os.execvp` on the loop's only dispatch route, and it runs from the
  main checkout — a commit there happens on every dispatch, against a tree the coordinator may have
  mid-edit. Handle this file with extreme care; a change that makes dispatch refuse or hang does not
  degrade a feature, it stops lanes launching at all.
- **A sweep the coordinator runs** (a `just` recipe, or a step in an existing one). Safer, but it is
  a rule that depends on being run — and this loop has spent two days measuring what happens to
  those.
- Something better.

**A hard constraint: a lane must never commit to the main checkout.** Whatever you build, if it is
coordinator-side, *build it and leave it for me to run* — do not run it against the live tree
yourself. Your own commits go on your branch as usual.

`git add` on this path must also stay compatible with `#766`'s receipt refusal: the `.sha256` files
are part of the corpus and a half-committed pair (`.md` without its receipt) is worse than neither.

### Half 2 — make truncation visible

`dev/citation_audit.py` (landed by `#786`, on master now) reports
`citation_audit: examined 161 citation(s)`. Run from a worktree it will silently examine a truncated
corpus and say nothing about it. **Any corpus-scanning tool must state the tracked-vs-on-disk split
it actually examined** — an audit that saw 87% must not read like one that saw all of it (`#671`).

This is the half that survives even if Half 1 is judged not worth doing, and it is the more
important of the two. A committed corpus can drift out of date again; a tool that names its own
input boundary stays honest permanently.

## Half 3 — correct two provenance clauses, precisely

`#786` added two entries to `.dreamwork/lessons.md`. **The principles and their evidence are correct
and must stay** — they are the homes those principles earned, and this is not a licence to rewrite
them. Exactly one clause in each is false: both say the miscitations lived in *"19 ephemeral session
prompts"* / *"30 ephemeral prompts"*. They were **persisted, but uncommitted**. Correct that clause
and nothing else, and cite `#788` for the correction so the entry carries its own evidence (`#764`).

## Red-proof

- **Direction 1:** whatever you land, break it and watch it red on the **discriminating** message —
  for Half 2, a tool run against a corpus with untracked members must name the split; sabotage the
  split reporting and the test must fail on *that*, not on a generic assertion.
- **Direction 2, and this is the one that matters:** construct the case where your fix **looks**
  applied but is not. For Half 2 specifically: a corpus where tracked and on-disk counts are
  **equal** makes a broken split-reporter indistinguishable from a working one. Your fixture must
  have an untracked member, or the test passes vacuously — that is precisely the shape that let
  `#786` through. Show the vacuous version failing to catch a sabotage, then show yours catching it.
...
Cite these by **content**, not by my wording — two of my recent briefs attributed principles to ids
whose entries do not contain them, which is what `#786` was filed for. Verify each before relying on it.

- **`#671`** — *"`ledger.py sweep` never got the `#294` store dispatch — it examines zero ledger
  entries and says so confidently."* A check that examined nothing must not read as passing. Here:
  an audit that examined 87%.
- **`#651`** — *"A guard whose message names a failure mode it cannot detect."* `examined 161
  citation(s)` names a corpus it did not fully see.
- **`#764`** — *"The fix for miscited task ids was to cite `lessons.md:<line>` — and line citations
  rot too."* Why the Half 3 correction must carry its own evidence.
- **`#440`** — one supported way. If the corpus gets committed, there must be exactly one route that
  does it, not a dispatch-time add *and* a sweep.
- **`#611` / `#685`** — gitignored/untracked state cannot travel into a worktree. The mechanism
  behind this entire task; the lint delta is the same phenomenon in a different costume.

## Run these, explicitly

```
just pytest test_citation_audit.py test_dispatch_lane.py
just pytest $(python3 dev/repo_wide_guards.py list)
python3 dev/citation_audit.py
```

`test_dispatch_lane.py` is in the list because `#701` just landed `validate_stdout` there and Half 1
may touch that file — if your change reaches any other caller, **find it by the call graph rather
than by the file list I gave you.** A verification list derived from the files you will edit rather
than from what they reach is `#763` instance six, and it has already cost this loop one red gate.

## Bars

Re-measure and quote what *you* got; if your numbers differ from mine, say so rather than
reconciling to them.

- Main-checkout lint reads **2** — `python3 lint.py | tail -1`; the count is in the **trailer** and
  the rows are **indented**, so `grep -c '^WARN'` returns a false **0** (I did that today and nearly
  shipped it into four briefs). A worktree reads **6**; the delta of 4 is the gitignored ledger store
  that cannot travel (`#611`, `#685`) — the same mechanism as this task.
...
## One more bar: master is RED and it is not your doing

At `8d60bbae` a full `just pytest` reads **1 failed, 2751 passed**. The single failure is
`test_reanchor_citations.py::test_each_reviewed_anchor_line_contains_the_named_evidence`, owned by
`cx-789anchor` (line-pinned anchors rotted when #689 inserted 12 lines into watch.py). Do not fix it
and do not count it against yourself — but quote your failure list explicitly rather than saying
"same as baseline", so a real red of your own cannot hide behind mine.
