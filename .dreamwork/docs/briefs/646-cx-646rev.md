# BRIEF — #646 REVIEW: the author landed three commits and died before reporting

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-646rev
Branch: cx-646rev
Base sha: 8d60bbae
...
**Your branch `cx-646rev` is branched FROM the author's tip**, so all three of its commits are
already in your history and your review commits go on top -- the author's work stays intact and
attributable. (You are on your own worktree rather than literally inside the author's, so that the
dispatcher's corpus-name and base-sha checks stay satisfied; the review relationship is unchanged.)

Lane-owns: you are the REVIEWER of this work. **Export `DREAMWORK_LANE_ROLE=reviewer`
before using any scratch tooling** — `#694` landed role-keyed scratch precisely so a reviewer cannot
overwrite the author's evidence, and that separation only engages if the role is set. The author's
scratch is READABLE and is evidence; do not write into it.

## Situation

`glm-646policy` (a glm52 lane) landed three commits and then died **without writing a report**. So
there is no author account of what was done, what was verified, or what was left open. You are the
only reader this work will get before I decide whether to merge it.

```
97d24224 feat(#646): add POST /subagent-policy write/reset route + helpers
6f586594 feat(#646): add subagent-policy textbox UI under posture control
54a81006 test(#646): POST /subagent-policy save/reset/round-trip + delete helper
```

`git diff beddf975...glm-646policy` — 425 insertions across `watch.py`, `client/router.js`,
`client/style.css`, `client/views.js`, `test_watch.py`.

**Read the author's brief first:** `.dreamwork/docs/briefs/646-glm-646policy.md`. It is the spec the
work must satisfy, and it contains requirements a dying lane is most likely to have dropped.

## What the spec required — check each, and say which are MISSING

This is a union spec: Max asked for this widget twice (`#580` and `#646`) and **neither ask is a
subset of the other**. Verify BOTH halves landed:

1. **Explicit update/save button, plus a reset button.** NOT live-apply, NOT blur-to-commit. Every
   other posture control arms on a shared 10s cooldown; he asked for this one to be explicit. A
   version that "helpfully" made it consistent with the others has failed the spec.
2. **Placeholder CYCLES through several options, and diversity of placeholders matters** (`#580`'s
   half — the half most likely dropped). Examples he gave: no subagents / specific models / custom
   worktree dirs / roles / build boxes / deploy auth.
3. **Reset means DELETE the file**, not clear-to-empty. `watch.py`'s `read_subagent_policy` docstring
   is explicit: *"a present-but-blank file reads as unset ... 'no policy' is expressed by deleting
   the file rather than by an empty one that looks set."* A reset that writes an empty file leaves an
   inert file that `lint`'s `check_subagent_policy` then has to complain about.
4. **Byte-for-byte round-trip.** The value must be read with `read_text_full`, NOT `read_text`.
   `#632` is the defect that deleted 12 answered questions by using the display-shaped reader for a
   durable value. **Test it yourself** with a policy containing newlines, leading/trailing
   whitespace, and a non-ASCII character.
5. **`subagent_policy` must NOT appear in the external posture summary.** `watch.py:~4110` records
   that exclusion deliberately (his authored prose, `SUMMARY_DENIED`). Check the diff did not add it.

## Your verdict, and the standard

Run the tests. Then **break each behaviour the tests claim to bind and confirm they red** — a test
suite written by a lane that died mid-task is exactly where a vacuous assertion hides. Specifically
hunt the two green-looking failures the brief named: **a save that returns 200 while the file on
disk is unchanged**, and **a reset that appears to clear the field while leaving a file behind.**

Deliver: MERGE / MERGE WITH NAMED GAPS / DO NOT MERGE, with reasons tied to the spec. You may commit
fixes on top of the author's commits if a gap is small and clearly right; say what you changed and
why. If a gap needs a design decision, describe it and leave it to me.

Browser guards bind 39890–39899 — check before binding, and if you cannot run them, **say you could
not** rather than reporting the UI as visually verified.
