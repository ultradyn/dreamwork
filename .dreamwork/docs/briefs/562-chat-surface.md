...
1. **The count line is total-only.** `chatList` (watch.py:4037-4040)
   renders `topic chats · ${d.chats.length}`. His question: "is that
   unread or total?" — it is total, and it doesn't say so.
2. **A chat cannot be opened.** `chatRow` (watch.py:4026-4036) emits an
...
- `transitions.md` — the route change IS the reference implementation;
  ...
  re-render idiom (see the comment at watch.py:4020-4027) — reuse it,
  author nothing new.
