...
| `.dreamwork/review-dispatches/<branch>-r<round>-<digest16>.launch.json` | `test_dispatch_lane.py` (`test_launch_review_creates_attached_branch_worktree_and_records_cwd`) | Machine-local, gitignored review-launch witness, written atomically by `dev/dispatch_lane.py` (`launch_review`). JSON fields: `attempt_id`, `branch`, `round`, `review_lane`, `pinned_sha`, `prompt_sha256`, `prompt_bytes`, `prompt`, `worktree`, `permission_mode`, `state`, `runner_exit`, `runs`. Meanings: `attempt_id` is `<review_lane>-<digest16>`; `branch` and positive `round` identify the review; `review_lane` is the attached review-worktree branch; `pinned_sha` is the full reviewed commit; `prompt_sha256`/`prompt_bytes` identify the exact prompt; `prompt` and `worktree` are absolute paths, with `worktree` also the runner cwd; `permission_mode` is literal `plan`; `state` is the latest preparation, worktree, or spawn outcome; `runner_exit` is currently always null because the detached runner's exit is not observed; `runs` is 0 before launch is attempted and 1 once dispatch begins. The test reads the record and binds this complete field list plus the `prompt`, `review_lane`, `worktree`, `pinned_sha`, and `state` semantics; `--review-status` (#1207) consumes the `runner_exit`/`state` by re-probing the `review_lane` runner's cwd-containment and classifying each dispatch `in-progress` (runner present) vs `runner-absent` (runner gone, exit never observed — the alarm a dead review now expresses) vs `unknown` (probe examined 0, #868). It is read-only: it never writes `runner_exit` or any state (the null is honest, #1177) | `test_dispatch_lane.py`; `lint.py` does not inspect `*.launch.json` |
