# BRIEF — #791 REVIEW LENS B: did each lane's red-proof actually bind?

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-791revB
Branch: cx-791revB
Base sha: beddf975
...
Lane-owns: NOTHING. **Read-only review lane.** Report to the coordinator inbox; fix nothing.

## The question, which is different from lens A's

Six merges landed this hour (`git log --oneline dc739001..beddf975`). Every lane reported a
red-proof in both directions. **A red-proof that did not actually bind is the most expensive kind of
false green in this loop**, because it buys confidence with nothing behind it — and unlike a plain
bug, nothing downstream ever catches it.

For each merge, read the lane's report in `.dreamwork/inbox.md` and then check its claim against the
landed code:

1. **Did the direction-1 injection red on the DISCRIMINATING message**, or merely on "an assertion
   failed"? A test that reds because *any* exception occurred does not bind the behaviour the lane
   claimed.
2. **Is the direction-2 construction real?** Several lanes reported an *open* false-green honestly
   (#689 constructed a lying-label case and said its test accepts it; #670 named a vacuous
   `not is_inside_worktree(safe)` assertion; #785 named an uncalibrated 8 GiB floor). **Those honest
   admissions are not defects — they are the deliverable working.** Your job is to find the ones that
   were NOT named: a direction-2 claim that sounds complete but is not.
3. **Does each new test fail if you break the thing it names?** Actually try it, on a scratch copy —
   snapshot with `dev/redproof.py`, never `git checkout` (#349), snapshot the FIXED file (#608).
4. **`#786`'s tests specifically:** its central measurement was WRONG (it audited a corpus from a
   worktree and could not see 12% of it — see #788). Its *tests* passed anyway. Ask what a test could
   have asserted that would have caught a truncated input, and whether any other guard in the repo
   has the same shape: correct logic, silently incomplete input.

## The trap to avoid

Do not grade reports on eloquence. A lane that wrote a long red-proof section and a lane that
actually injected a defect look similar in prose and are not similar. **Check the code, not the
narration.**

Equally: do not manufacture findings to justify the lane. **"All six red-proofs bind, here is how I
checked each" is a complete and valuable result.** Say what you verified and how; report what you
could not determine rather than guessing (#702).
