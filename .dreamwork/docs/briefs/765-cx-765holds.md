...
Worktree:   /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-765holds
Branch:     cx-765holds
...
Lane-owns:  lint.py, test_lint.py, .dreamwork/lane-765-report.md

Your brief is at BRIEF.md in your worktree and reproduced below.
Read the ledger entry and its notes before planning.

# BRIEF — #765: a hold is a claim about write-time, and nothing re-evaluates it

**Task:** `python3 dev/ledger.py get 765 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry in full. It carries three measured instances and, more importantly, the trap.

## The defect

A recorded hold is a claim about the world at **write time**, and nothing ever re-evaluates it. The text keeps reading as current long after its condition expired. Three instances, all mine, all in one night:

1. `#630`'s **title** said "blocked on his G2 ruling" after the ruling had arrived. I scanned the P1 list, read the title, passed over a dispatchable task, and told him it was blocked — which was false.
2. `#631`'s **title** said "blocked on his three design calls" after all three were answered.
3. `#631`'s **`queued_dispatches` note** said "NOT parallel with component-registry work". True when written; every named prerequisite had since landed. Found only because I read the entry rather than the note.

`#725` covers titles. **Nothing covers `status.json`'s `queued_dispatches` notes or the ledger's `blocked_on`.**

## The trap — read this before designing anything

**A check that flags the WORDS fires on every legitimately-current hold, and most holds are current most of the time.** "held", "blocked", "not parallel", "waiting on" — that check trains the reader to skip the line, which is `#755`, and this loop has already paid for it once tonight: `#767` retired a WARN that had been wrong on every run all night while I quoted the warning count as my gate bar fourteen times without asking which two.

`#746` measured that `#725`'s title check **still fires on negated forms** like "not blocked on X anymore" — the same disease one level down. **Measure how many live holds your proxy names before believing it.**

## The framing that might be decidable — and it is structural, not a check

A hold whose release condition is **prose** is undecidable. A hold that **names the id or event that releases it** is checkable:

- `"#736/#628: held until load drops"` — prose, undecidable.
- `"held on #630 P2"` — a fact the ledger can evaluate.

So the candidate is: **require a hold to name its releaser, then check only the named-releaser form, reporting prose ones as unclassifiable rather than dropping them** (`#702`).

**Whether that ceremony is worth it is exactly what you must MEASURE before building.** Count today's live holds across ledger `blocked_on`, task titles, and `status.json`'s `queued_dispatches`; classify each as named-releaser or prose. If prose dominates, a named-releaser check governs almost nothing and is ceremony.

**Building nothing and shipping the measurement plus a lesson is a legitimate and complete outcome.** Say so plainly if that is where the numbers land — I would rather have the number than a check.

## Direction 2

If you build a check: construct a hold whose named releaser has landed **but whose real condition has not** (a task can land while the thing it was waiting for remains true in substance). Show what your check decides. A named-releaser check that reads "releaser landed, therefore hold expired" will be confidently wrong here, and that is the interesting case.

If you build nothing: the equivalent obligation is to state what a future implementer would get wrong, given your numbers.

## Constraints

- **`#755`: a check that fires on healthy input is worse than no check.** This is the governing rule for the whole task.
- **`#671`: a check that examined nothing must not read as passing.**
- **`#136`: "hold is current" and "I cannot evaluate this hold" must not render identically.**
- **`#702`: report the prose ones; never drop them.**
- **`#440`: one supported way.**
- **Do NOT write to the live `.dreamwork/status.json` or `.dreamwork/questions.md`** — they are the coordinator's and his. Measure against a copy.
- **Do NOT run mutating ledger verbs against the live ledger.** Use a fixture via `--ledger`.
- Do NOT rewrite historical briefs or lane reports.

## Verification

- `python3 lint.py` from the worktree — **the lane bar is 5 warnings.** I measured this at `bc7aab6b` today; it was 6 until `#767` retired a false one this morning. Do not compare against the main checkout's number (`#611`/`#667`). **If you add a check, say exactly how the line count moves, before and after.**
- `just pytest <files you touched>` — the one supported form. **Do not run the bare full suite**; I own the single merged-tree sweep. (Standing contract, not an override.)
- Prefer an exact pytest node id over a `-k` expression containing spaces.
...
- Use `dev/lane_scratch.py` for filesystem measurement; the scratchpad is tmpfs (`#634`).
- No browser guards, no port binding, nothing near `:35110` / `:35113`.

## Report

`.dreamwork/lane-765-report.md`, with a DOGFOOD REPORT section. **Lead with the measurement**: how many live holds exist, and how many name their releaser. That number decides the task, and it is worth having even if nothing ships.
