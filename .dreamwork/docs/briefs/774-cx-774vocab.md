# BRIEF — #774: the resolution vocabulary has two copies, and the second one is the retired one

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-774vocab
Branch: cx-774vocab
Base sha: e699d4f6
...
Lane-owns: dreamwork_db/question_parse.py, test_question_parse.py, test_dreamwork_db_import.py, lint.py (ONLY the resolution-marker constant — see "Contention" below)

**Task:** `python3 dev/ledger.py get 774 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry, it carries the full measurement. Also read `get 767`, which is the ruling this bug re-broke, and `get 645` whose newest note assigns you the fixture rename.

## The bug, already measured — do not re-litigate it, verify it and fix it

`dreamwork_db/question_parse.py:66` derives `resolution_date` from **only** the legacy arrow head:

    ^\s*→[^:]*?\((\d{4}-\d{2}-\d{2})(?:\s+(\d{2}:\d{2}))?\s*\)

`lint.py:448`, **as landed by #767 hours before that module was written**, accepts the arrow head **or** a dated `- **Answer (via watch, <date>` **or** a dated `- **Comment (via watch, <date>`. #767 also established that `Folded` alone warns `FOLDED_ONLY`, and any other dated marker warns `FUTURE_FORMAT` and **names the marker rather than guessing** — that is #702 at this seam.

Measured against the live `.dreamwork/questions.md` (read-only; I ran this, you should reproduce it):

| entry | line | arrow head | post-#767 markers |
|---|---|---|---|
| #572 | 155 | NONE | `Answer 2026-07-31` ×2 |
| #614 | 205 | NONE | `Comment 2026-07-31` |
| #613 | 292 | NONE | `Answer 2026-07-31`, `Comment 2026-07-31` |

These are the **same three entries #767 examined and ruled HEALTHY**. So increment 7's headline "3/67 answered entries missing a resolution date" is a parser artifact, not a live-data finding.

## What you are fixing, and the part that is not optional

**The defect is not the regex. The defect is that there are two copies of the vocabulary.** A lane fixed lint in #767; a module written afterwards re-derived the rule from scratch and got the retired version. Patching `question_parse.py` to match lint today leaves exactly the same drift free to happen a third time.

So: **extract one source of the resolution-marker vocabulary and make both `lint.py` and `question_parse.py` consume it** (#440 — one supported way). Where it lives is your call; argue it in your report. What must be true when you are done is that there is no second place where a resolution marker is recognised, and adding a future marker form means editing one definition.

Carry #767's full outcome set across, not just the happy path: a recognised dated resolution, `FOLDED_ONLY`, and `FUTURE_FORMAT` **naming the marker it could not classify**. A parser that silently returns `None` for a marker it does not recognise is the exact failure that produced this task — `None` must mean "the source carries no resolution", never "I did not recognise what the source carries" (#702).

## Re-measure the denominator, and say what it is

Increment 9 will build on this number, and right now the recorded number is wrong. After the fix, re-run the dry run against the live `questions.md` **read-only** and report the true count of answered entries with no recorded resolution. If it is zero, say zero — a check that now reports nothing wrong on healthy input is the correct outcome, not a suspicious one (#755: a check that fires on a healthy input is worse than no check).

## The fixture rename you inherit from #645 increment 8

`test_dreamwork_db_import.py` (landed at `ef909220`) builds synthetic fixtures wearing the real ids `#572`/`#613`/`#614`, and its class docstring at line ~328 reads *"The three Answered entries with no resolution date (#572/#613/#614)"*. That claim is false and it sits where it reads as a citation (#764 — a citation must carry its own evidence).

**Renumber those fixtures to ids the live ledger does not use, and correct the docstring.** Do **not** delete the tests: the behaviour they cover — an entry with no recorded resolution imports as null, idempotently, and verifies clean — is correct and must survive. Only the ids and the false characterisation go. `test_dateless_bodies_have_no_arrow_head` deserves a second look specifically: after your fix, "no arrow head" is no longer the same thing as "no resolution".

## Lessons that bear on this task

Selected for you — do not go looking through `lessons.md`, it is thousands of lines and this is the slice that applies:

- **#440 — one supported way.** Two copies of a rule are a bug that has not fired yet. This task is that bug firing.
- **#702 — report what you cannot classify.** An unrecognised marker is reported by name, never defaulted to absent.
- **#651 — a name must not imply more than it proves.** Increment 7 proved byte-span losslessness and called the module lossless; a derived field walked straight past the proof. Your fix should make the field's correctness provable, not assumed.
- **#764 — a citation must carry its own evidence.** The fixture docstring above.
- **#755 — a check that fires on a healthy input is worse than no check.** Three healthy entries were reported as defective for hours.
- **#759 — hold the SUBJECT fixed, vary the INTERPRETER.** Your red-proof must not re-derive the expected answer from the code under test.

Run `python3 dev/lessons_index.py --act red-proof` before your first injection.

## Red-proof, both directions

Direction 1 is easy here and you must still do it: injecting the retired arrow-only rule back into the shared vocabulary must red on a **discriminating** message that names a marker form, not on a bare count mismatch.

Direction 2 is the one that matters: construct an input where `resolution_date` is populated but wrong — a date lifted from prose, or from a `Folded` line when #767 says `Folded` alone is not a resolution — and show what your check decides. A parser that reports *a* date is not the same as one that reports the *right* date.

## Contention — read this before you touch lint.py

`#773` **landed in `lint.py` at your base** (`brief_corpus_reach`, ~line 3875, now corpus-fingerprinted). No lane is live in that file as you start, so you are not racing anyone — but that region was edited very recently, so:

- Touch **only** the resolution-marker definition and its call sites. Do not reformat, reorder, or tidy anything else in `lint.py`.
- **Rebase onto master before you finish**, and resolve any conflict yourself — do not hand a conflicted branch back. If master moved, re-run the full check after rebasing, not before.
- If you find you cannot make the shared-source change without broad edits to `lint.py`, **stop and report that** rather than making them. Fixing `question_parse.py` alone plus a written argument for where the shared source belongs is an acceptable outcome; a large unrelated `lint.py` diff is not.

## Bars

- Worktree lint bar is **5 warnings**, not 1. Measure it at your base before you start and quote what you measured — do not trust this number or any number in a note (#756).
- Count tests by collecting, not by grepping `def test_`.
- `just test` is the gate. Limit to 2 threads.
- Do not use `attn`.
