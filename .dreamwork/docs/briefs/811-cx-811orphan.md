# BRIEF — #811: 385 lines of citation tooling that nothing runs, and nothing would notice if it broke

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-811orphan
Branch: cx-811orphan
Base sha: 05f7f902
...
Lane-owns: `dev/check_watch_citations.py`, `dev/apply_reanchors_i3.py`, `dev/repo_wide_guards.py`,
`justfile`, and any test file you add for them. **Do NOT touch** `watch.py`, `test_watch.py`,
`client/*`, `test_guard_argv.py`, `test_dreamwork_db_import.py` — four other lanes hold those.

**Task:** `python3 dev/ledger.py get 811 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`
**Note the `--ledger` form**; a bare `get` refuses from a worktree.

## The measurement

Two tools exist and **nothing references either**:

- `dev/check_watch_citations.py` — 242 lines, landed by `#801` today.
- `dev/apply_reanchors_i3.py` — 143 lines, same shape.

Neither is named by a test, by `dev/repo_wide_guards.py list`, by the `justfile`, or by `lint.py`.
I verified this by search; **verify it yourself before acting**, because the whole task turns on it
and a stale premise would send you the wrong way.

## Why this is a real defect and not tidiness

An unreferenced tool is not merely unused — **it is unverified, and it decays silently.** The citation
anchors it operates on are `(path, line, symbol)` triples into `watch.py` and `client/router.js`, and
those files moved **29 times today**. A tool nobody runs against a corpus that moves hourly is
guaranteed to be wrong by now, and the first person to trust it will be the one who finds out.

Worse, its existence *implies* coverage. Someone auditing "do we check citations?" finds a
242-line checker and concludes yes. That is `#671`'s shape at the repository level: an artifact that
reads as a positive all-clear while examining nothing, because nothing invokes it.

## The decision I am NOT pre-making

Three honest routes, and I want the choice argued rather than assumed:

- **(a) Wire them in** — register in `dev/repo_wide_guards.py` and/or the `justfile`, with tests. Cost:
  they must actually work, and if they are already stale you inherit fixing them. Benefit: we get the
  checking the code was written to do.
- **(b) Delete them.** Cost: `#801`'s work is discarded. Benefit: honest — the repo stops claiming
  coverage it does not have.
- **(c) Mark them explicitly as one-shot migration scripts** — a header stating they are not standing
  checks, plus whatever makes that non-misleading. Cost: a third category to maintain.

**Run each tool first and report what it actually says today.** That measurement should drive the
choice: a tool that still passes cleanly argues for (a); one that is already incoherent argues for
(b) or (c). `#763` binds — read each tool's own docstring and `#801`'s landing note before calling a
deliberate design a defect; `apply_reanchors_i3.py`'s name suggests a one-shot migration, which would
make (c) correct for it and (a) correct for the checker. **Judge with IGC** using the worktree-local
`./igc-method.md`, and it is entirely acceptable to reach different verdicts for the two files.

## Red-proof

- **If you choose (a):** direction 1 is a broken anchor that the registered check catches with a
  **discriminating** message naming *which citation failed to resolve* — and you must **name the
  production seam you break, path plus symbol or branch**. Direction 2, mandatory: a checker that
  resolves ZERO citations and reports success passes vacuously forever — assert it examined a
  non-zero, plausible population. `#816` (live in another lane) is that exact failure at 388/388, so
  it is not hypothetical.
- **If you choose (b) or (c):** direction 1 is a guard that the repo contains no unreferenced
  executable in `dev/` — sabotage it by re-adding an orphan and watch it red. Direction 2: such a
  guard that derives an empty candidate set passes forever; prove it found the real population.
...
- **`#671`** — a check that examined nothing must not read as passing. The core of this task.
- **`#611`** — a check that did not run must say so; absence reads as a pass.
- **`#764`** — a citation that rots, and what a false measurement teaches its readers.
- **`#763`** — read the owning docstring before calling a deliberate decision a defect.
- **`#440`** — one supported way; if you wire them in, join the existing registration ceremony rather
  than inventing a second.

## Run these, explicitly

```
python3 dev/check_watch_citations.py ; echo "rc=$?"
python3 dev/apply_reanchors_i3.py --help ; echo "rc=$?"
python3 dev/repo_wide_guards.py list
just pytest $(python3 dev/repo_wide_guards.py list)
python3 lint.py | tail -1
```

**`apply_reanchors_i3.py` may MUTATE files** — read it before running it without `--help`, and if it
has no dry-run mode, say so rather than running it live in your worktree.

## Bars

**Master is GREEN as of `05f7f902`** on targeted files; a full gate is in flight.
...
- Rebase onto master before you finish. Master moved 29 times today and four other lanes are live.
