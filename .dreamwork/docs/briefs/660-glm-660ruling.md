# BRIEF — #660: a standing ruling stated wider than the argument that justifies it

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/glm-660ruling
Branch: glm-660ruling
Base sha: 05f7f902
...
Lane-owns: `.dreamwork/docs/plans/orchestrator-posture.md`,
`.dreamwork/docs/plans/posture-autonomy-axis.md`, and any test you add for them.
**Do NOT touch** `watch.py`, `test_watch.py`, `client/*`, `dev/repo_wide_guards.py`, `justfile`,
`test_guard_argv.py`, `test_dreamwork_db_import.py` — five other lanes hold those right now. If your
fix genuinely needs one, STOP and report back rather than working around it; a brief whose scope
cannot express its deliverable is my error to fix, not yours to route around.

**Task:** `python3 dev/ledger.py get 660 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`
**Note the `--ledger` form** — a bare `get` refuses from a worktree.

## The defect

`orchestrator-posture.md:234` and `posture-autonomy-axis.md:277` state the `#445`/`#342`
**widen-not-sibling** ruling **unconditionally**: widen `posture`, never a sibling file, never a
second route, never a second arm.

The justification the docs themselves quote is **conditional**:

> *"a widening ... lets the closed-set discipline already guarding pace/asking guard this **for free**"*

That premise is **false by construction for a free-text field.** A free-text value inherits nothing
from a closed-set validator, because there is no closed set. So the ruling's stated scope is wider
than the argument that supports it.

**The transferable point — and it is the real deliverable — is about how rulings are RECORDED, not
about this decision:** a standing ruling whose premise is narrower than its statement will be either
wrongly obeyed or quietly broken by the next person who meets the gap. **Both outcomes are bad and
neither is visible.** That is what to fix.

## This is live right now, which is why it is worth doing today

Two things are testing this exact gap as you read it:

1. **`#646` landed a free-text `subagent-policy` field** — and it was deliberately built as a
   **separate file and a separate route**, i.e. the ruling as stated forbids what actually shipped and
   what Max is using. He set a policy through it at 15:21 today.
2. **`#818` is live in another lane deciding** whether chat delivery should widen `posture` with a new
   axis or take a different route. It will read these documents.

So the fix has a consumer, immediately. **Do not resolve this by deleting the ruling** — it is right
for closed-set axes, and `#440`'s one-supported-way pressure is real. Make its **scope** match its
**justification**, and make the free-text case explicit rather than an unstated exception.

**`#763` binds hard here:** read the owning decision (`#445`, `#342`, and `#646`'s design note) before
you call any of this a defect. If you conclude the ruling was always meant to cover free text and it
is `#646` that broke it, **say so and hand the conflict back to me** — that is a legitimate finding
and I would rather have it than a doc edited to agree with whichever side you read last.

## Red-proof

A doc-only change still gets both directions.

- **Direction 1:** whatever guard or test you add, break it and watch it red on a **discriminating**
  message — the failure must name that a ruling's stated scope exceeds its recorded justification, not
  merely that a string was absent. **Name the seam you break: path plus symbol or branch.**
- **Direction 2, MANDATORY:** construct the case where the fix **looks** applied but is not. The
  obvious one here: a check that greps for the literal phrase "free text" passes the moment someone
  rewords it, and passes vacuously if it scans zero documents. **Assert your check examined a
  non-zero, plausible population** — a doc guard that finds no documents and reports success is
  `#671` exactly, and `#816` is a live task about that failure at 388/388.
...
Cite these by **content**, not by my wording — verify each before relying on it. I miscited an id
three times today and three separate lanes caught me, so please do check.

- **`#671`** — a check that examined nothing must not read as passing.
- **`#763`** — read the owning decision before calling it a defect; and when a brief overrides a
  standing contract it must say it is overriding it.
- **`#764`** — a document that ships a false or over-broad claim teaches its readers that its claims
  are decorative.
- **`#440`** — one supported way. The reason the ruling exists; do not discard it.
- **`#651`** — a guard's message must name a mode the guard can actually detect.

## Run these, explicitly

```
just pytest test_lint.py
just pytest $(python3 dev/repo_wide_guards.py list)
python3 lint.py | tail -1
```

**The doc-map is TEST-ENFORCED** (`test_lint.py::TestDocMapPlans`) — if you add or rename a plans
doc, `python3 lint.py | grep doc-map` before and after.

## Bars

**Master is GREEN** — a full `just pytest` at `05f7f902` read **2849 passed, 0 failed** in 359s.
Do not read that as reassurance: `#809` landed today demonstrating two mutations that survive their
entire relevant test file.

- Main-checkout lint reads **1** — the `lessons.md` near-duplicate, which is **human-gated and must
  stay**. A worktree reads ~5. `python3 lint.py | tail -1` — the count is in the **trailer** and the
  rows are **indented**, so `grep -c '^WARN'` returns a false **0**.
...
- Do not bind `:35110` (deployed dashboard, live — he is using it right now) or `:35113` (dev).
- **Do not use `attn`** — report to the coordinator inbox and let me decide what reaches the human.
- Rebase onto master before you finish and resolve conflicts yourself. Master moved 30 times today
  and five other lanes are live.
