# BRIEF — #776: the dispatcher validates the prompt's shape but not whether what it tells the lane to read exists

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-776cite
Branch: cx-776cite
Base sha: 2f57cd8e
...
**Task:** `python3 dev/ledger.py get 776 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — the entry states the case, both occurrences, and the scope limit. Read it before designing.

## What happened, twice, in one session — both mine

1. I briefed a lane that three named entries were "live data, not a parser artifact". A fix landed hours earlier had already refuted that. The lane built fixtures around three real ids under a false characterisation.
2. I instructed a lane to run `ledger.py get 199`. **`#199` is not a ledger entry** — it is a lesson reference, and the command refuses. In the same brief I said `watch.py` is "~15k lines"; it is 6255, because the client extraction landed and I quoted a pre-extraction number.

The irony is the point: both briefs told the lane *"do not trust any number in a note, measure it yourself (#756)"* while the prose around that sentence carried unmeasured claims. The rule was aimed outward only. **The lanes caught both** — the route worked, the brief was wrong.

## Build exactly this, and no more

`dev/dispatch_lane.py` already validates the delivered prompt and **already fails closed**. Extend that one gate:

- **Refuse** when the brief contains an instruction to RUN a ledger lookup — the `ledger.py get <N>` form — for an `<N>` that does not exist in the ledger. That is a command the lane is told to execute which is guaranteed to fail, so refusing costs nothing and cannot be a false alarm.

**I measured the scope limit for you, because the obvious widening is wrong.** My own filing note proposed resolving "every `ledger.py get <N>` / `#<N>` instruction". The second half of that is a trap. Bare `#N` citations are everywhere in briefs and they are *mostly* real tasks — I checked: `#671`, `#440`, `#755`, `#136`, `#349` all resolve; `#199` does not. But citing a retired or non-task id **in prose** is legitimate ("#199's rule", naming a lesson), so refusing on it would fire on a healthy brief — **#755, and #707: every widening multiplies false attribution.**

So: a bare `#N` that does not resolve is **reported in the dispatch output and does not block the launch** (#702 — say what you could not classify, rather than either refusing or staying silent). Refusal is reserved for the executable form. If your reading of the corpus says even that split is too broad, say so with the counts and propose narrower — an argued narrower scope is a better outcome than a check that cries wolf.

## I prototyped the refusal against this brief and it FAILED — read this before you build

Before dispatching, I ran the proposed rule over this very prompt. It flagged `#199` as a nonexistent id and would have **refused this brief** — because the brief *quotes* `ledger.py get 199` as the example of the bug rather than instructing anyone to run it.

That is not a detail to patch. It is the design problem: **an instruction to run a lookup and a quotation of a failing lookup are the same bytes.** Any brief that documents this class of mistake — a lane report, a lessons entry, a future brief about a stale citation — contains the failing form on purpose. A gate that cannot tell those apart will refuse exactly the briefs that are most careful, and refusal is a hard stop on the loop's only dispatch route.

So treat the refusal as **unproven, not as the specification**. Your first job is to decide whether it can be made safe:

- If you find a rule that reliably separates instruction from quotation, build the refusal and **show it passing on this brief** (`.dreamwork/docs/briefs/776-cx-776cite.md` after dispatch — your own prompt is a first-class fixture and it is the adversarial case).
- If you cannot — and I think that is the likely honest answer — then **build the report instead of the refusal**: the dispatcher prints every `ledger.py get <N>` whose id does not resolve, names it, and launches anyway. That still would have caught both real occurrences, because I read dispatch output. It cannot ever block a good brief. State plainly in your report which one you built and why, with the evidence that decided it.

A reported-only check is a legitimate, complete outcome for this task. Do not build a refusal you cannot prove safe merely because the entry proposed one.

**Explicitly NOT in scope:** parsing prose for line counts, file sizes, lint bars, or any other natural-language numeric claim. That is a detector for arbitrary assertions, it would fire on healthy briefs, and the honest remedy for the numeric class is a coordinator habit (measure at dispatch or omit the number), not a check. Do not build it. If you think you see a cheap safe subset, **report it rather than implementing it.**

## Red-proof, both directions

- **Direction 1:** feed the gate a prompt containing `ledger.py get 199` (a real, historically-occurring case) and watch it refuse with a **discriminating** message that names the id and says it does not exist — not a generic validation failure.
- **Direction 2 is the one that decides whether this check is worth having.** Construct a **healthy** brief that cites several real ids in prose plus one legitimately-retired id, and prove the gate lets it through. A dispatch gate that refuses good briefs is far worse than the bug it prevents, because it blocks the one route the loop has (#755, #440). Show what your check decides on both.
...
This gate is **the only supported dispatch route** (#440). Every lane in the fleet launches through it, and a refusal is a hard stop with no fallback. Your change must not:

- slow the launch by a ledger query per bare `#N` — the executable form is rare, so resolve only what you must;
- refuse when the ledger is unreadable or locked. **An unavailable ledger is not a bad brief.** Decide and state what happens then; degrading to "allow, and report that the check did not run" is almost certainly right, and a silent pass is not — a check that examined nothing must not read as passing (#671).

`test_dispatch_lane.py` collects **22** at your base (measured today). The existing validation tests are your regression net; keep them green.

## Lessons selected for this task

- **#755 — a check that fires on a healthy input is worse than no check.** The governing constraint.
- **#707 — every widening multiplies false attribution.** Why bare `#N` refusal is out.
- **#702 — report what you cannot classify.** What to do with an unresolved bare citation instead.
- **#671 — a check that examined nothing must not read as passing.** The unreadable-ledger case.
- **#440 — one supported way.** This gate is it; breaking it stops the whole loop.

## Bars

**Measured in your worktree at dispatch, 2026-08-01, base `2f57cd8e`:** worktree lint **5 warnings**; `test_dispatch_lane.py` collects **22**. Re-measure and quote what *you* got; if we disagree, your numbers win and the disagreement is worth reporting (#756).

- Count tests by collecting, not by grepping `def test_`.
- `just test` is the gate. Limit to 2 threads. Machine load is ~22 from unrelated work today — if a timing-sensitive check is flaky, say so rather than retrying until green.
