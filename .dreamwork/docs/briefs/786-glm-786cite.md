# BRIEF — #786: my briefs cite real task ids for principles those ids do not contain

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/glm-786cite
Branch: glm-786cite
Base sha: dc739001
...
Lane-owns: .dreamwork/lessons.md, dev/citation_audit.py (new, name it what you like),
test_citation_audit.py

**Task:** `python3 dev/ledger.py get 786 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`

## This is my defect, found by a lane checking a citation I gave it

Lane `cx-783cover` was handed "#755 — a check that fires on a healthy input is worse than no check"
in its lesson block. Instead of taking it, it looked: #755's real title is *"status.json's
`queued_dispatches` names task ids that nothing verifies are still open"*. It reported the mismatch.

**Measured at `ccc59791`, and re-derive these rather than trusting them:**

- `#755` real subject = queued_dispatches rot. `grep -c "healthy input"` on its entry → **0**.
- `"fires on a healthy input"` in `.dreamwork/lessons.md` → **0**. In the whole ledger → **0**.
- It appears in **19** briefs under `.dreamwork/docs/briefs/`, always attributed to `#755`.
- `#136`'s real title is *"A questions.md that parses to nothing must say so"*; my briefs render it
  as *"a signal that always fires is one nobody reads"* — **0** hits in its entry. **30** briefs.
- Corpus is **245** briefs.

**Not everything is wrong, and the fix must not assume it is.** I checked the routine set: `#671`,
`#651`, `#707`, `#440`, `#756`, `#764`, `#465`, `#606`/`#471`/`#666` all match what I claim they
say. `#349`'s title is about lesson retrieval but its body does carry the git-checkout rule, so it
is loose rather than false. Verify my triage; a citation I marked fine may not be.

## The principles are sound. Only the citations are false.

*"A check that fires on a healthy input is worse than no check"* is a real and load-bearing idea —
#763's first refusal rests on it and it was right. So **an unhomed principle needs a HOME, not a
deletion.** For each one:

1. Find the id or `lessons.md` line that genuinely states it and re-point the citation, **or**
2. Write the lesson so the citation becomes true.

Prefer (1) where the home exists. Where you take (2), write it in `lessons.md`'s own voice with the
evidence that earned it — several of today's tasks are that evidence.

**`lessons.md` has a standing constraint you must respect:** pruning or merging entries is Max's
call, not a lane's (its own near-duplicate row says so, and lint has flagged that row for weeks
deliberately). **Adding** a lesson is fine. **Do not remove or merge anything.**

## The trap in the mechanical half

The tempting fix is a dispatch-time check: `dispatch_lane.py` already resolves ledger references in
a brief head, so "this brief cites #N; does #N's text support the wording beside it?" is at least
askable. **Two reasons to be careful, and I want your judgement, not compliance:**

- A fuzzy text-match would fire on every legitimate paraphrase. Briefs *should* paraphrase — the
  four-to-six-lessons block exists to compress. A check that reds on healthy paraphrase is the exact
  failure this task's own misquoted principle warns about, which would be a fine joke and a bad tool.
- **`dev/dispatch_lane.py` is owned by another lane this tick (#701).** So do NOT edit it. If you
  conclude a dispatch-time check is right, **report the integration point and the exact predicate**
  for a follow-up, and land your tool standalone so it can be wired later.

An acceptable outcome is: the corpus audited, the false citations named, the principles rehomed, and
a standalone tool that reports what it could not classify. A reasoned argument that no automatic
check can do better than the audit is also acceptable — say so with the measurement.

## What "report what you cannot classify" means here

A citation you cannot resolve is a finding, not a guess. Do not invent a plausible id to make a
brief look consistent — that is the defect, performed once more.

## Lessons selected for this task

- **A citation must carry its own evidence** — the parent principle. Its entry is `#764`, whose
  title is literally *"The fix for miscited task ids was to cite lessons.md:<line> — and line
  citations rot too"*. This task is that entry's subject recurring in a new medium, so it is the one
  citation here I am confident about.
- **A name must not imply more than it proves** (`#651`) — a lesson block implies a lane can go read
  the authority.
- **A check that examined nothing must not read as passing** (`#671`) — an audit that skips
  unresolvable citations must say it skipped them.
- **Every widening multiplies false attribution** (`#707`) — the bound on any text-matching check.
