# BRIEF — #794: my own lint guidance teaches a comparison that hides new warnings

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-794lintrow
Branch: cx-794lintrow
Base sha: a564bd80
...
Lane-owns: `briefs/boilerplate.md` (the lint-guidance paragraph ONLY), `doc-map.md`,
and `lint.py` **only if** you conclude the tool itself should help — argue it first.

**Task:** `python3 dev/ledger.py get 794 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`

## This corrects guidance I have put in every brief this session

I have been telling every lane: *read the bar with `python3 lint.py | tail -1`, because the count is
in the trailer and the rows are indented, so `grep -c '^WARN'` returns a false `0`.* That is true, it
fixed a real defect, and **it must stay**. But as stated it is incomplete, and the incompleteness is
the same species as the defect it fixed.

**Measured, 2026-08-01:** while `cx-368modular` worked, the main-checkout trailer moved **2 → 1**
(#787 retired a citation row on master) while its worktree still read **6**. A **new** warning that
lane created — `doc-map.md` omits the `modularity-and-startup` plan — replaced a warning that had
disappeared. **The count was stable-to-falling while the composition changed.** A lane comparing
trailers would have reported "no new warnings" and been wrong. That lane caught it only by reading
every row.

Confirmed at the merge: main-checkout lint now reads `clean (2 warning(s))` and the second row is
exactly that `doc-map.md` omission.

**Re-derive this yourself.** `python3 lint.py` and read the rows; compare against
`git stash`-free means (do not stash — the stack is shared across worktrees). Report what you got.

## Two deliverables

1. **Correct the boilerplate's lint paragraph.** A lane must compare the **row set**, not the number.
   Keep the indented-rows warning verbatim — it is still true and still load-bearing; a lane that
   greps `^WARN` still gets a false 0. This paragraph is read by every lane on every dispatch, so it
   is a change to the standing contract: keep it minimal and **quote the before/after in your report**.
2. **Add the `doc-map.md` plans row for `modularity-and-startup`.** This is the concrete outstanding
   instance and it is why the bar is 2 rather than 1. `cx-368modular` correctly did not own
   `doc-map.md` and flagged it rather than reaching for it.

## The judgement call I am NOT pre-making

Should `lint.py` itself help — e.g. by making row-set comparison easy, or by emitting a stable
machine-readable form a lane can diff? **Argue it with IGC** (worktree-local `./igc-method.md`).
Weigh it against the cost: `lint.py` is run by every lane and by my merge gate, and a change there is
a change to the loop's most-used instrument. **An argued "the contract fix is enough, do not touch
the tool" is a fine answer** — say why.

## Red-proof

- **Direction 1:** if you change `lint.py`, break the new behaviour and watch it red on the
  **discriminating** message. If you only change docs, then direction 1 is: show that following the
  OLD guidance misses a real new warning, and following the NEW guidance catches it — construct that
  pair concretely rather than asserting it.
- **Direction 2, mandatory:** construct the case where the corrected guidance **still** misses
  something. Obvious candidate: two warnings that swap in the same run so both count AND row-text
  sets shift in ways a naive diff reconciles; or a warning whose text embeds a changing number so it
  never compares equal and trains the reader to ignore diffs (that is the `#786` principle, now
  homed in `lessons.md` — cite it by content, and note it fires on THIS task's own remedy).
...
- Master is **GREEN** at `a564bd80` (gate20: 2789 passed, 0 failed, at `176bc4cb`; three merges since).
  Main-checkout lint reads **2** — and **your deliverable 2 should take it to 1**. Do NOT drive it to
  0: the survivor is `lessons.md:580`'s near-duplicate of `:622`, **permanently human-gated** — its
  own text says merging is Max's call. A lane that removes it has pruned a lesson he reserved to
  himself.
- Count tests by **collecting**, not by grepping `def test_`.
- **Cite lessons by CONTENT, not by my wording.** Three lanes today found ids in my briefs whose
  entries do not carry the principle I attributed (`#755`, `#136`, `#702`, and `#671`-vs-`#611`).
  Verify before relying, and report mismatches rather than reconciling to me.
- Limit builds and tests to 2 threads. Do not bind `:35110` or `:35113`.
- **Do not use `attn`** — report to the coordinator inbox.
