...
- **#294** — Migrate the durable task ledger to SQLite and a tool/CLI API · P1 ·
  ...
  **entry/task SPLIT** (`entry` + `task` + `task_by_entry` join), following `#346`'s ratified design literally.
  ...
  · inc5 merged (d04eae77): R3 git-history import — `--import-history` recovers groomed
  ids as rows + synthetic first-sight/landed events (actor=`migration:git` only, R3
  honored), journal-contract hash chain (`prev_hash` load-bearing), `--verify` post-R3
  consistency (a groomed row needs event backing, else fabrication). Live acceptance on
  654 commits: **70/74 recovered (140 events)**; `#101` `#222` `#223` `#267` NEVER headed
  an entry in any commit — born as bare spans, so no row and no event (the contract
  forbids fabricating NOT NULL bodies), reported as unrecoverable. `ledger_store.py`
  untouched (no schema change). Coordinator red-proof: neutering the chain comparison
  fails `test_verify_history_chain_tamper_fails`, restored byte-identical. 85 migration
  tests green. **REMAINING: the cutover only** (R2 exclusive lease + re-point readers +
  R4 rename/shim). Lane model: llmp-glm-5-2 (1668s, 100 calls).
  · **coordinator correction 19:04**: my inc4 fold-call moved this entry to Recently
...
- **#454** — questions collapse to a rolled-scroll card of 5-6 lines, persisted like other UI state ·
  ...
  **human via watch 2026-07-29 01:06:** *"questions on the questions page should be collasible. However, the
  ...
  · merged (dc9ae802): rolled open question cards — runtime-measured --rollh clamp (5.5 rendered lines via lineHeightOf probe, no guessed ratio), CSS-mask scroll edge, IndexedDB persistence + cross-tab sync, gesture on the #111/#169 idiom with reduced-motion parity. Lane: 5 red-proofs, 12/12 guards. Coordinator: independent red-proof on the affordance line (revert fails test_page_has_qroll_wiring, restored byte-identical), qroll+qfocus guards PASS on master, 332 pytest green, audit-styleguide exit 0. Awaiting cards deliberately get no roll. Model: inherited (pre-routing-rule dispatch).
...
- **#452** — focus a single question on its own page · **P2** · dashboard/asking · origin: **human** ·
  **human via watch 2026-07-29 01:04:** *"should be able to focus on a question, like open up to a page showing
  ...
  · landed 8881334f (merge of wt/452; lane work c05de30e..eb3eb293, 8 commits, native subagent). /question?qid=<title> — a page showing only that question, which the loop's churn cannot move. Key = the title (the same string data-qid already carries): survives body rewrites, priority re-sorts, and the open->answered fold — resolution searches BOTH questions_open and answered_entries, so the page FOLLOWS the question across the fold instead of reporting a live question as gone. A retitle degrades honestly: .qmissing says 'most likely re-titled or removed', substitutes nothing (a near title is a different question), links back — the #402 rule at dim-rail level. #294's future question_id can be accepted beside the title without invalidating a link. qfocus guard registered (63): 21 checks incl. arrival dissolve via transitionstart (load-independent, #442), answering from the focused surface, tick-follows-fold, RM parity. The guard found a REAL bug in its own harness: the expand handler was registered before the router's and its preventDefault swallowed folded-card focus clicks — now its own named check. Lane: 6 red-proofs, no green red-runs. Coordinator independently verified on the merged tree: qfocus PASS; red-proof shadowing the .qmissing return fails exactly the 3 missing-state checks and nothing else, watch.py restored byte-identical; 324 test_watch pass; qacard/docktarget/noteprop (the strip-list consumers) reported PASS by the lane.
...
- **#471** — a single guard cannot be run on its own, and I cannot explain why the full run disagrees · P2 ·
  ...
  · closed 2026-07-29 15:36, verified by the current coordinator: 9c768eef (the explanation + guard-port fix) and 80dc4a40 (run-and-judged accounting) both resolve, and 80dc4a40 IS the successor the entry's final bullet asked for — a run reporting which registered guards actually ran and judged. It has demonstrated itself repeatedly: it named artifactwrap/burndownmock as never-judged in #475's first run, and every guard run today prints 'N of N registered guard(s) ran and judged'. The single-guard isolation capability and the explanation (#461's argv[3] pin forcing own-server guards onto the shared port) are both landed and in daily use.
...
- **#478** — the cited-sha check declines to run in the full suite, twice, and says so at OK · P2 ·
  ...
  · landed 7ba7caa (merge of wt/478; lane work 5d4dc00, native subagent). The cited-sha full-suite flake was NOT suite env — both handoff suspects refuted with evidence (GIT_DIR/WORK_TREE grep-clean and cannot produce the signature; the batch-check len-guard WARNs rather than going silent). The real cause: TestCitedShas.build() takes HEAD's 7-char prefix, and ~3.7% of commits are all-digit, which the check's PID filter (requires a hex letter) drops as not-a-sha — collapsing the collected list to one sha, so the all-missing branch (#380) reports wrong-tree at OK. Fix: build() re-rolls until the prefix discriminates, asserted; a pinning test records the mechanism and confirms the discriminating case (live cite collected, dead neighbour still WARNs). Lane: 160/160 post-fix. Coordinator: 351 test_lint.py pass on the merged tree, plus 20/20 independent runs of the previously-flaking test — deterministic.
...
- **#421** — how we ask him questions, researched rather than guessed · P1 · loop-instructions ·
  ...
  *questions* entry and `DREAMWORK.md` but not back into the *ledger* — the fold has three destinations and
  only two were written. **Checked, and nothing remains:** `file-formats.md:141-161` already states the
  shape B parses — a sub-decision is **declared, not guessed from prose**, and a fold carrying one forward
  must NAME it. So `#421` is **done**, and the only outstanding work on it was discovering that.
...
- **#446** — a second `Answer` on a question **overwrites the first**, and the text is gone before anything can
  ...
  · **RE-VERIFIED 2026-07-29 05:12, and the two mirror questions this entry asked are now ANSWERED (lane `wt/answerloss`, no commits — the finding was stale and it said so).** It re-proved the fix on the production line rather than reading the diff: reinstating the overwrite at `cur["answers"].append(rec)` in `_parse_entries` fails with the first answer's text **named in the message** and an empty list beside it, which is the right assertion shape — this entry warned that a count of 2 passes on a parse that kept the wrong one, and the check asserts both texts with the fixture's two strings derived and asserted unequal. **The mirrors are clean and that was an open question, not an assumption:** in `answers.md` `lift_answer=False` means two `Answer` bullets stay separate `follows` contributions with no overwrite path, and `## Answered` behaves the same — so the defect was specific to the answer *lift*, not to the grammar. Tree left byte-identical; live `questions.md` untouched.
