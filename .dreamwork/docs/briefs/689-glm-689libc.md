# BRIEF — #689: watch.py computes which libc resolved and throws the answer away

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/glm-689libc
Branch: glm-689libc
Base sha: dc739001
...
Lane-owns: watch.py (the statx/libc resolution region ONLY), test_watch.py

**Task:** `python3 dev/ledger.py get 689 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`

## The whole defect, in two lines of production code

`#680` (merged `8f45a85f`) made `_resolve_statx_libc()` return `(libc, label)`, where the label names
which resolution won — `"CDLL(None)"`, `"find_library: libc.so.6"`, or `"unresolved: ..."`. **That
reporting was the point of #680**, because a silent `CDLL(None)` loads the main program image with
no real libc behind it and is otherwise indistinguishable from a real resolution.

The sole production call site is:

```python
libc, _resolved_as = _resolve_statx_libc()
```

and `_resolved_as` is never read. So the fact is computed and discarded, and the silence #680 was
filed to end is still there in production.

**Verify before changing:** grep it yourself. The entry reports three occurrences in `watch.py` —
the definition, a docstring mention, and that binding. Confirm that is still true at your base.

## The finding underneath it is the more valuable half

**#680's two tests exercise `_resolve_statx_libc` DIRECTLY**, so they pass while the production path
throws the answer away. The helper can never regress; the CALL SITE can drop the value without a
single test noticing — and it already has.

That is a clean instance of a class worth naming in your report: **a test that binds the helper
rather than the use proves the helper works and nothing about whether anyone uses it.** Whatever you
land, your own test must bind the **call site**, or you will reproduce the exact defect you are
fixing one layer up.

## Scope

Small. Make the resolved label reach somewhere a human or a check can see it — that is the
deliverable, and where it should surface is your judgement to argue: a startup log line, the
dashboard's data, a diagnostics route. **Weigh noise honestly**: this resolves once at import on
every start, so a line that prints unconditionally on a healthy glibc box is noise nobody reads,
while one that only speaks when the resolution was the suspicious `CDLL(None)` fallback may be the
better shape. Say which you chose and why.

Do not refactor the loader; `#680` settled its form and that ruling stands.

## Red-proof

- **Direction 1:** force each resolution outcome and show the label reaches your chosen surface,
  naming which one won. Break the wiring and watch a test red on the **call site**, not the helper.
- **Direction 2:** the false-green `#680` itself left open, quoted from its report — *"the
  acceptance test trusts the label STRING but nothing BINDS the label to the lib actually returned.
  A loader that returns `CDLL(None)` while reporting `find_library: libc.so.6` passes the check."*
  Say plainly whether your work closes that, narrows it, or leaves it untouched. Leaving it is fine;
  implying it is closed is not.

## Lessons selected for this task

- **A check that examined nothing must not read as passing** (`#671`) — a test binding the helper
  while the call site drops the value.
- **A name must not imply more than it proves** (`#651`) — the label must name the load that
  actually happened.
- **Report what you cannot classify** (`#702`) — `"unresolved: ..."` is a real outcome and must
  surface as one.
- **A signal nobody reads is no signal** — the noise question above. (Number under audit, #786.)
