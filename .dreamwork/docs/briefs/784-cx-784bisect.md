# BRIEF — #784: audit the bisect method that confidently named a revision the evidence clears

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-784bisect
Branch: cx-784bisect
Base sha: dc739001
...
Lane-owns: a new `dev/` tool and/or `.dreamwork/docs/` procedure doc of your choosing, plus its test.
You may READ `dev/capture/*` but do not edit it.

**Task:** `python3 dev/ledger.py get 784 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`

## The failure is mine and it is fully documented

I bisected #781 to *"PASS at `947531af`, FAIL at `2730557f` (the client extraction)"* and briefed it
as established, adding *"the bisect is already done; do not redo it."* **It is wrong.**

Lane `cx-781guards` ran qroll from an archive of `2730557f` and every assertion **PASSED**. It also
measured the extraction's byte-identity claim directly: `watch._get_page()` is **576,217 bytes**,
SHA-256 `08d4e0bf33cb02cbd045267d1a830001d3c074cb32bb2abba64cf83c90d0615f`, **identical at both
revisions**. So the extraction changed nothing observable and could not have caused it. The real
cause was later stale-response sequencing plus later delta-envelope consumption, both since fixed.

**It found this only because the standing red-proof direction-2 requirement made it check the healthy
tree anyway.** A lane that obeyed my line would have patched the wrong seam.

## Your subject is the METHOD, not this answer

The answer is already corrected. What is unaudited is **what my harness actually compared**, because
that failure mode would silently corrupt every future bisect in this repo.

**The leading hypothesis, which you should try to confirm or kill:** I ran old revisions of the
guards against contamination from the present-day tree — `client/dist/`, `node_modules`, a running
server, or an archive that was not clean — so every revision failed for a present-day reason and
"first bad" was simply the first one I tried. A harness that fails-before-judging at *every*
revision reads as "broken here" at whichever commit you happen to start from.

**The strongest single lead is in the lane's own report:** it had to `git init` the frozen archive,
because the historical `bdinput` executes `git show HEAD:watch.py` and an archive has no `.git`. It
reported that a source-pinning guard needs this or it fails **before judging**. That is exactly the
did-not-judge sentinel that a careless bisect would score as a FAIL.

## Deliverable

Either a bisect harness that is trustworthy, or the written recipe for one — I do not mind which,
and a recipe with a worked example may be worth more than a tool nobody invokes. What it must do:

- Distinguish **did-not-judge** from **judged-and-failed** at every revision. A bisect that cannot
  tell these apart is not measuring what it claims, and this is the whole task in one line.
- State its preconditions and verify them per revision (the `.git` case above is one; find others).
- Refuse rather than guess when a revision cannot be judged.

## Red-proof

- **Direction 1:** construct a revision range where the true first-bad is KNOWN — you have a real
  one to hand: `947531af`/`2730557f` for qroll, where the correct verdict is *both pass*. Show your
  method returns that, and show it reds discriminatingly when you break it.
- **Direction 2, and this is the one that matters:** construct the contamination case — a revision
  that fails for a present-day reason rather than its own — and show your method reports
  DID NOT JUDGE instead of FAIL. If it cannot detect some class of contamination, name that class
  plainly rather than implying the harness is sound.

## You own the guard ports 39890-39899 this tick

You are the only lane authorised to bind them; the fleet is otherwise pytest-only. **Report the
`guard_preflight` verdict with every guard result.** My standing rule, so you know how your output
will be read: I accept a GREEN under load but not a RED — load manufactures false reds, not false
greens — so a red under CAUTION-or-worse gets re-run before I believe it. The #781 lane discounted
its own runs in the wrong-answer regime and said so; do the same.

## Lessons selected for this task

- **A check that examined nothing must not read as passing** (`#671`) — did-not-judge scored as
  FAIL is that defect driving a bisect.
- **Report what you cannot classify** (`#702`) — refuse rather than pick a first-bad.
- **A guard under load returns a WRONG answer, not a slow one** (`#606`/`#471`/`#666`) — a bisect
  run across a loaded machine inherits this at every step.
- **A name must not imply more than it proves** (`#651`) — "first bad" is a strong claim.
