# BRIEF — #810: master is RED, and the registry that calls itself "the one home" has never heard of the client running every lane in this fleet

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-810registry
Branch: cx-810registry
Base sha: 7f978b35
...
Lane-owns: `client_env.py`, `dev/mcp_screenshot_root.py`, `test_client_env.py`,
`test_mcp_screenshot_root.py`.

**Task:** `python3 dev/ledger.py get 810`. The note is the specification.

## This is a live red on master and it is mine, not yours

```
FAILED test_client_env.py::TestTheRegistryIsTheOneHome::test_only_client_env_names_the_registry_variables
E  client env variable names belong only in client_env.CLIENTS;
   found: mcp_screenshot_root.py:114 names 'CLAUDE_CODE_SESSION_ID'
```

I merged four lanes as a batch and gated once, so `#800`'s work reached master with this in it. **The
guard is right and must not be weakened** — read its docstring: the thing it exists to stop is
*imitation*, a second reader of a registry variable that works today and diverges the day the registry
gains a client or renames a variable. That is precisely what happened.

## The shallow fix is a trap — read this before you write anything

`dev/mcp_screenshot_root.py:112-118`:

```python
def _session_id(environ):
    ids = {value for key in ("CLAUDE_CODE_SESSION_ID", "CODEX_COMPANION_SESSION_ID")
           if (value := environ.get(key))}
```

The obvious fix is to derive that key set from `client_env.CLIENTS`. **Do that alone and you break
the fleet.** I measured the registry:

```
client-env CLIENTS: exactly ONE entry
  claude-code   detect=('CLAUDECODE',)  session_id_var='CLAUDE_CODE_SESSION_ID'  subagent=None

grep CODEX_COMPANION client_env.py  →  NOT IN REGISTRY
```

So `CODEX_COMPANION_SESSION_ID` exists nowhere in the registry. Derive naively and the key set shrinks
to one name, and `server_cwd()` stops identifying the shared MCP server **for every codex lane** —
which is every lane in this fleet, all eight of them, right now. You would turn a guard failure into a
behaviour failure and the tests would go green while the tool got worse. **That is the exact shape of
the vacuous fix this loop has filed three defects about today.**

## So the real deliverable is the registry entry

`#800`'s lane *knew* about a harness client the registry does not, and expressed that knowledge by
hardcoding it in a consumer. The fix is to put the knowledge where the registry says it belongs.

**Two halves, and both are needed:**

1. **Register the codex client** in `client_env.CLIENTS` — so the name has a home.
2. **Make `_session_id` derive from the registry** — so there is one reader.

**Before you add an entry, read `client_env.py` around lines 160-195.** It documents a deliberate
convention: `session_id_var=None` means *"measured: this client has no such var"* — an assertion, not
an omission. Adding an entry means making claims about `detect`, `session_id_var` and `subagent_var`,
and **each claim should be something you measured, not something you assumed.** You are running inside
a codex lane; `/proc/self/environ` and the `ccc` process environment are available to you, so measure
what a codex runner actually exports rather than guessing. If you cannot establish one of the fields,
say so and use the convention the file documents for "unknown" rather than inventing a value.

**Do not call the single-entry registry a defect without checking whether it is a decision** — grep
the module docstring and `git log` the file first. I filed `#763` instance fifteen against myself
today for exactly that mistake.

## Red-proof

- **Direction 1:** break the registry-derived lookup and watch it red on the **discriminating**
  message — the failure must name *which* client's variable went missing, not merely that a session id
  was not found.
- **Direction 2, mandatory and specific:** construct the case where the fix **looks** applied but is
  not. The shape that matters here: **an environment carrying ONLY `CLAUDE_CODE_SESSION_ID` cannot
  tell a registry-derived lookup from the old hardcoded one** — both find it. Your fixture must
  include a codex-only environment, or the test passes for the wrong reason and the fleet breaks
  anyway. Show the one-client fixture failing to catch a sabotage, then show yours catching it.
- **The red-proof contract changed twice this hour** (`#795`, `#800`): you must **name the production
  seam you break — path plus symbol or branch** — and editing a test's expected value is not a
...
## Prove the tool still works, not just that the guard stops complaining

`python3 dev/mcp_screenshot_root.py` currently resolves the live shared server correctly — I verified
it by hand before merging:

```
server output root : /home/xertrov/.llm-general/skills/ud-dreamwork/.playwright-mcp
  source           : server PID 1328852 cwd /home/xertrov/.llm-general/skills/ud-dreamwork
```

**Run it from your worktree after your change and quote what you get.** It must still name a server
PID and its cwd. If it starts reporting `UNKNOWN`, you have shipped the trap above — and `UNKNOWN` is
the honest answer `#800` deliberately built, so it will not look like a failure. That is what makes it
dangerous.
...
- **`#440`** — one supported way. A registry with a second reader is two ways wearing one name.
- **`#651`** — a guard whose message names a failure mode it cannot detect; and its sibling, a
  registry that claims to be the one home while a consumer holds knowledge it lacks.
- **`#671`** — a check that examined nothing must not read as passing. A one-client fixture is that.
- **`#763`** — read the owning docstring before calling a decision a defect. Binds directly to the
  `session_id_var=None` convention.
- **`#611`** — a check whose absence reads as a pass. (Verify: this line is `#611`'s, not `#671`'s.)

## Run these, explicitly

```
just pytest test_client_env.py test_mcp_screenshot_root.py
just pytest $(python3 dev/repo_wide_guards.py list)
python3 dev/mcp_screenshot_root.py
```

If your change reaches another consumer of `client_env`, **find it by the call graph, not by the file
list I gave you** — `#763` instance six.

## Bars

**Master is RED and this task is the reason.** At `7f978b35` a full `just pytest` reads
**2 failed, 2816 passed**:

1. `test_client_env.py::...::test_only_client_env_names_the_registry_variables` — **yours**. Removing
   it is the deliverable.
2. `test_dreamhub.py::TestProbeAll::test_one_slow_project_does_not_delay_the_others` — **NOT yours,
   and not a real failure.** It is a timing test; it passes quiet in 1.19s. Load was 39 and it belongs
   to `codebase-memory-mcp`, not the fleet. Do not fix it, do not count it against yourself, and do
   not let it mask a real red of your own — **quote your failure list explicitly** rather than saying
   "same as baseline".

- Main-checkout lint reads **1**; the remaining WARN is the `lessons.md` near-duplicate, which is
  **human-gated and must stay**. Your worktree reads **5**. `python3 lint.py | tail -1` — the count
