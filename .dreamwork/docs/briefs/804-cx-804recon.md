# BRIEF — #804: one design of record, and a correction worth rescuing from the rival

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-804recon
Branch: cx-804recon
Base sha: b16898e0
...
Lane-owns: `.dreamwork/docs/plans/main-agent-recap.md`. **Do NOT create
`.dreamwork/docs/plans/recap-design.md`** — that file existing on master is the failure mode.

**Task:** `python3 dev/ledger.py get 804` then `get 691`. Read #691's notes **to the end** — the
answer to "why is this open" is in there and it is not "the design is missing".

## My error, stated plainly, because you need it to judge what carries over

I dispatched a lane to *design* the cheap-model recap. **The design had already landed** as
`.dreamwork/docs/plans/main-agent-recap.md`, 670 lines, and #691's own entry says so in capitals:
*"THE DESIGN LANDED; THE TASK STAYS OPEN BECAUSE IT AWAITS HIM."* I read the header and Max's
verbatim request and stopped before the note. That is #763 instance sixteen.

The lane produced a 463-line rival at `recap-design.md` on branch `cx-691recap`. **It is not being
merged.** Two design documents for one feature is precisely what `DREAMWORK.md:14-23` refuses.

## But the rival contains a real correction, and discarding it would waste it

Master moved **20 commits** under that lane. Critically, **#645 landed
`dreamwork_db.store.dreamwork_store_spec`** while it worked — one canonical store composer replacing
two rival `StoreSpec` builders. `main-agent-recap.md`'s **"Where in the DB"** section (around `## 6`)
**predates that** and describes a seam that no longer exists in that form. The rival lane reindexed
after its rebase, re-read the new code, and names the exact current seam.

**Deliverable:** fold that correction into `main-agent-recap.md`. Keep it as the single design of
record. `cx-691recap` stays unmerged and is cited for provenance.

**Read BOTH documents before deciding what carries over.** I am naming the storage seam because it is
the one I verified — not because it is necessarily the only stale part. `git show cx-691recap:.dreamwork/docs/plans/recap-design.md`
is the rival. If it treats some other section materially better, **say so and argue it** rather than
mechanically taking only the seam; and if you find the rival is wrong somewhere the original is
right, say that too. I would rather you correct my instruction than follow it.

## Two things you must NOT do

1. **Do not close #691, and do not write anything implying it is settled.** It awaits *him*: Q1 is
   his to answer and the design gates implementation on his review. This task changes a document, not
   a task state.
2. **Do not start implementing the recap.** The feature is gated on his review. `#691`'s entry
   records that as an explicit instruction from him, not a convention.

## Red-proof (doc lane form)

- **Direction 1:** every claim you carry over must be checkable against the code as it is **now**.
  For the storage seam specifically: open `dreamwork_db/store.py` and confirm the composer's name and
  shape yourself. If the rival's description is already stale (master has kept moving), fix it to
  what you measure and say the rival was stale too.
- **Direction 2:** show that a section you did **not** change is still accurate, rather than assuming
  the rest of a 670-line document is fine because you only came for one part. Name any other section
  you suspect is stale but did not verify — an honest "I checked sections 6 and 7, not 1–5" is worth
  far more than an implied whole-document review (`#671`, `#702`).
- Run `python3 lint.py | tail -1` before and after and quote both; a plan doc's citations are
  governed by `check_citation_range` and a fold can easily break one.
## Standing bars

**Master is RED on ONE known test and it is not yours:**
`test_lint.py::TestDocMapPlans::test_this_repo_maps_its_own_plans`, owned by `cx-794lintrow`
(a plan landed without its doc-map row). Do not fix it, do not count it against yourself, and
**quote your own failure list explicitly** rather than saying "same as baseline" — a real red of
your own must not hide behind mine.

- Read the lint bar with `python3 lint.py | tail -1`: the count is in the **trailer** and the rows
  are **indented**, so `grep -c '^WARN'` returns a false **0**. But compare the **ROW SET**, not the
  number — a lane measured today that the count can fall while a new warning appears (#794).
  Main checkout reads 2; a worktree reads ~6 (the gitignored ledger store cannot travel — #611/#685).
- Count tests by **collecting**, not by grepping `def test_`.
- **Cite lessons by CONTENT, not by an id I give you.** Five separate lanes today found ids in my
  briefs whose entries do not carry the principle I attributed (#755, #136, #702, #440, and
  #671-vs-#611). Verify before relying, and **report the mismatch rather than reconciling to me**.
- Run `python3 dev/redproof.py` for snapshot/restore; `python3 dev/lessons_index.py --act red-proof`
  before any injection. **Never `git checkout` to restore an injection** (#349); snapshot the FIXED
  file, not the pre-fix one (#608). NOTE: `cx-795proof` is changing `redproof.py` this tick — use
  your worktree's copy and do not edit it.
- A red-proof that edits a TEST EXPECTATION proves nothing (#795, found today). Inject into the
  **production seam** and make the test fail on the **discriminating** message.
- Limit builds and tests to 2 threads. Do not bind `:35110` (deployed dashboard, live) or `:35113`.
