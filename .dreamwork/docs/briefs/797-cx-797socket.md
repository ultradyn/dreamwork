# BRIEF — #797: a socket stdout passes the check and the runner still dies of SIGPIPE

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-797socket
Branch: cx-797socket
Base sha: 7ecb732a
...
Lane-owns: `dev/dispatch_lane.py`, `test_dispatch_lane.py`

**Task:** `python3 dev/ledger.py get 797 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`

## HANDLE THIS FILE WITH EXTREME CARE — it is the loop's only dispatch route

`dev/dispatch_lane.py` ends in `os.execvp`. **Twelve lanes are launching through it today.** A change
that refuses wrongly does not degrade a feature — it stops every lane launching. That asymmetry
governs every decision below.

## What was measured (re-derive it; do not inherit it)

I merged #701 an hour ago, having verified three healthy shapes launch (regular-file redirect — what
the whole fleet uses — plus tty and bare). I asked an independent reviewer to find a **fourth** shape
I had not tested. It found one that is **not healthy and passes anyway**:

- **State:** stdout is one end of a Unix `socketpair`; the reader consumes three lines and closes;
  the exec'd runner uses default SIGPIPE handling.
- **Result:** `validate_stdout()` **accepts** it — it rejects only `S_ISFIFO`. The runner starts,
  writes three lines, and exits **-13**, with no stderr.
  Measured: `stdout_type=socket lines=['0','1','2'] returncode=-13 runner_started=True`.
- **Control:** `/dev/null` (a character device) launches normally, `returncode=0`.

That is the identical silent mid-launch death #701 claims to have made unavailable, through a shape
the check does not classify.

## The trap, which is the whole task

The tempting fix — refuse anything that is not a regular file or a chardev — **would refuse shapes
that work today** and is exactly the failure #755's principle names (now homed in `lessons.md` via
#786; cite it by content). #707 is the same warning from the other side: every widening multiplies
false attribution.

So: what property actually distinguishes "a reader that can close early and kill me" from "a
destination that cannot"? Is it the file type at all, or is it something about the peer? **Judge the
candidate forms with IGC** — worktree-local `./igc-method.md`. An argued *"this cannot be classified
from the fd alone; here is what the caller must declare instead"* is a legitimate answer —
`DREAMWORK_ALLOW_PIPED_STDOUT=1` already establishes that a declaration seam exists (#440: keep ONE).

## Red-proof

- **Direction 1:** reproduce the socket death, then show your change refuses it **naming that
  specific mechanism** — not a generic validation failure.
- **Direction 2, MANDATORY and more important than direction 1:** prove every healthy shape still
  launches — **regular-file redirect (the shape the whole fleet uses right now), a tty, bare, and
  `/dev/null`**. A refusal that fires on any of those is far worse than the bug you are fixing.
