# BRIEF — #782 follow-up: the widening caught one thing too many, and my brief hid the failure from you

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/glm-782b
Branch: glm-782b
Base sha: d51c6b5a
...
Lane-owns: dreamwork_db/core.py, dreamwork_db/__init__.py, test_dreamwork_db_core.py,
test_ledger_write.py, test_dreamwork_db_migrate.py

**Task:** `python3 dev/ledger.py get 782 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` —
read the entry and especially the newest note, which records this finding.

## Your branch already has the work. This continues it; it does not restart it.

`5a39a121` (was `51f3cea8` before I rebased it onto current master) is on this branch and it is **good** — it widened the classifier in lockstep with the
catch (new `Corrupt` on code 26), kept `Busy` first, added the `_connect` and READ-`BEGIN` seams,
and took #779's `NoReturn` note. Keep all of it. **Do not revert and start over.**

## First: a failure my brief prevented you from seeing

My "Run these, explicitly" list named `test_dreamwork_db_core.py`, `test_dreamwork_db_migrate.py`
and `test_dispatch_lane.py`. It **omitted `test_ledger_write.py`**, which is a caller of the seam you
changed. So your bar was green and my merge gate is red. That is my defect, not yours — a
verification list derived from the files you would edit rather than from the call graph of what you
were changing. It is recorded as instance six of #763.

**Measured at your HEAD:**

```
just pytest test_ledger_write.py test_dreamwork_db_migrate.py test_dreamwork_db_core.py
-> 1 failed, 61 passed
FAILED test_ledger_write.py::test_file_is_one_transaction_task_row_rolled_back_with_its_event
   dreamwork_db/core.py:283: DatabaseError   (the test expects sqlite3.IntegrityError)
```

Re-run it yourself and confirm before acting — do not take my number.

## The finding, which is a design question and not just a red test

Your out-of-scope note said `sqlite3.IntegrityError` *"is now caught but left honestly unclassified —
no current caller distinguishes it."* The first half is right and the second is not quite: no
**production** caller catches it (I grepped; the two `user_events/sqlite.py` hits are comments), but
**six test sites do** — `test_ledger_write.py:375` and `test_dreamwork_db_migrate.py:248/253/259/270/276`
— and they encode a contract.

**The deeper problem is the message, not the test.** `Busy`, `Corrupt` and `SchemaMismatch` are all
*store* conditions: the store cannot serve the request. `IntegrityError` is the **opposite** — the
store working perfectly and refusing caller data that violated a constraint. sqlite classified it
precisely; the ladder now catches it and announces `unclassified store error during SQL`. That
statement is false, and it is false in the ladder's own terms: this is #651 (the name implies less
than the original proved) and #702 (declaring unclassifiable a thing that *is* classified).

## What to land

Two honest routes. **Weigh them and say which you took and why:**

1. **Exclude `IntegrityError` from the catch** and let it propagate as sqlite raised it. Simple, and
   defensible on the ground that a constraint violation is a caller-data fault rather than a store
   fault, so the ladder's promise ("callers never see raw sqlite for store conditions") is not
   broken by letting it through.
2. **Give it its own named class** — e.g. `ConstraintViolation(DatabaseError)` — carrying the
   original. Truer to this ladder's stated rule of naming what it can prove, and it keeps the
   single-door property (#440): callers catch ladder types, not sqlite types.

I lean to (2), but the argument matters more than my lean, and (1) may be right if a named class
would tempt callers to catch what they should be validating before the write (#681 built exactly
that validation into `ledger_write` for this reason). **Judge it with IGC** — use the
**worktree-local `./igc-method.md`**, not any copy under `~/.claude-p`; the standing contract is
explicit about that and my earlier briefs got it wrong.

**What you must NOT do:** update the six tests to expect `DatabaseError`. That ratifies the category
error instead of fixing it, and it converts a real signal into a weaker one everywhere at once.

If route (2) changes what those six sites should expect, changing them is legitimate — but then each
change must be *because the new class is the right expectation*, not because the test was in the way.

## Red-proof

- **Direction 1:** a constraint violation must produce your chosen outcome, and breaking that branch
  must red on the specific case — the discriminating failure is the wrong **class**, not merely that
  an exception occurred.
- **Direction 2:** `Corrupt`, `Busy` and `SchemaMismatch` must all still behave exactly as
  `51f3cea8` left them. Re-run the direction-2 proof you already did: a **locked** store still raises
  `Busy` and still reaches `dispatch_lane`'s `DID NOT RUN; launch allowed` path. That path runs
  immediately before `os.execvp` on the loop's only dispatch route, and five lanes are live right now
  (#755).
- **Direction 2b:** show that an `OperationalError` which is neither busy nor schema-shaped still
  becomes the honest unclassified `DatabaseError` and is not swept into your new class.
...
- **#651 — a name must not imply more than it proves.** Here inverted: the name must not imply
  *less* either. "Unclassified" about a precisely-classified error is the same defect mirrored.
- **#702 — report what you cannot classify.** And do not report as unclassifiable what you can.
- **#707 — every widening multiplies false attribution.** The widening is the whole subject; this
  is the one case where it went a step past what it could name.
- **#755 — a check that fires on a healthy input is worse than no check.** `Busy` must keep
  launching lanes.
- **#440 — one supported way.** Whichever route you take, there must be exactly one way a caller
  learns a constraint was violated.

## Run these, explicitly — note what my last brief left out

```
just pytest test_dreamwork_db_core.py test_dreamwork_db_migrate.py test_ledger_write.py test_dispatch_lane.py
just pytest $(python3 dev/repo_wide_guards.py list)
```

`test_ledger_write.py` is the addition. If your change reaches any other caller of the ladder,
**find it by the call graph rather than by the file list I gave you** — that is the failure this
brief opened with, and I would rather you correct my list than inherit it.

## Bars

**Measured 2026-08-01 at your HEAD before the rebase:** the three-file run above is **1 failed,
61 passed** — that failure is your starting state and removing it is the deliverable. Main-checkout
lint reads **2** (`python3 lint.py | tail -1` — the count is in the trailer; the rows are indented,
so `grep '^WARN'` returns a false 0). A worktree reads **6**; the delta of 4 is the gitignored ledger
store that cannot travel (#611, #685). Full tree at `c109ed8e`: **2717** collected; master has since moved to `d51c6b5a` (#763 merged), so re-measure.

- Count tests by collecting, not by grepping `def test_`.
- **Four other lanes are live.** `cx-783cover` owns `dev/capture/*` and the guard ports — do not run
  browser guards. `glm-780registry` owns `dev/repo_wide_guards.py`. #763 has MERGED (`d51c6b5a`) and dispatch now validates the base sha against the branch point — that is why your worktree and branch changed name.
  `glm-694scratch` owns `dev/lane_scratch.py` and `dev/redproof.py` — you will be USING those tools
  while it changes them; use your worktree's copies and do not edit them.
