# BRIEF — #775: the fleet reads 0 live because the matcher names a binary that has already exec'd away

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/glm-775live
Branch: glm-775live
Base sha: e699d4f6
...
**Task:** `python3 dev/ledger.py get 775 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — the entry carries the full measurement. Reproduce it; do not take it on trust.

## The measurement, already taken

With two lanes genuinely running, `just status-sync` printed `already in sync (163 open, 0 live)` and pruned `dreamers` to `[]`. `status_sync.py:187-196` matches live lanes by looking for `ccc` in process argv — unanchored and "deliberately broad" after an earlier fix for `^ccc @`. It fails **both** ways:

- **False negative (the one that bit).** `ps -eo args | grep -c ccc` finds **zero** dispatcher processes. `dev/dispatch_lane.py` replaces itself with `ccc`, and `ccc` in turn execs the runner — so live argv is `codex-code-mode-host` or the grok harness, and the string `ccc` is gone before anything looks. The earlier widening made the pattern survive `ccc --yolo @glm52`, but the process it names does not survive the exec chain at all.
- **False positive (latent).** The argv lines on this host that *do* contain `ccc` are `containerd-shim-runc-v2 -namespace moby -id 1640fa687243...` — `ccc` occurring inside a container hex id. An unrelated Docker workload counts as a live lane. **#755: a check that fires on a healthy input is worse than no check.**

**The signal that works is already there:** each lane's **worktree path** appears in live argv (7 matches for `cx-774vocab`, 5 for `glm-262witness` at measurement time), and `status.json` already records which worktree each lane owns. The registry a correct check needs exists.

## The trap, stated so you do not walk into it

**Do not fix this by matching the runner binary names** (`codex-code-mode-host`, `grok`, …). That is the identical mistake one level down: a new runner is added, nothing matches, and the fleet silently reads zero again — which is exactly how this bug was born. Match on something **the dispatch route controls and `status.json` already records**. Say in your report which invariant you chose and what would have to change for it to break.

## Two things that must be true when you are done

1. **With lanes live, `dreamers` is populated and `current_task_ids` names them.** You are yourself a live lane, so your fix must detect *you* — that is a genuine end-to-end check and you should run it. But do not let it be your only evidence (#759: hold the SUBJECT fixed and vary the INTERPRETER — a self-detecting run varies both at once). Build fixtures where the process table is injected rather than observed, so the behaviour is provable without a live fleet.
2. **With no lanes live, it reports zero and the lint guard goes quiet.** `lint.py:2197` already warns when `lanes` is non-empty and `dreamers` empty. **Leave that guard alone — it is the backstop and it is currently doing its job** (it is why the main-checkout bar reads 2 warnings right now instead of 1). After your fix, a healthy fleet must return the bar to 1. A guard that still fires on a correct fleet is a worse outcome than the bug (#755).

`lint.py` is held by another lane (`cx-774vocab`). **Do not edit `lint.py`.** If you believe the guard needs changing, report that instead of doing it.

## Red-proof, both directions

- **Direction 1:** re-inject the `ccc`-substring matcher and watch your check red on a **discriminating** message — one that says the fleet read zero while lanes were recorded, not a bare count mismatch.
- **Direction 2 is the one that matters here, because this bug IS a false green.** Construct a process table where lanes are genuinely live but the detector cannot see them, and show what your check decides. Then construct the mirror: an unrelated process that merely contains your match token, and show it is not counted. A liveness check that cannot distinguish "no lanes are out" from "I cannot see lanes" is #136 at this seam, and that is precisely the state being fixed.
...
- **#136 — "nothing needs you" and "the channel is broken" must not render identically.** The whole bug.
- **#755 — a check that fires on a healthy input is worse than no check.** Both the container false-positive and the guard-after-fix condition.
- **#702 — report what you cannot classify.** A process you cannot attribute to a lane is reported, not silently dropped or silently counted.
- **#671 — a check that examined nothing must not read as passing.** `already in sync (… 0 live)` is exactly that sentence today.
- **#440 — one supported way.** There should be one definition of "a lane is live", not one in `status_sync` and another in the heartbeat tick line.

## Out of scope, but say what you learn

The #673 heartbeat tick line reports `0 ccc-live` from the same blind spot — two independent readouts of one broken signal. **Do not fix the tick line in this lane** (it is not in your owned files). If your fix produces something the tick line could consume, say so concretely in your report and I will file the follow-up.

## Bars

- Worktree lint bar is **5 warnings**, not 1 — and the main checkout currently reads **2**, not 1, for the reason above. Measure at your base and quote what you measured; do not trust any number in a note, including these (#756).
