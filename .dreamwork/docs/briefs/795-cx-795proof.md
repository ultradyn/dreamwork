# BRIEF — #795: a red-proof can be claimed by editing the test, and the tool cannot tell

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-795proof
Branch: cx-795proof
Base sha: a564bd80
...
Lane-owns: `dev/redproof.py`, `test_redproof.py`, and `briefs/boilerplate.md`'s red-proof
paragraph **only if** your answer is a contract change.

**Task:** `python3 dev/ledger.py get 795 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — the note is the specification.

## HANDLE WITH EXTREME CARE — every live lane is using this file right now

`dev/redproof.py` owns the snapshot/restore protocol that the standing contract points **every** lane
at, and **seven lanes are running against it as you read this.** A change that makes `begin`,
`restore` or `check` misbehave does not degrade a feature — it destroys other lanes' evidence, which
is the exact injury `#694` was filed for. Use your worktree's copy; do not touch theirs. If your
change alters the on-disk registry format, **handle the old layout or refuse loudly on it** — do not
silently start writing somewhere new while live lanes' snapshots sit unreachable.

## The finding, measured by an independent reviewer today

`cx-791revB` audited all six of this tick's merges by re-injecting into production itself rather than
grading the lanes' prose. Two submitted red-proofs did not bind:

- **#786** changed a test's expected value from `NO_RELATIONSHIP` to `UNCLASSIFIABLE` and reported
  the resulting failure as its red. Correct production code *necessarily* fails a sabotaged
  expectation. That proves an intentionally false assertion is false — nothing about whether the test
  reaches the production branch it names. (The reviewer's real injection confirmed the landed test
  *does* bind. The work was sound; the proof was not.)
- **#670**'s blanket `return False` red at `precondition failed: test is not running inside a git
  worktree` — a message that names a real environmental condition and a broken detector
  **identically**. Non-discriminating.

**Both passed `redproof.py check: clean`.** That sentence is load-bearing at my merge gate and it
says nothing about whether the injection was in *production* code.

**Re-derive this before building.** Reproduce both shapes yourself — snapshot a test file, edit an
assertion, watch it fail, restore, and confirm `check` reports clean. If it does not reproduce, that
is a finding and I want it more than I want a fix.

## The design question — not pre-decided, and a refusal is a valid answer

Can the tool refuse or flag an injection whose target is a test file? That is a cheap lexical signal
(`test_*.py`) with real limits:

- Some legitimate injections **do** target tests — a guard's own fixture, or `#791`'s own reviewer
  work. A check that refuses those fires on healthy input, and that principle now has a home in
  `lessons.md` (cite it by content; `#786` is why the id-form matters).
- `#707`: every widening multiplies false attribution.

Weigh at least: **refuse** / **warn** / **require the lane to name the production symbol it broke**
(making the claim checkable rather than the file) / **conclude the tool cannot judge this and the
CONTRACT must ask for the production seam by name**. **Judge with IGC** — worktree-local
`./igc-method.md`. An argued *"the tool cannot decide this; here is the contract change instead"* is
a fine and possibly correct outcome — do not build something weak to avoid returning empty-handed.

## Red-proof (recursive, and mind the irony)

- **Direction 1:** break whatever you land and watch it red on the **discriminating** message. If
  your answer is a contract change rather than code, direction 1 is: show that a proof following the
  OLD contract passes while proving nothing, and that the NEW contract rejects it — construct that
  pair concretely.
- **Direction 2, mandatory:** construct the case where your new check **passes a proof that still
  proves nothing.** It exists: inject into *production* code, in a branch the test never reaches.
  Show it, and say plainly that your mechanism does not close it (`#702`) rather than implying it does.
- **Never `git checkout` to restore an injection** (`#349`) — and note you are working *on* the tool
  that exists to make that unnecessary. Snapshot the FIXED file, not the pre-fix one (`#608`).
