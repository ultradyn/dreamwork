...
- **#417** · landed `5fe331a` · 2026-07-28 18:09 · by glm52 (wt/417) — proposal only, chart untouched: four commits-per-period treatments (faint histogram / sparkline rail / level-line weight / copy only) priced in one table with buys, costs, makes-harder-to-read, guard and motion columns; ten renders of the real panel against the live ledger at 1280x900 and 390x844, verified by the coordinator as ten distinct sha256s and cropped to a panel whose live width really is 553/358px; guard impact quantified per candidate (C2 grows the panel 177->202px and breaks the constant-height premise, C4 costs 19px with no new motion idiom); `watch.py` modified uncommitted for rendering and reverted, worktree clean at exit; `#ask` above the fold at both viewports via the shared checker; visual verdict owed because glm52 cannot see and grok is 401
