# BRIEF — #783: three guards that pass while the thing they name is broken

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-783cover
Branch: cx-783cover
Base sha: c109ed8e
...
Lane-owns: dev/capture/qroll.mjs, dev/capture/bdinput.mjs, dev/capture/selectkeep.mjs

**Task:** `python3 dev/ledger.py get 783 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`

## Where this came from, and why you are not starting from zero

`#781` (merged `c109ed8e`, one commit before your base) fixed these three guards. Its lane then
red-proved them in direction 2 — break the subject, confirm the guard reds — and **all three stayed
GREEN**. It reported that rather than describing them as covered. You are building the coverage those
three experiments proved missing.

**Each gap comes with the exact injection that exposed it.** Re-run each one to see the false-green
for yourself before you write anything — that is your direction-2 baseline, and it is already known
to reproduce:

1. **qroll** — inject `if (m.rolled) return` into the cross-tab storage listener. This genuinely
   breaks rolling an already-open *unrolled* peer while leaving unroll sync intact. qroll passes,
   because it opens tab 2 from persisted **rolled** state and only ever tests the unroll direction.
   **Missing: roll-sync into an existing open tab.**
2. **selectkeep** — disable the `data-qid` arm of `viewNodeKey`. selectkeep passes, `sameCard:true`
   included, because its fixture never reorders the selected card and morphdom happily reuses that
   position. The guard's class claim *"keyed by qid"* is therefore unbound by anything.
   **Missing: a tick that MOVES the selected card.**
3. **bdinput** — replace production `tick()`'s `setLiveContent(html)` with wholesale `innerHTML`.
   Real polling would destroy focus, value and caret on every tick; bdinput passes all 33
   assertions. **Missing: the production `tick()` caller itself** — see below, this one is different.

## bdinput is the hard one and it is not the same kind of gap

The other two are fixture gaps: the scenario is too narrow, so widen the scenario. bdinput's gap is
**structural**. Its two "forced ticks" reconstruct the snapshot → build → `setLiveContent` chain *by
hand* instead of invoking `tick()`. So the guard exercises a hand-built replica of the production
path and never binds the real one — which is why sabotaging real `tick()` leaves it green.

This is the deeper half of a finding #781 already landed the shallow half of. Both guards carried a
comment claiming they "use the production path"; that was **materially false**, and #781 fixed the
*fetch* half (they now go through `fetchDataResponse()` rather than hand-fetching `dataJsonUrl()`).
The *caller* half is yours.

The obvious move — make the forced tick call production `tick()` — may be blocked by whatever made
the guard hand-roll it in the first place (timing, the 8s `holdRerenderUntil`, needing to interleave
a snapshot). **Find out which, and say so.** If binding real `tick()` is genuinely impractical,
report that with the reason and land the narrower thing you *can* bind; do not leave the comment
claiming production-path fidelity that the guard does not have (#651).

## A fourth instance may already be there, introduced by the fix

#781 added `if (next) setData(next)` to both bdinput and selectkeep. That is correct as far as it
goes — an envelope is not a data document. But check what happens when `fetchDataResponse()` returns
falsy: as written, the guard **skips the update and proceeds to `buildCurrent()` on stale data**,
which could pass while nothing ticked at all. That is the same #671 shape one level down.

Verify before you treat it as a defect: it may be that a falsy return only happens when there is
genuinely nothing to apply, in which case proceeding is right. **Determine which, and if it can
silently swallow a real tick, make the guard fail rather than continue.** A guard that cannot tell
"applied an update" from "applied nothing" is the whole family this task is about.

## Scope discipline

Three named gaps. Close them. **Do not audit the other 83 guards for the same shape** — that is a
real and probably productive question, and it is a separate task, not a widening of this one (#707).
If you notice the pattern elsewhere while working, report the sighting; do not chase it.

Equally: **do not weaken anything to make room.** The assertion sets in these three files are
currently byte-identical to `947531af`, the last known-good revision — I verified that myself at the
merge gate by diffing the `ok(...)` text (21/33/7). If your work changes an existing assertion,
that is a claim you must argue, not a detail.

## Red-proof

- **Direction 1:** each new assertion must red on its own named message when you inject the defect
  it exists for. The three injections above are your starting set. A new check whose only failure
  mode is a crash repeats the #471 did-not-judge state these guards were in yesterday.
- **Direction 2:** re-run each injection above after your work and confirm the guard now **fails**
  where it previously passed. That transition — green-under-sabotage becoming red-under-sabotage —
  is the entire deliverable, so quote it per guard.
...
- **Never `git checkout` to restore an injection** (#349). Snapshot the FIXED file, not the pre-fix
  one (#608).
- #781's lane squashed its sabotage commits out of reachable history before rebasing, and
  `redproof.py check` confirmed none remained. Do the same and quote the check.

## Lessons selected for this task

- **#671 — a check that examined nothing must not read as passing.** Three sites at once; the task
  in one line.
- **#651 — a name must not imply more than it proves.** "keyed by qid" and "uses the production
  path" are both currently claims the guards do not earn.
- **#755 — a check that fires on a healthy input is worse than no check.** Your new assertions must
  pass on the healthy tree, and you must not buy coverage with flake.
- **#707 — every widening multiplies false attribution.** Three named gaps, not a guard-suite audit.
- **#136 — a signal that always fires is one nobody reads.** Its converse here: a guard that never
  fires is equally unread.

## Run these, explicitly

```
DREAMWORK_GUARDS="qroll bdinput selectkeep" DREAMWORK_HUB_GUARDS="" just guards
just pytest test_watch.py
just pytest $(python3 dev/repo_wide_guards.py list)
```

**You are the only lane authorised to bind guard ports (39890-39899) this tick** — three other lanes
are live and none of them touch `dev/capture/`. Do not bind `:35110` or `:35113`.

If the preflight reports CAUTION or worse under load, believe a RED less than you believe a GREEN:
a browser guard under load returns a WRONG answer, not a slow one (#606/#471/#666). Report the
preflight verdict alongside every guard result.
...
the main checkout reads **2**. Full tree at this base: **2717** collected. The three guards **PASS**;
that is your starting state and preserving it is a bar.
