# BRIEF — #645 increment 9 RE-LAND: your verbs must pass the gate sweep, not just appear in its map

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/glm-645i9b
Branch: glm-645i9b
Base sha: 91663687
...
Lane-owns: dreamwork_db/, test_dreamwork_db_*.py, dev/ledger.py, test_ledger_cli.py

**Task:** `python3 dev/ledger.py get 645 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry and its notes.

## What happened: increment 9 landed, gated RED, and was reverted

The work was good — five mutating question verbs refusing pre-watermark with exit 2 and naming the cutover command, `reviews register/link` deliberately allowed, both red-proof directions clean, 37 tests. It merged, then the full gate reded:

```
FAILED test_ledger_cli.py::test_the_map_covers_every_verb
  add to _VERB_ARGV with a minimal valid argv: ['questions-answer', 'questions-comment',
  'questions-fold', 'questions-post', 'questions-retitle', 'reviews-link', 'reviews-register']
```

**This was my fault, not the previous lane's.** The `Lane-owns:` line I wrote named `dreamwork_db/` and its tests. The CLI verbs necessarily went into `dev/ledger.py`, whose governing test is `test_ledger_cli.py` — both outside that line, so the lane never ran the test that governs the file it had to extend. The `Lane-owns:` above is corrected. Run `test_ledger_cli.py`.

## FIRST: recover the reverted work, and verify the recovery

The merge was reverted at **`5f1ea764`**. Branch `glm-645i9` still exists but **is now an ancestor of master**, so a plain merge or cherry-pick of it brings back **nothing**. This is a known trap.

```
git revert --no-edit 5f1ea764
git diff master --stat
```

**If that `git diff` prints nothing, STOP and report it** — it means the recovery did not happen and everything below would be built on an empty tree. It should show `dev/ledger.py`, `dreamwork_db/questions.py`, `dreamwork_db/reviews.py`, `test_dreamwork_db_cli.py` (~+1477 lines).

## THEN: the real work, which is not adding map rows

`_VERB_ARGV` (`test_ledger_cli.py:526`) is **not a checklist**. `test_every_verb_is_gated_not_just_get` (`:703`) **iterates it** and runs each verb against a real git worktree of a real post-cutover repo with a **real absent store**, asserting for every one: it refuses, it does not write, it uses the right stream, and it exits with the right rc (`unrefused`, `wrote`, `wrong_stream`, `wrong_rc`).

So adding your seven rows makes the sweep **exercise your verbs against the absent-store gate for the first time.** Expect that to be the actual work.

**The specific thing to check, from your predecessor's own log:** it found that *"the `source_of_truth` gate checks the TASK ledger cutover, not the questions cutover"* and changed the dispatch accordingly. **Verify your question and review verbs still hit the absent-store gate at the top of `_dispatch`.** If they were routed around it, they will no-op or write against a store that is not there — which is worse than either failure the gate exists to prevent, and it is the same "no second path" principle increment 9 is built on (#440). If you conclude a verb legitimately should not be gated, **argue it explicitly in your report**; do not encode that silently by omitting a row.

**Do not make the test pass by weakening it.** Do not delete rows, relax assertions, or add an exemption list. If a verb cannot pass the sweep, that is a finding about the verb.

## Also carry forward the finding your predecessor made

During development the guard was **silently inert**: `_QUESTION_MUTATING_CMDS` held short names (`q-post`) while argparse registers `questions-post`, so the set never matched and the watermark check was skipped on every verb. The refusal tests stayed green throughout — the verbs were refusing for the wrong reason. Only the positive-case test caught it.

Keep that lesson live in the re-land: **for each of the five mutating verbs, prove BOTH that it refuses pre-watermark AND that it succeeds post-watermark.** A refusal never observed to let anything through has not been shown to be a refusal (#671, #755).

## Red-proof

Both directions were discharged before and the injections are already encoded in the recovered tests — re-run them, do not redo them from scratch. What is **new** and needs its own direction-1 proof is the gate-sweep interaction: break one new verb's absent-store gating and show `test_every_verb_is_gated_not_just_get` reds naming that verb.
...
- **#671 — a check that examined nothing must not read as passing.** The inert guard, verbatim.
- **#755 — a check that fires on a healthy input is worse than no check.** Why the post-watermark success case is mandatory.
- **#440 — one supported way.** The absent-store gate and the pre-watermark refusal are the same principle at two levels.
- **#702 — report what you cannot classify.** Any verb whose gating you are unsure of.
- **#756 — never quote a stale bar.** Including the numbers below.

## Bars

**Measured in your worktree at dispatch, 2026-08-01, base `91663687`:** worktree lint **5 warnings**; `test_ledger_cli.py` collects **36**. Re-measure and quote what *you* got.

- **`just test` in FULL is your gate this time, not a targeted subset.** The previous attempt passed 107 targeted tests and still reded the repo. Limit to 2 threads.
- Count tests by collecting, not by grepping `def test_`.
- Machine load is ~22 from unrelated work — if something is flaky on timing, say so rather than retrying until green.
