...
Worktree:   /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-630p5
Branch:     cx-630p5
...
Lane-owns:  client/style.css and the token/styles files you add, client/dist/ (rebuilt), .dreamwork/lane-630p5-report.md

Your brief is at BRIEF.md in your worktree and reproduced below.
Read the ledger entry and its notes before planning.

# BRIEF — #630 P5 stage 1: tokens, `styles.css`, `conventions.md`

**Task:** `python3 dev/ledger.py get 630 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry and all its notes. P1, P2 and P3 are landed (`#653`, `d34b6bae`, `#751`); P4 is `#631`, in flight as its own increments. **P5 is the last unclaimed phase**, and it is the one that directly serves his stated 2026-07-31 focus: the UI as a built frontend, claude-design compatible.

**Design:** `.dreamwork/docs/plans/component-transition.md`. P5 reads:

> **P5 — the claude-design track (Q2, staged), parallel after P1.** Stage 1: tokens + `styles.css` + `conventions.md` (nearly free — the file is real today). Stage 2: wrapper exports for the starting set (§2a), `.d.ts` + `.prompt.md` + fixture props. Cheapest-early-signal rule (§6-R5): run ONE wrapper (`QaCard`) through the design tool end-to-end before authoring the rest.

**Land stage 1 only.** Stage 2 is a separate increment and §6-R5 says explicitly to prove one wrapper end-to-end before authoring the rest — do not start it.

## What exists today, measured

- `client/style.css` — **singular**, 1798 lines, the real stylesheet the served page uses.
- There is **no** `client/tokens*` and **no** `conventions.md`.

So "the file is real today" refers to `client/style.css`. Stage 1 derives tokens and a claude-design-shaped `styles.css` **from** it. **Re-measure this yourself** — it was taken at 07:15 on 2026-08-01.

## The constraint that governs the whole phase

**Extraction, never restatement.** `DREAMWORK.md:14-23`'s second-truth rule is scoped but alive: his `#614` answer relaxed *"one renderer, and it is the Python one"* and ruled the webui's state a legitimate second description — but the on-disk master state rule is **untouched and stays exactly as strict**. `#668` carries the ruling verbatim; read it rather than inferring its scope.

Applied here: a token file that **restates** values already in `client/style.css` creates two truths about one colour, and they agree only on the day they are written. The supported shapes are (a) tokens are the source and `style.css` consumes them, or (b) tokens are **derived** from `style.css` by a build step with a staleness guard. `#630` P1 already established the pattern for (b) — `client/dist` is guarded by a sha256 manifest that makes staleness a lint ERROR rather than something a reader must notice.

**Pick one and say why.** What you may not ship is a hand-maintained parallel list.

## The proof this phase already owes

`component-transition.md` names **"wrapper equality (P5, permanent — the check that beats one-time byte identity)"** and **"wrapper purity (P1/P5): no HTML tag literal in the wrapper-exports"**. Those bind stage 2, not stage 1 — but read them now, because stage 1's output shape determines whether stage 2 can satisfy them. If your token design makes wrapper purity harder, say so.

For stage 1 itself the proof is narrower and concrete: **the served page must not change.** P1 and P2 both held byte-identity and I verified both by hand on two ephemeral ports. If stage 1 intends no visual change, prove none — capture `GET /` before and after and compare bytes. If your chosen shape does change the page, say so explicitly and justify it; do not let it happen quietly.

## Direction 2

A byte-comparison of the served page **passes vacuously** if both sides are empty, or if the asset loader silently fell back. Assert the check's own preconditions: that the stylesheet loaded, is non-empty, and that the page exceeds a floor **derived at runtime** from the real asset sizes — not a literal tuned to today's tree.

And the sharper one for this task: construct a token extraction that is **wrong but consistent** — a value renamed in both places, so the two agree with each other and disagree with the original. Show what your staleness guard decides. A guard that only compares tokens against `style.css` cannot see this; say so if it cannot.

## Constraints

- **`#440`: one supported way** to know a colour.
- **`#755`, `#671`, `#136`, `#702`** as usual.
- Client assets are built: `client/*.js` and `client/style.css` feed `just build-client`, which emits `client/dist/` under a sha256 manifest guard (`lint.py` reports `OK client/dist … 14 inputs and 3 outputs`). **Change a client source and you must rebuild, or lint goes red by design.**
- Do NOT convert a surface, mount a component, or touch `watch.py`.
- Do NOT rewrite historical briefs or lane reports.

## Verification

- `python3 lint.py` from the worktree — **the lane bar is 5 warnings.** I measured this at `bc7aab6b` today; it was 6 until `#767` retired a false one this morning. Do not compare against the main checkout's number (`#611`/`#667`). The `client/dist` line must stay OK.
...
- **Browser guards apply to this one** — it is client-side. If a `dev/capture/` guard covers the styling you touch, run it and name it; if none does, say that rather than implying coverage. Guards bind ephemeral ports; **nothing near `:35110` or `:35113`**, which are the deployed and dev dashboards and are not yours.
...
`.dreamwork/lane-630p5-report.md`, with a DOGFOOD REPORT section. Lead with which of the two token shapes you chose and the argument against the other, then whether the served page changed.
