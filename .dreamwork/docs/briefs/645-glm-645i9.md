# BRIEF — #645 increment 9: the CLI unit, whose defining behaviour is a REFUSAL

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/glm-645i9
Branch: glm-645i9
Base sha: 2f57cd8e
...
Lane-owns: dreamwork_db/, test_dreamwork_db_*.py, test_question_parse.py

**Task:** `python3 dev/ledger.py get 645 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry and **all** its notes, including the two newest, which retract a premise increment 8 was briefed on. Then `get 774`.

**Design:** `.dreamwork/docs/cx-645-db-api-design.md`, **§12 increment 9**. That spec is authoritative; this brief adds only what it does not say.

> **9. CLI unit.** Add `questions post/...` and `reviews register/link/...` over repositories. Before the watermark mutating question verbs refuse with the cutover command, so no second writer path exists.

Increments 1–8 are landed. Increment 10 is the dark read unit; **it is not yours.**

## Read this before you use any number from increment 7

Increment 7 reported *"3/67 answered entries missing a resolution date (#572, #614, #613)"*. **That number was wrong and it is now fixed.** Those three entries were never dateless — they carried a dated `Answer`/`Comment (via watch, …)` marker that `lint.py` has accepted since `#767`, while `question_parse.py` read only the retired `→` arrow head. `#774` unified the vocabulary and re-measured against the live source: **67/67 resolved, 0/67 missing.**

Two consequences you must not get wrong:

1. **Do not carry `3/67` into anything.** If a CLI count or fixture depends on the number of unresolved answered entries, the answer is **zero**, and a fixture that needs a genuinely unresolved entry must construct one rather than name a real id.
2. **The API changed.** Entries now carry `resolution_marker` (with a `ResolutionKind`), not a bare `resolution_date`. Reading `resolution_date` on a `FUTURE_FORMAT` marker **raises `UnclassifiedResolutionMarker` naming the marker** — deliberately, so a parser can never answer "no resolution" when it means "I did not recognise this" (#702). Your CLI must surface that distinction to the reader rather than collapsing it to a blank column.

I am telling you this because I briefed increment 8 on the wrong version and it built fixtures around three real ids that did not mean what I said. **Check what has landed since a number was recorded before you rely on it.**

## The refusal is the increment, not a side condition

The spec's real content is the second sentence: *"Before the watermark mutating question verbs refuse with the cutover command, so no second writer path exists."*

The watermark is `questions_cut_over` in `meta` (design §"Cutover protocol" step 4), and the governing sentence is §446: **"There is no steady-state dual read or dual write. The watermark is the single dispatch just as it is for the ledger."** So a `questions post` that quietly writes the DB before cutover would create exactly the second writer path this design exists to prevent — and it would look like a working feature.

Therefore:

- A mutating question verb invoked **pre-watermark** must refuse, and its refusal must **name the cutover command** so the reader knows what to run. A bare "not supported" is a worse outcome than the bug.
- **Read verbs and the `reviews` verbs are not automatically in the same bucket** — the spec constrains *mutating question* verbs. Decide each verb explicitly, state your classification in the report, and say which side of the line each one landed on and why. A verb you cannot classify is reported, not guessed (#702).

## Red-proof, both directions — and direction 2 is the whole risk here

- **Direction 1:** remove the watermark check from one mutating verb and watch a test red on a **discriminating** message — one that says a pre-watermark mutation was accepted, not a bare exit-code mismatch.
- **Direction 2 is where this increment can fool you.** A refusal test passes trivially if the verb is broken, unreachable, misspelled, or if the watermark is *never* set in any fixture — the verb refuses because nothing works, not because the guard fired. So you must also prove the **positive** case: with the watermark set, the same verb SUCCEEDS. A guard that has never been observed to let anything through has not been shown to be a guard (#671, and #755 — a check that fires on a healthy input is worse than no check).
...
## Still dark, and still scratch-only

Increment 9 adds a CLI surface; it does **not** cut over. Nothing you write may touch the live `.dreamwork/questions.md`, the live ledger, or the live `.dreamwork/status.json`. Operate on scratch stores. Reading the live `questions.md` **read-only** to report a census is fine and is how increments 7 and 8 worked.

## Lessons selected for this task

- **#440 — one supported way.** The entire reason the pre-watermark refusal exists.
- **#671 — a check that examined nothing must not read as passing.** A refusal that fires because the verb is broken proves nothing.
- **#755 — a check that fires on a healthy input is worse than no check.** Post-watermark, the verb must work.
- **#702 — report what you cannot classify.** Both unclassified verbs and `FUTURE_FORMAT` markers.
- **#651 — a name must not imply more than it proves.** Increment 7 proved byte-span losslessness and called itself lossless while a derived field was wrong underneath it; that is how #774 happened.

## Bars

**Measured in YOUR worktree at dispatch time, 2026-08-01, at base `2f57cd8e`** — not quoted from memory, because #776 was filed today for briefs that did exactly that:

| measure | value |
|---|---|
| worktree lint | **5 warnings** |
| main-checkout lint | 2 warnings (the extra is #775's `lanes`/`dreamers` guard, expected while lanes are out) |
| `test_question_parse.py` + `test_dreamwork_db_import.py` collected | **69** |

Re-measure anyway and quote what *you* got. If your numbers disagree with this table, your numbers win and the disagreement is itself worth reporting (#756).
- Count tests by collecting, not by grepping `def test_`.
- `just test` is the gate. Limit to 2 threads.
- `test_no_raw_connect.py` must stay green — `dreamwork_db.core` is the one connection route.
