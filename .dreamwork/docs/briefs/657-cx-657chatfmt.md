# BRIEF — #657: he asked for this again ten minutes ago — chat list formatting, and a count that was briefly wrong

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-657chatfmt
Branch: cx-657chatfmt
Base sha: a7a88df9
...
Lane-owns: the **chat list row rendering and `watch.list_chats`**, plus `bin/ud-dw-chat`'s reply
writer if the third defect turns out to live there. **`cx-281tasks` and `cx-738draw` are both live in
`client/` right now** on different surfaces — keep your diff tight to the chat list.

**Task:** `python3 dev/ledger.py get 657 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`
— read the entry and its note in full. **Note the `--ledger` form**; a bare `get` refuses in a worktree.

## He re-asked for this ten minutes ago, by name

2026-08-01 14:59, typed into the dashboard: *"we have a task about topic chats formatting on
dashboard, it needs to be investigated and fixed. find the task and delegate to subagent."*

This is that task. **It is a re-ask, which in this loop is the signal that something has been sitting.**
His original report is `#657`, filed 2026-07-31.

## Three defects. They are probably not one bug, and the entry says which is which.

### Bug 1 — rows run together with no break at all

His paste, verbatim: `… 2 turnsreplied · First reply, and the round trip works …`

**Read that carefully: `2 turnsreplied`.** The boundary is *missing entirely*, not merely
under-spaced. That distinction matters for the diagnosis — a margin/padding problem produces cramped
rows, not concatenated text. Look for rows emitted without a block-level container, or joined without
a separator.

### Bug 2 — the count was 2, then became 1

His parenthetical is the diagnosis: *"it showed 2 turns for the pending chat, even though I'd just
submitted it. (later it changed to 1)"*

Turn counts are **derived at read time, never stored** — `chat.json` holds identity only (id, mode,
`created_from_receipt`, created). So a fresh chat with one human turn briefly counted **2** and then
settled to **1**. Something transient was counted as a turn.

**Establish this by OBSERVATION, not by reading alone: submit a fresh chat and watch the count.** I
have a hypothesis in the entry — an in-flight/pending item counted alongside the persisted turn, or
`dw-turn` markers parsed such that a partially-written transcript satisfies the pattern twice — and
**it is a hypothesis, not a finding.** Check it and say whether it held. A fix aimed at my guess
rather than at your measurement is the failure this loop keeps paying for.

### Bug 3 — paragraph breaks collapsed (observed separately, may share a root with bug 1)

An agent turn written through `bin/ud-dw-chat reply` had its **paragraph breaks collapsed**: a
four-paragraph reply rendered as one continuous block.

**The decisive question, and the two answers need different fixes:** does the transcript *store* the
newlines and the renderer drop them, or does the **writer strip them on the way in**? **Only one of
those loses data** — and if it is the writer, existing transcripts are already damaged and you should
say so rather than only fixing forward. Check the bytes on disk before you touch either side.

## Red-proof

- **Direction 1:** break each fix and watch it red on the **discriminating** message — for bug 2 the
  failure must name *what was counted that is not a turn*, not merely that a number differed.
- **Direction 2, mandatory, and bug 2 hands you the exact shape:** the count is **transiently** wrong
  and settles correct. **A test that reads the count after everything has settled cannot see this bug
  at all** — it passes against the broken code. Your test must observe the state he saw: immediately
  after submission, before the transcript is complete. If you cannot construct that, say so plainly
  rather than shipping a settled-state test that implies coverage it does not have (`#671`).
  For bug 1, the mirror: a single-row fixture cannot distinguish joined rows from separated ones —
  you need at least two.
- **The red-proof contract changed today** (`#795`, `#800`): **name the production seam you break —
  path plus symbol or branch** — and editing a test's expected value is not a direction-1 proof.
  **Read the current paragraph in `briefs/boilerplate.md`.**
- Run `python3 dev/redproof.py`; `python3 dev/lessons_index.py --act red-proof` before any injection.
- **Never `git checkout` to restore** (`#349`); snapshot the FIXED file (`#608`).

## Scope discipline

Three defects, and **bug 3 may be out of scope if it turns out to live in the writer** — that is a
data-integrity fix with its own blast radius. **If it does, report it and let me file it** rather than
widening this lane. Bugs 1 and 2 are the deliverable he named.

`#373` (topic chats v2) is a separate, larger task. **Do not start it.** If you find bug 1's real fix
belongs in v2's rework, say so and fix the presenting symptom anyway — he is looking at it now.

**Editing `client/*` makes lint ERROR until `just build-client` is re-run** (`#653`'s staleness
detector). Rebuild and commit the artifacts.

## Lessons selected for this task

Cite these by **content**, not my wording — verify each before relying on it.

- **`#671`** — a check that examined nothing must not read as passing. A settled-state test for a
  transient bug is the purest form of it available to you.
- **`#651`** — a name must not imply more than it proves.
- **`#440`** — one supported way, if you touch the row renderer.
- **`#611`** — a check whose absence reads as a pass. (Verify: `#611`'s line, not `#671`'s.)
- **`#763`** — establish the mechanism before fixing; my hypothesis in the entry is not a measurement.
...
Browser guards bind **39890–39899**, hub **39880–39889**. Load ~23; `#666` measured guards return
WRONG answers under load — **if one reds, re-run it quiet before believing it**, and if you could not
run them, say so rather than reporting a visual fix as verified when it was not.

## Bars

**Master is RED on three nodes and NONE of them are yours** — `/subagent-policy` was added as a
thirteenth write route without registering or receipting it (`#814`, dispatched, owns them):
`test_reconcile_submissions.py::test_submission_routes_match_watch` and two
`test_user_events_http.py::E2Shadow` nodes. Do not fix them, do not count them against yourself, and
...
- Main-checkout lint reads **1**; the remaining WARN is the `lessons.md` near-duplicate, which is
  **human-gated and must stay**. Your worktree reads ~5. `python3 lint.py | tail -1` — the count is
  in the **trailer** and rows are **indented**, so `grep -c '^WARN'` returns a false **0**.
...
- Do not bind `:35110` (deployed dashboard — **he is actively using it right now**) or `:35113`.
- Do not use `attn` — report to the coordinator inbox and let me decide what reaches the human.
- Rebase onto master before you finish.
