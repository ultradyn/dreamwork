# BRIEF — #776 RE-LAND: the check was right, the door was wrong

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-776cite2
Branch: cx-776cite2
Base sha: 8c6481fb
...
**Task:** `python3 dev/ledger.py get 776 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry and all its notes, including the one recording that the refusal design was killed before dispatch.

## What happened

A previous lane built this and it was **good**: report-only, never refusing; unresolved `ledger.py get N` and bare `#N` named in dispatch output; a missing/unreadable/locked ledger reporting `ledger reference check DID NOT RUN` and allowing the launch; and the honest limit that the bounded detector cannot catch the bare `get 199` form (reported rather than widened — that was the right call and it stands).

It merged, then the full gate reded:

```
test_no_raw_connect.py::test_no_raw_sqlite_connect_in_production_sources
  dev/dispatch_lane.py:159: connection = sqlite3.connect(
```

Resolving a ledger id means reading the ledger DB, and the lane reached for raw `sqlite3`. **`dreamwork_db/core.py` is the one sanctioned door** and that guard exists to enforce it across *all* production sources (#440). `dev/dispatch_lane.py` is a production source. Reverted at **`8c6481fb`**.

**Nothing else was wrong with the change.** This is a one-door fix, not a redesign.

## FIRST: recover the reverted work, and verify the recovery

```
git revert --no-edit 8c6481fb
git diff master --stat
```

**If that `git diff` prints nothing, STOP and report it.** The branch `cx-776cite` is now an ancestor of master, so a plain merge or cherry-pick brings back nothing — that is why this is a revert-of-the-revert. It should show `dev/dispatch_lane.py` and `test_dispatch_lane.py` (~+144 lines).

## THEN: route the read through the sanctioned door

Replace the raw `sqlite3.connect` with `dreamwork_db.core`. Relevant surface: `StoreSpec`, `DatabaseHandle`, `Access`, and the exception ladder `DatabaseError` / `NotFound` / `Busy` / `SchemaMismatch`.

**The behaviour you must not lose is the degraded path**, because it is the part that makes this check safe. `Busy` is what a locked store raises, and today's `DID NOT RUN` path is exactly the right response to it — an unavailable ledger is not a bad brief, and the launch must still proceed. Make sure every failure mode you previously handled by catching a bare `sqlite3` error still maps to a named outcome through `core`'s ladder, and that **none of them can turn into a refusal or a crash.** `dispatch_lane.py` runs immediately before `os.execvp`; an exception here is a hard stop on the loop's only dispatch route.

If routing through `core` genuinely cannot express something the raw connection could — a read-only open against a store with no schema, say — **report that rather than working around it with a second door.** That finding would be worth more than the feature.

## Run these, explicitly

Your targeted subset is `test_dispatch_lane.py`. **Also run `test_no_raw_connect.py`** — it is the guard that reded, and it governs all production sources rather than your files, so no owned-files list would ever point you at it. That is a known structural gap (filed as #778); until it is fixed, this brief names the guard by hand.

```
just pytest test_dispatch_lane.py test_no_raw_connect.py
```

Both must be green before you report. The boilerplate's targeted-subset rule still applies otherwise — do not run the whole tree; the coordinator owns the full merged-tree sweep.

## Red-proof

The prior red-proofs are encoded in the recovered tests — re-run them, do not redo them. What is **new** is the door: inject a raw `sqlite3.connect` back into `dispatch_lane.py` and confirm `test_no_raw_connect.py` reds naming the file and line. That is direction 1 for the change you are actually making.

Direction 2 for the same change: prove the degraded path still degrades. Point the check at a **locked** ledger and show it reports `DID NOT RUN` and **still launches** — through `core`, not through a bare exception handler. A check that silently starts refusing on a locked store would be worse than the bug (#755, #671).
...
- **#440 — one supported way.** `dreamwork_db.core` is the single connection route; a second door is the defect.
- **#671 — a check that examined nothing must not read as passing.** The `DID NOT RUN` path.
- **#755 — a check that fires on a healthy input is worse than no check.** Why a locked ledger must never block a launch.
- **#651 — a name must not imply more than it proves.** The bare-`get 199` limit stays named, not quietly widened.
- **#707 — every widening multiplies false attribution.** Do not widen the detector while you are in here.

## Bars

**Measured at dispatch, 2026-08-01, base `8c6481fb`:** worktree lint bar **5 warnings**; `test_dispatch_lane.py` + `test_no_raw_connect.py` collect **23** together at base. Re-measure and quote what *you* got.

- Count tests by collecting, not by grepping `def test_`.
- Machine load is ~22 from unrelated work — if something is flaky on timing, say so rather than retrying until green. Limit to 2 threads.
