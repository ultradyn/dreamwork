# BRIEF — #803: the memory advisory is silent when it cannot measure, and accepts impossible values

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-803mem
Branch: cx-803mem
Base sha: 7ecb732a
...
Lane-owns: `dev/concurrent_tests.py`, `test_concurrent_tests.py`

**Task:** `python3 dev/ledger.py get 803` and read `get 785` for why the trigger changed.

## Small, and worth doing carefully because the last fix here nearly cost real damage

#785 changed this advisory from `swap-used > 50%` to `MemAvailable < 8 GiB`, because the old form
fired beside 28G free and sent me hunting a leak that did not exist — I came close to killing 82 of
Max's browser processes on the strength of it. So the bar here is: **report the reading, never
diagnose a cause the number does not prove.**

An independent reviewer found three concrete input states (LOW consequence — the line is advisory and
modern Linux supplies a sane field — but each is a real missed state, not a suspicion):

1. **Older kernel, 16 GiB total, swap 8/8 GiB used, NO `MemAvailable`:** renders only the
   pytest/browser advisory. The old trigger would have made noise. #785 chose silence-on-absent
   deliberately (never fabricate a verdict) and **that choice is right** — but silence when the field
   is *absent* is different from silence when memory is *fine*, and the reader cannot tell them
   apart. `#611`/`#702`: a check that did not run must say so, because absence reads as a pass.
   One line naming the absent field is probably the whole fix.
2. **A negative parsed value** renders `mem: -1G available of 16G`.
3. **A value larger than `MemTotal`** is silently treated as healthy.

**Re-derive all three** against constructed `/proc/meminfo` inputs before changing anything.

**NOT in scope:** the uncalibrated 8 GiB floor. #785 named that as an open limit and it stays open —
closing it needs a measurement of where a guard actually stalls, which nobody has taken.

## Red-proof

- **Direction 1:** break each new branch and watch it red on the **discriminating** message — that
  the field was absent, or that the value was impossible; not merely that output differed.
- **Direction 2, mandatory:** the healthy path must stay silent. A machine with plenty of memory must
  produce no clause at all — an advisory that speaks on every run is one nobody reads, and that
  principle has a home in `lessons.md` now (cite by content, via #786). Prove the calm case explicitly.
## Standing bars
