# BRIEF — #801: 106 watch.py citations outside the guarded set still point at the wrong lines

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-801cites
Branch: cx-801cites
Base sha: 7ecb732a
...
Lane-owns: the affected documentation files (34 of them — enumerate them yourself), and a checking
tool if you build one. **NOT `test_reanchor_citations.py` or `dev/apply_reanchors_i3.py` unless you
are extending the guarded population deliberately — say so first.**

**Task:** `python3 dev/ledger.py get 801` and read `get 789` for what was already fixed.

## What #789 fixed, and what it did not

#689 inserted 12 lines at `watch.py:3353`. #789 repaired the **guarded** population — the 74
`(path, line, symbol)` anchors bound by `test_reanchor_citations.py` — and made them self-healing by
deriving the line from the symbol. Master is green on that.

An independent reviewer then measured **everything else**:

- **106 citations, 85 unique target lines, 34 tracked files** are definitely shifted and unguarded.
- Method (re-derive it): compare the pre-insertion source at `dc739001` against current, require
  exact old-line equality at `N+12`, and **exclude** the 68 `watch.py` evidence lines governed by the
  reviewed-anchor population.
- Concrete: `.dreamwork/docs/262-witness-audit.md:10` says `make_handler (watch.py:4870)`. Before the
  merge line 4870 was `def make_handler(...)`; now 4870 is blank and the definition is at 4882.
  `ws-delta-transport.md` carries 17 affected citations.

A reader following any of these lands on unrelated code.

## The judgement, which is most of the work

**Do not mechanically add 12 to 106 numbers.** That repairs today and rots on the next `watch.py`
edit — and `watch.py` is the most-edited file in the repo. For each affected citation decide which it
is:

1. **A real anchor** a reader is meant to follow → it should join the guarded, self-healing
   population (`#789`'s form: name the symbol, derive the line).
2. **Prose decoration** that happens to carry a number → the number is a liability; say so and remove
   or generalise it.
3. **A historical record** describing the tree as it was at some past sha → then the sha belongs in
   the citation and the line is correct *relative to it*. Do not "fix" these; they are not broken.

Class 3 is real and easy to damage — several of these docs are audits and post-mortems whose whole
point is what the code looked like then. **Getting one of those wrong is worse than leaving all 106.**

**Judge with IGC** — worktree-local `./igc-method.md`.

## Red-proof

- **Direction 1:** whatever you land, break it and watch it red naming the specific citation and how
  far it drifted.
- **Direction 2, mandatory:** construct a citation that is **stale but shaped correctly** and show
  your work does not make it *look* current. A rewritten line that still asserts an unverified number
  is the same defect with fresher prose (#764). And if you convert citations to the guarded form,
  show what happens to one whose symbol appears many times — ambiguity must be loud, not silently
  resolved to the first hit (#671).
## Standing bars
