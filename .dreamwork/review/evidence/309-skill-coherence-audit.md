...
- **Why it is wrong.** The file has two sections, both real and both
  ...
  - `watch.parse_ledger` (`watch.py:6006`) returns `(open ids, landed
    ids)` and reads **both** sections via `LEDGER_SEC_OPEN` /
    `LEDGER_SEC_LANDED` (`watch.py:5935-5936`). The docstring states the
  ...
  - The burndown series (`ledger_series`, `watch.py:6033`) derives
