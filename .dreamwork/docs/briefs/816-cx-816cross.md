# BRIEF — #816 (P1): a coverage line that calls visible briefs invisible, and a fixture green against a 388/388 UNRESOLVABLE audit

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-816cross
Branch: cx-816cross
Base sha: 05f7f902
...
Lane-owns: the two seams named in `#816`'s note and their tests. **Do NOT touch** `watch.py`,
`test_watch.py` (`cx-818chatwake`), `client/*` (`cx-657chat2`), `test_guard_argv.py` or
`test_dreamwork_db_import.py` (`cx-820vacuous`). If your fix genuinely requires one of those, STOP and
report that back to me rather than taking it — a brief whose scope cannot express its deliverable is a
brief I got wrong, and I would rather re-cut it than have you work around it.

**Task:** `python3 dev/ledger.py get 816 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`
— the note carries both findings with their evidence. **Note the `--ledger` form**; a bare `get`
refuses from a worktree.

## Provenance: these came from a cross-change lens, and that lens found things no single-change review could

`#806` was review lens D — it looked only at **interactions between changes**, each of which was
already gated and merged individually. Four lenses ran today (`#790` correctness, `#791` red-proof,
`#805` contract drift, `#806` cross-change) and produced **14 defects with ZERO overlap**. These two
are lens D's, and they exist precisely because nothing else was looking at the seams *between*
landings. Read `#806`'s report before you start.

## The two findings, and both are the same disease

1. **A coverage line reports visible briefs as invisible.** A statement about coverage that is false
   in the safe-sounding direction: it *understates*, so nobody chases it, and the number quietly
   licenses a conclusion it does not support. This loop already lost two days to exactly that — the
   brief-corpus `AUDIT IS INCOMPLETE` banner, which produced one false conclusion (`#786`'s) before
   anyone noticed the corpus was simply untracked.

2. **A fixture that passes against a 388/388 UNRESOLVABLE audit.** Every single item unresolvable, and
   the check is green. This is `#671`'s canonical shape at full scale: a check that examined nothing
   reading as a positive all-clear. **A 100% failure rate producing a pass is not a threshold problem;
   it means the fixture is not connected to the thing it claims to audit.** Find out what it *is*
   connected to before you decide what to change.

## Deliverable

Close both. For each: make the check bind the property it names, and make its failure message name a
mode it can actually detect (`#651`).

**Do not just adjust a threshold.** If 388/388 unresolvable passes, no threshold makes that check
meaningful — the wiring is wrong. Say what the wiring was.

## Red-proof

- **Direction 1:** break the production seam — **name it as path plus symbol or branch** — and watch
  each check red on a **discriminating** message. For (2) the failure must name *that the audit
  resolved nothing*, not merely that a value differed.
- **Direction 2, mandatory:** construct the input where the thing checked is broken but the check
  still passes. For (2) you have it handed to you: the current fixture IS that input — 388/388
  unresolvable and green. Show your fixed check going red on that exact state. For (1): a coverage
  line recomputed from the same wrong source it always used will agree with itself; prove your number
  comes from the population it claims to describe.
...
- **`#671`** — a check that examined nothing must not read as passing. Finding (2), at full scale.
...
- **`#786`** — a false measurement licensed a wrong conclusion; and a check that fires on a healthy
  input is worse than no check, so do not cure this by making the checks brittle.
- **`#764`** — a document that ships a false measurement teaches its readers that its measurements
  are decorative. Finding (1) is this.
...
```
Plus the test files covering the two seams — name them in your report with collected counts.

## Bars

**Master is GREEN as of `05f7f902`** on targeted files; a full gate is in flight. `#809` landed today
demonstrating two mutations that survive their whole relevant file — do not read green as reassurance.
...
- Rebase onto master before you finish. Master moved 29 times today and four lanes are live.
