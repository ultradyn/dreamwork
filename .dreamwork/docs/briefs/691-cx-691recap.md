# BRIEF — #691: design the cheap-model recap. DESIGN ONLY — his review gates implementation.

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-691recap
Branch: cx-691recap
Base sha: beddf975
...
Lane-owns: `.dreamwork/docs/plans/recap-design.md` (create it). **NO production code this lane.**

**Task:** `python3 dev/ledger.py get 691 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry and ALL notes. His verbatim request is in there; treat it as the spec.

## Two instructions bind the PROCESS, not just the feature

1. **"Present the design for me to review first (GATES IMPLEMENTATION)."** No code lands before he
   has reviewed a design. If you write production code this lane, the lane has failed regardless of
   quality. The deliverable is a design document.
2. **"Ask questions if unsure."** Genuine open questions are a deliverable, not a failure. Put them
   in a clearly-marked section of your design doc. **Do NOT write to `.dreamwork/questions.md`** —
   that is the coordinator's and the human's channel and lanes are forbidden from writing it. I will
   carry your questions there.

And the feature itself **must be feature-gated** so it can be turned off later. Say exactly how.

## What he asked for, and what you must decide

Cheap-model recap of the main agent's recent actions, fed a digest of the session log, saved to the
DB, most-recent shown on the dashboard, transitioning when updated, run once per heartbeat (or every
nth) starting at ~40% of the heartbeat period (~2min after the beat). `glm52` via `ccc` for now, but
**configurable**.

Design decisions I am NOT pre-making — argue each:
- **What the digest contains and how it elides.** He said "as much detail as is reasonable (cap
  length and elide middle when too large)". Where does the cap come from and what does elision keep?
- **DB shape.** Read `dreamwork_db/` first — `#645` is migrating questions to the DB in parallel and
  a recap log should not invent a second storage idiom (#440). Read `dreamwork_db/core.py`'s error
  ladder (`Busy`/`Corrupt`/`SchemaMismatch`/`ConstraintViolation`) and say how a recap write fails.
- **Failure posture.** A recap generator that dies must not affect the heartbeat or the loop. Say
  what happens when `ccc` is down, slow, or returns garbage. **A stale recap presented as current is
  the defect to design against** — say how the surface distinguishes fresh from stale from absent
  (this is the #671/#611 family: a panel that examined nothing must not read as current).
- **Configurability.** Which model, which cadence, on/off. Note `#650`'s subagent-policy file is his
  authored prose and a *separate* concern; do not fold recap config into it.
- **Where it renders.** The client is now real `.js` under `client/` (`views.js`, `router.js`).
  `glm-646policy` is editing those files this tick — your design may name them but you must not edit
  them.

**Judge rival designs with IGC** — the **worktree-local `./igc-method.md`**, not any copy under
`~/.claude-p`.

## Red-proof does not apply as usual — but honesty does

There is no code to red-proof. The equivalent obligation: for each design claim that could be wrong,
say what would falsify it and what you actually checked. A design that asserts "this is cheap" without
measuring a digest size, or "this cannot block the heartbeat" without reading how the heartbeat runs,
is the same defect one layer up. **Name what you could not determine (#702) rather than smoothing it.**
