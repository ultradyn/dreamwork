# BRIEF — #670: a default-named screenshot writes into a stranger's git tree

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/glm-670mcproot
Branch: glm-670mcproot
Base sha: dc739001
...
Lane-owns: the MCP/playwright configuration this repo controls (find it — that search is part of the
task), plus any `dev/` helper and test you add. You may READ `dev/lane_scratch.py`; do not edit it.

**Task:** `python3 dev/ledger.py get 670 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`

## Measured, not theorised

Lane `lane-663indicator` found the shared MCP playwright browser's default screenshot directory and
allowed roots pointing at **another lane's worktree** — `.worktrees/lane-clientextract`. A
default-named screenshot from `lane-663indicator` would have been written into that tree. It worked
around it by pinning output to `/tmp/.playwright-mcp`, but only because it happened to notice.

**Why this is worse than the shared-scratchpad case it rhymes with (#652):** a scratch collision
corrupts a lane's own snapshot and stays inside scratch. This one writes a file **into a git working
tree belonging to someone else**, and every consequence is quiet — an untracked file appearing in a
`git status` the lane did not create (and this repo's convention treats untracked content in a
worktree as the signal for "does this hold unsaved work?" before removal), a `git add`/`--only` that
sweeps it, or a worktree-removal check that refuses because of a stranger's screenshot. In the
measured case the target was the coordinator's own checkout.

**Verify the current state before fixing.** That measurement is from an earlier session and the
harness may have moved. If the default is now safe, say so — a finding that the bug is gone is a
result, and better than a fix for a problem that no longer exists.

## The entry recommends (b), and gives the reason

- **(a) Brief-level:** tell lanes to pin the output root before any screenshot. Cheap, immediate,
  and carries the known weakness that brief coverage is exactly what fails — one more line competing
  for a lane's attention.
- **(b) Config-level:** pin per-lane output roots in the MCP configuration so the default is correct
  and a lane cannot get it wrong by not reading. Same shape as #652's chosen fix — **derive the path
  from lane identity rather than ask the lane to remember one** — and `dev/lane_scratch.py` already
  computes exactly such a path, so the derivation exists and needs wiring, not inventing.

**Prefer (b), or (a)+(b).** Judge with IGC — use the **worktree-local `./igc-method.md`**, not any
copy under `~/.claude-p`.

**If (b) is impossible because the MCP server config is not per-lane addressable, SAY SO explicitly**
and fall back to (a). An honest *"the config cannot express this"* is a real finding about the
harness, not a failure — and it is the single most useful thing you could report if true.

**Note `dev/lane_scratch.py` changed today** (#694 landed role-keyed paths: author adds no segment,
a reviewer gets `role-reviewer/`). If you wire its derivation, you inherit that — which is probably
right, since a reviewer taking screenshots has the same collision problem one level down.

## A second, smaller finding to fold in

From the same lane: **the MCP browser blocks `file://` URLs**, so verifying a built review artifact
requires binding an ephemeral port. Every artifact-verifying lane hits this once. Put it wherever a
lane will actually meet it rather than in a report.

## Red-proof

- **Direction 1:** show a default-named screenshot lands inside the acting lane's own space; break
  the wiring and watch a test red naming the wrong root.
- **Direction 2:** show that a lane which does NOT pin anything still cannot write into another
  lane's tree — that is the entire point of preferring (b). If your fix depends on the lane doing
  something, it is (a) wearing (b)'s clothes; say so rather than claiming the default is safe.
- Do not bind the guard ports 39890-39899 — `cx-784bisect` owns them this tick. If you need a
  browser, use an ephemeral port outside that range and say which.

## Lessons selected for this task

- **Derive the path from identity rather than ask the lane to remember one** — #652's chosen fix,
  and the reason (b) beats (a).
- **A name must not imply more than it proves** (`#651`) — "default output root" implies lane-local.
- **Report what you cannot classify** (`#702`) — if the config cannot express per-lane roots, that
  is the finding.
- **Every widening multiplies false attribution** (`#707`) — do not turn this into a harness audit.
