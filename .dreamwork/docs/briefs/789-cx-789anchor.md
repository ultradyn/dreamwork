# BRIEF — #789: line-pinned citation anchors rot on every watch.py edit, and one just reddened my merge gate

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-789anchor
Branch: cx-789anchor
Base sha: 8d60bbae
...
Lane-owns: test_reanchor_citations.py, dev/apply_reanchors_i3.py, and a new module if your design
needs one. **NOT `watch.py`** — another lane is editing it right now (see below).

**Task:** `python3 dev/ledger.py get 789 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`

## What happened, measured — re-derive it, do not inherit it

I merged `#689`, which inserted **12 lines at `watch.py:3353`**. That reddened
`test_reanchor_citations.py::test_each_reviewed_anchor_line_contains_the_named_evidence`, which pins
74 `(path, line, symbol)` anchors. My measurement at `8e3bfc10`:

```
53 watch.py anchors sit BELOW line 3353 — 52 broken, 52 repaired by exactly +12, no outlier
 1 of them was already correct by coincidence (the symbol appears at both lines)
15 watch.py anchors sit ABOVE the insertion — untouched
 6 non-watch.py anchors (ledger_parse.py, ledger_write.py, client/router.js,
   dreamwork_db/migrate.py) — all intact
```

Example: `track_question_updates` was `watch.py:3683`, is now `watch.py:3695`.

**Re-derive every one of those numbers before building on them.** I am telling you the method (walk
the anchor tuples, test `symbol in lines[line-1]` at the pinned line and at `line+11`, partition on
3353) so you can audit the method rather than trust the number. This loop has twice today shipped a
brief whose asserted measurement was wrong, and the only reason both were caught is that the lane
checked anyway.

## The guard is CORRECT. Do not "fix" it by weakening it.

Those citations genuinely no longer point at their evidence. A guard that noticed is doing its job.
**The defect is the anchor FORM, not the check** — and the cheap fix (add 12 to 52 numbers) makes the
test green today and rots again on the very next insertion. Landing only that is not the deliverable
and I will send it back.

## Why this is P1: the tax compounds

`watch.py` is the most-edited file in the repo — 306 commits in 14 days, and it has nearly doubled
since 28 July. Under the current form, **any insertion above line ~3300 reddens the merge gate with
up to 53 failures that have nothing to do with the change being gated.** That is a tax on every
future `watch.py` increment, and worse, it manufactures precisely the kind of noisy red that trains a
coordinator to stop reading reds — the principle `lessons.md` homed **one hour before this failure
happened** (`#786`: *a check that fires on a healthy input is worse than no check*). Go read that
entry; it is the argument for why you are here.

Scope bound worth knowing: this guard is **not** in `dev/repo_wide_guards.py`'s always-run registry,
so lanes running targeted tests plus the registry never see it. Only a full `just pytest` does — i.e.
my merge gate. The fleet is not blocked; I am.

## The design question, which IS the deliverable

The anchor tuple **already carries the symbol**. The line is redundant information that rots while
the symbol does not. Weigh these and say which you took and why:

1. **Make the symbol authoritative, the line derived** — the guard locates the symbol and reports
   drift, or refreshes the line. Cost: a symbol that appears many times is ambiguous, and a symbol
   that gets renamed fails differently than one that moves.
2. **Keep the line and give the guard a reanchoring tool** it runs when drift is detected.
3. Something better. These are leads, not a menu.

**`dev/apply_reanchors_i3.py` is 143 lines, was written for `#777` increment 3, and is currently
referenced by nothing.** I have had an open question for two days about whether it earns a permanent
home; this failure answers it — it is precisely the tool this needs. Read it first and decide whether
to generalise it or fold its logic into the guard. Either is defensible; leaving it orphaned while
building a third thing is not (`#440`).

**Judge the rival forms with IGC** — use the **worktree-local `./igc-method.md`**, not any copy under
`~/.claude-p`. You are choosing between options with real trade-offs, which is exactly what it is for.

## A concurrent edit makes this concrete, not theoretical

**`glm-646policy` is editing `watch.py` right now** (a posture-control UI increment). Its change will
shift these anchors **again**, by a different amount, before your work merges. This is not a warning
to work around — it is the requirement, made physical: **a blanket `+12` is guaranteed to be wrong by
the time you land.** Whatever you build must survive an arbitrary concurrent insertion into
`watch.py` that you cannot see and did not predict.

Do **not** edit `watch.py` yourself, and do not coordinate with that lane directly — route anything
through me.

## Red-proof

- **Direction 1:** break your mechanism and watch it red on the **discriminating** message — the
  failure must name *which* anchor drifted and by how much, not merely that an assertion failed.
- **Direction 2, mandatory and the one this task turns on:** construct the case where your fix
  **looks** applied but is not. Specifically: **simulate an insertion into `watch.py` at a line you
  did not anticipate** (a scratch copy — do not edit the real file) and show your mechanism still
  reports the truth. A fix validated only against the +12 shift you were handed is a fix tuned to one
  fixture. If your design cannot survive an arbitrary shift, say so plainly rather than implying it
  is closed (`#702`).
...
id whose entry does not contain it, which is what `#786` was filed for. Verify each before relying on it.

- **`#764`** — *"The fix for miscited task ids was to cite `lessons.md:<line>` — and line citations
  rot too."* The parent principle; this task is its most expensive instance yet.
- **`#786`** — the two newly-homed entries, especially *a check that fires on a healthy input is
  worse than no check*. Your noise bound.
- **`#671`** — *"`ledger.py sweep` never got the `#294` store dispatch — it examines zero ledger
  entries and says so confidently."* A reanchoring tool that silently skips an anchor it could not
  resolve is this defect.
- **`#651`** — *"A guard whose message names a failure mode it cannot detect."*
- **`#440`** — one supported way. One route for reanchoring, not a tool *and* a hand-maintained list.

## Run these, explicitly

```
just pytest test_reanchor_citations.py
just pytest $(python3 dev/repo_wide_guards.py list)
```

If your change reaches any other consumer of the anchor list, **find it by the call graph rather than
by the file list I gave you** — a verification list derived from the files you will edit rather than
from what they reach is `#763` instance six, and it has already cost this loop one red gate.

## Bars

**Master is RED right now, and this task is the reason.** At `8d60bbae` a full `just pytest` reads
**1 failed, 2751 passed** — that one failure is this test and it is your starting state. Removing it
*durably* is the deliverable. Re-measure and quote what you got.
...
- Do not bind `:35110` (deployed dashboard) or `:35113` (dev).
