# BRIEF — #738: he ruled eight minutes ago, and his ruling makes one half right and the other half a defect

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-738draw
Branch: cx-738draw
Base sha: a7a88df9
...
Lane-owns: the **draw-mode control and its preference code only**, in `client/`. **`cx-281tasks` is
live in `client/` right now** working on the `/tasks` page — different surface, same directory. Keep
your diff tight to the draw-mode code so the rebase is trivial, and do not refactor anything shared.

**Task:** `python3 dev/ledger.py get 738 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`
— read my note recording his ruling. **Note the `--ledger` form**; a bare `get` refuses from a worktree.

## His ruling, verbatim, eight minutes old

Receipt `757598ba`, 2026-08-01 14:53, typed into the dashboard composer:

> **"should persist per browser and sync across tabs of same browser."**

The previous lane implemented `localStorage` (key `dw:draw-mode`, the `burnStepPref`/`burnLimitPref`
idiom) and asked him to rule on two consequences. **He ruled on both, and they go opposite ways.**

### Consequence 1 — PER-BROWSER IS CORRECT. Build nothing here.

He did **not** want tint parity. `tint` persists server-side via `.dreamwork/watch-tint` + `POST
/tint`, shared across machines and committable. **He does not want that for this.**

So the follow-up the previous lane proposed and left for a wave that owns `watch.py` — `POST
/draw-mode` + a `.dreamwork/draw-mode` file + a `collect()` read — is **REFUSED. Do not build it.**
The setting is deliberately per-browser and per-machine. If you find yourself editing `watch.py`, stop
and re-read this paragraph; the whole point of his answer is that the server is not involved.

### Consequence 2 — INDEPENDENT PER-TAB IS THE DEFECT. This is your entire deliverable.

The previous lane's own words: *"two windows on the same project keep independent draw modes."* That
is exactly what he has now ruled against. **Same browser, two tabs, must agree.**

## The mechanism, and the sharp edge that will bite you

The standard route is the **`storage` event**: writing `localStorage` fires `storage` in *other* tabs
of the same origin, so a sibling adopts the change with no server round trip and no polling.

**The edge, and it is the predictable bug here: `storage` does NOT fire in the tab that made the
write.** So the writing tab must apply its own change directly, and the listener handles only
siblings. Get this wrong in either direction and you get a real defect:

- apply in both paths → the writing tab applies twice (visible if the change animates or toggles);
- apply only in the listener → the writing tab is the one tab that does not respond to its own click.

**Both of those look fine in a single-tab test.** That is what makes this task's red-proof
non-optional rather than ceremonial.

## Red-proof

- **Direction 1:** break the sync and watch it red on the **discriminating** message — the failure must
  name that the *sibling tab did not adopt the change*, not merely that a value differed.
- **Direction 2, mandatory and this is the one the task turns on:** construct the case where the fix
  **looks** applied but is not. **A single-tab test cannot distinguish any of the three
  implementations above** — writing tab correct, sibling broken; sibling correct, writing tab
  double-applying; both wired to the same handler. Your test must exercise **two contexts**, or it
  passes vacuously for every version. Show the single-context test failing to catch a sabotage, then
  show yours catching it.
...
## Keep the existing path; do not invent a new one

Use the **existing `dw:draw-mode` key** and the `burnStepPref`/`burnLimitPref` idiom. This is an
addition to a preference path the page already uses — `#440`, one supported way. **Default stays
`animated`**: nobody who never touches the control should see any change.

**Editing `client/*` makes lint ERROR until `just build-client` is re-run** (`#653`'s staleness
detector). Rebuild and commit the artifacts, or you red your own gate.

Read `transitions.md` before changing anything that animates — a draw-mode switch arriving in a
sibling tab is a live re-render, and that document governs whether it may move.

## Lessons selected for this task

Cite these by **content**, not my wording — verify each before relying on it.

- **`#671`** — a check that examined nothing must not read as passing. A single-tab test here is that.
- **`#440`** — one supported way. Existing key, existing idiom.
- **`#651`** — a name must not imply more than it proves. A handler called "sync" that only fires in
  one direction is this.
- **`#611`** — a check whose absence reads as a pass. (Verify: `#611`'s line, not `#671`'s.)
- **`#763`** — read the owning decision before overriding it. His ruling IS the decision here; the
  server-side route is refused, not merely deprioritised.
...
Browser guards bind **39890–39899**, hub **39880–39889**. Load is ~23; `#666` measured guards return
WRONG answers under load, so **if a guard reds, re-run it quiet before believing it**, and if you
could not run them say so rather than reporting a UI change as visually verified when it was not.

## Bars

**Master is GREEN as of your base** — the one red earlier today (`test_client_env.py`, `#810`) is
merged and cleared; I re-ran it at `15 passed`. A full gate is in flight. **Quote your own failure
list explicitly** rather than saying "same as baseline".
...
- Do not bind `:35110` (deployed dashboard, live — **he is watching it right now**; he deployed and
  answered a question in the last fifteen minutes) or `:35113` (dev).
- Do not use `attn` — report to the coordinator inbox and let me decide what reaches the human.
- Rebase onto master before you finish; `cx-281tasks` is in `client/` too.
