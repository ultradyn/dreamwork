# BRIEF — #263: design the durable user-event inbox and replay CLI (DESIGN ONLY)

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-263inbox
Branch: cx-263inbox
Base sha: beddf975
...
Lane-owns: `.dreamwork/docs/plans/user-event-inbox.md` (create it). **NO production code.**

**Task:** `python3 dev/ledger.py get 263` — read the entry and ALL notes. It is `type: design`,
origin human, and it already carries **accepted design decisions**. Read those first; your job is to
complete the design on top of them, not to relitigate what is settled.

**Scope gate, and it is firm:** implementation of this needs an authorisation from Max that he has
not given (the "lane G" question is still open and is mine to put to him). **A design document is in
scope; code is not.** If you believe a decision cannot be made without prototyping, say so and
describe the prototype — do not build it.

## What the design must settle

The entry names these: an immutable disk event written **before** acknowledgement; the monitor only
wakes the dreamer; early-loop replayable/idempotent ingestion carrying statuses, receipt ids and
errors; a CLI like `ud-dw-user-events --limit 20` returning exact events and processing status;
append-only JSONL vs a one-file spool; atomicity, concurrency, redaction/retention/migration, and
dual witnesses.

**Ground it in what already exists rather than designing in the abstract.** The E3 cutover already
journals a receipt for every write route, `dev/journal_consume.py` already implements
`pending → process → consume --through <ord>`, and `#531` is why the consume is bounded: an event
landing between the read and the consume is otherwise advanced past unread. `#519` records that a
receipt has no id on the wake line, so the same instruction can arrive twice and must be acted on
once. Read that machinery and say explicitly what your design **replaces**, what it **keeps**, and
what it **migrates** — a design that ignores the working half is not a design, it is a rewrite.

**Judge rival storage shapes with IGC** — worktree-local `./igc-method.md`.

## Honesty obligation in place of red-proof

No code, so no injection. The equivalent: for every design claim that could be wrong, say what would
falsify it and what you actually checked. Specifically — **a durability claim that was not tested
against a crash mid-write is a guess.** Describe the crash cases (partial line, torn record, full
disk, concurrent writer) and what your shape does about each. Name what you could not determine
rather than smoothing over it (#702).
