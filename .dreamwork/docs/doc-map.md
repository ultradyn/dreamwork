...
| `.dreamwork/docs/plans/` | Active feature plans, alphabetical (answers-page, artifact-templates, composer-row, daemon-mode, dreamhub-stage1, goal-hierarchies, heartbeat-into-monitor, hub-public-auth, lan-bind, note-reply-threading-254, parallel-architecture, task-store-schema, tasks-page, ud-dreamtask, ud-dreamwork-github, user-event-journal, version-and-upgrade) | Prune when features fully land; `lint.py` fails the row against the directory |
