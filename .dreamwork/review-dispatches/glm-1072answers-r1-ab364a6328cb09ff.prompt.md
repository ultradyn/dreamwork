...
Head `0124d2db`, rebased onto master `0ea95d68`. 3 commits, 12 paths, 676 insertions. Adds an
...
Its rebase (master `6d91f7f4` → `0ea95d68`) touched only `lessons.md` / `questions.md` /
`coordinator-checklist.md` with no code overlap, and it re-armed the red-proof in full afterwards.
Take all of that as given and spend the round on the constructions above.

Review sha (pinned at dispatch, #1056): 0124d2db58c394cd89b2cd92844a165483c4a3c4 — review THIS commit; state the sha you actually reviewed in your verdict, and report (do not silently resolve) any mismatch with the branch tip.
