# BRIEF — #807: the recipe built to commit the corpus refuses on the only state it was built for

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-807corpus
Branch: cx-807corpus
Base sha: 7f978b35
...
Lane-owns: `dev/dispatch_lane.py`, `test_dispatch_lane.py`, and the `commit-corpus` recipe in the
justfile.

**Task:** `python3 dev/ledger.py get 807`. The note is the specification and I measured it myself.

## How this was found, because it matters to how you should read it

I did not audit for this. **I tried to USE `#788`'s deliverable and the first command refused.** That
is the strongest possible evidence a check is wrong: it fired on the healthy input, immediately, for
the one person it was built for.

```
$ just commit-corpus
brief integrity check failed: integrity receipt 173-cx-173gitstat.sha256 has no governed
brief artifact 173-cx-173gitstat.md; [...19 more...]
error: Recipe `commit-corpus` failed with exit code 2
```

**That message is false.** The file it says is missing is right there:

```
-rw-r--r-- 1 xertrov xertrov 29435 Aug  1 13:58 .dreamwork/docs/briefs/173-cx-173gitstat.md
```

## The mechanism — verify it yourself before you fix it

`dev/dispatch_lane.py:389-393` builds `governed` as the `*.md` files whose leading task id is
**`>= INTEGRITY_START_TASK` (766)**. Then `:406-416` walks every `*.sha256` on disk and reports a
fault when `brief not in governed`.

So a receipt whose brief is **present but has an id below 766** takes the *absent* branch and gets
reported as an orphan. `173 < 766`, and so are `262, 263, 368, 630, 631, 645, 646, 670, 689, 691,
694, 701, 763` — every one of which I dispatched today.

**The real split is between the writer and the verifier.** Dispatch receipts *every* brief regardless
of id; verification governs only ids `>= 766`. Two halves of one contract that disagree about their
own scope — and nothing noticed, because until today nobody ran the verifier over a tree containing
pre-cutoff receipts.

**Re-derive all of this before building.** I am giving you the method (`git status --porcelain`
against `ls`, then read `verify_pending` and find `INTEGRITY_START_TASK`) so you can audit the
diagnosis rather than inherit it. If I am wrong about the mechanism, say so — that is a better
outcome than a fix aimed at the wrong seam.

## Two fixes, and they are genuinely separable — argue both

### 1. The orphan check asks the wrong question

It asks *"is this brief governed?"*; the fault it reports is *"does this brief exist?"* Those are
different questions and only one of them is an orphaned receipt. An ungoverned-but-present brief is
not an orphan.

### 2. Decide what a receipt for a pre-cutoff brief MEANS — and make writer and verifier agree

This is the real deliverable and I am **not** pre-deciding it. Weigh at least:

- **Stop writing receipts below the cutoff.** Honest, but the receipts already on disk still need a
  disposition, and it makes `INTEGRITY_START_TASK` load-bearing in a second place.
- **Govern everything; retire the cutoff.** Simplest contract. But read *why* the cutoff exists first
  — `INTEGRITY_START_TASK` was introduced deliberately and you must not delete a decision you have
  not read. **Grep the module docstring and the owning ledger entry before calling it removable**;
  I filed `#763` instance fifteen against myself this session for calling a documented decision a
  defect.
- Something better.
...
Whatever you choose, `#440` binds: one supported way. The writer and the verifier must not carry two
different notions of which briefs are covered.

## The bar this must clear: the recipe has to actually work

`just commit-corpus` must **run to completion in the real main checkout** and stage the real pairs.
A fix that makes the tests pass while the recipe still refuses in the tree it exists for has changed
nothing — that is the entire defect, one level up.

**You cannot verify this half yourself: a lane must never commit to the main checkout, and must never
run this recipe against it.** So: make it correct, prove it against a fixture corpus that reproduces
the real shape (pre-cutoff pairs *and* post-cutoff pairs together — a fixture with only one class
cannot tell a fixed check from a broken one), and **state in your report exactly what I should see
when I run it.** I will run it and hold you to that prediction.

## Red-proof

- **Direction 1:** break your fixed check and watch it red on the **discriminating** message — it must
  distinguish *receipt with no file* from *receipt with an ungoverned file*, because conflating those
  two is the whole bug. A red that merely says "verification failed" reproduces the defect.
- **Direction 2, mandatory:** construct the input where the fix **looks** applied but is not. The
  specific shape: a fixture containing **only** governed pairs makes a broken orphan-check
  indistinguishable from a working one — every receipt is in `governed`, so the bad branch is never
  taken. That is exactly the vacuous shape that let this through. Show the vacuous fixture failing to
  catch a sabotage, then show yours catching it.
- **The red-proof contract changed twice in the last hour** (`#795`, `#800`): you must now **name the
  production seam you break — path plus symbol or branch** — and editing a test's assertion is not a
  direction-1 proof. **Read the current paragraph in `briefs/boilerplate.md`; do not work from
  memory.**
- Run `python3 dev/redproof.py` for the snapshot/restore protocol. Run
  `python3 dev/lessons_index.py --act red-proof` before any injection.
- **Never `git checkout` to restore an injection** (`#349`). Snapshot the FIXED file (`#608`).

## Handle this file with extreme care

`dev/dispatch_lane.py` is the loop's **only** dispatch route and it ends in `os.execvp`. A change that
makes dispatch refuse, hang, or exec wrongly does not degrade a feature — **it stops every lane in the
fleet launching at all**, including yours. `#797` landed a change to `validate_stdout` in this same
file twenty minutes ago; read it before you touch anything near it.

## Lessons selected for this task

Cite these by **content**, not my wording — verify each before relying on it.

- **`#651`** — a guard whose message names a failure mode it cannot detect. This defect is the pure
  form: it names *absence* for a file that is present.
- **`#671`** — a check that examined nothing must not read as passing. Its mirror: a check that
  refuses on a healthy input is worse than no check (`#786` homed that half).
- **`#766`** — why the receipt exists at all, and why a half-committed pair is worse than neither.
  Read this before changing what gets written.
- **`#440`** — one supported way. Writer and verifier must share one notion of coverage.
- **`#763`** — before calling something a defect, read the owning docstring for the decision. Applies
  directly to `INTEGRITY_START_TASK`.
...
If your change reaches another caller, **find it by the call graph, not by the file list I gave you**
— `#763` instance six.

## Bars

**Master is GREEN.** A full `just pytest` at `aadf579f` read **2806 passed, 0 failed** in 421s; four
merges have landed since (`#795`, `#797`, `#803`, `#800`) and a gate over them is in flight as I
write. Quote your own failure list explicitly rather than saying "same as baseline".

- Main-checkout lint reads **1** — the single remaining WARN is the `lessons.md` near-duplicate pair,
  which is **human-gated and must stay**. Your worktree reads **5**. `python3 lint.py | tail -1`;
  the count is in the **trailer** and the rows are **indented**, so `grep -c '^WARN'` returns a
  false **0**.
- **Compare the complete WARN ROW SET, not the trailer count** (`#794`, landed this hour).
- Count tests by **collecting**, not by grepping `def test_`.
- Limit builds and tests to 2 threads. Load is ~39 and it is not the fleet's.
