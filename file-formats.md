...
| `.dreamwork/review-dispatches/<branch>-r<round>-<digest16>.launch.json` | `test_dispatch_lane.py` (`test_launch_review_creates_attached_branch_worktree_and_records_cwd`, `test_launch_review_captures_runner_log_and_records_it`) | Machine-local, gitignored review-launch witness, written atomically by `dev/dispatch_lane.py` (`launch_review`). JSON fields: `attempt_id`, `branch`, `round`, `review_lane`, `pinned_sha`, `prompt_sha256`, `prompt_bytes`, `prompt`, `worktree`, `permission_mode`, `state`, `runner_exit`, `runner_log`, `runner_token`, `runs`. Meanings: `attempt_id` is `<review_lane>-<digest16>`; `branch` and positive `round` identify the review; `review_lane` is the attached review-worktree branch; `pinned_sha` is the full reviewed commit; `prompt_sha256`/`prompt_bytes` identify the exact prompt; `prompt` and `worktree` are absolute paths, with `worktree` also the runner cwd; `permission_mode` is literal `plan`; `state` is the latest preparation, worktree, or spawn outcome; `runner_exit` is currently always null in this file (the detached supervisor's exit is recorded separately — see below), and a non-null value is NOT a terminal witness (#1214 round 3 / P1(c)): a stale or corrupt launch record is the cheapest lying witness of the three, so `classify_review_dispatch` never trusts this field — only the sibling `.runner.exit.json` establishes `terminal`; `runner_log` is the absolute path to the captured runner output (the verdict is recoverable from the record alone — #1214); `runner_token` is a per-attempt hex string generated at launch (#1214 round 4) that binds the exit witness to this launch — it is carried in both this record and the sibling `.runner.exit.json`, and `_read_runner_exit` requires both to carry and match it, so a stale exit record from a different attempt or a collision resolving to the same computed path cannot read as this attempt's terminal witness; `runs` is 0 before launch is attempted and 1 once dispatch begins. The test reads the record and binds this complete field list plus the `prompt`, `review_lane`, `worktree`, `pinned_sha`, `runner_log`, and `state` semantics; `--review-status` (#1207/#1214) first reads the sibling `.runner.exit.json` (whose path is `runner_log` with `.log` swapped for `.exit.json`) and, when present, classifies the dispatch `terminal` naming the runner log the verdict is recoverable from; a missing or unreadable exit record falls through to re-probing the `review_lane` runner's cwd-containment and classifying `in-progress` (runner present) vs `runner-absent` (runner gone, exit never observed — the alarm a dead review now expresses) vs `unknown` (probe examined 0, #868). It is read-only: it never writes `runner_exit` or any state (the null is honest, #1177) | `test_dispatch_lane.py`; `lint.py` does not inspect `*.launch.json` |
| `.dreamwork/review-dispatches/<branch>-r<round>-<digest16>.runner.log` + `.runner.exit.json` | `dev/dispatch_lane.py` (`_supervise_review_runner` / `_write_runner_exit`) | Machine-local, gitignored runner-capture pair (#1214). The launcher forks a SUPERVISOR (not the reviewer directly) that forks the reviewer as a grandchild, tees its stdout+stderr to `.runner.log`, waits, and writes `.runner.exit.json` as its last act. `.runner.log` is the verbatim captured stream — a completed review's verdict lives here even when the reviewer wrote nothing durable itself (incident B). `.runner.exit.json` is an atomic JSON object `{"runner_exit": <int>, "runner_log": <abs path>, "runner_token": <hex>, "runner_log_sha256": <hex>}`; the integer is the reviewer's own exit (0 success, nonzero failure), observed by the supervisor that reaped it. `runner_token` and `runner_log_sha256` are the #1214 round 4 binding: the token is a per-attempt identity generated at launch and carried in both this record and the `.launch.json`, and the digest is a rolling sha256 over the exact bytes the supervisor captured (taken at close, immune to later replacement). Its ABSENCE is honest ("not observed") and must not be promoted to success; but presence is also not sufficient — `_read_runner_exit` requires ALL of (1) the record's `runner_exit` is an integer of EXACT type `int` (#1214 round 3 / P1(a) — a JSON `true` is a Python `bool`, and `bool` subclasses `int`, so `isinstance` accepted it; a bool is not an exit code), (2) the record's `runner_log` MATCHES the launch record's path (a divergent path is a lying or stale witness), (3) the record's `runner_token` MATCHES the launch record's `runner_token` (#1214 round 4 — binds the witness to this launch so a stale exit record from a different attempt cannot read as this attempt's outcome), (4) the log file is reopened with `O_NOFOLLOW` (a symlink at the path is refused, not followed — #1214 round 4) and EXISTS with non-blank content, and (5) the reopened log's sha256 MATCHES the record's `runner_log_sha256` (#1214 round 4 — a stable snapshot of the captured bytes, so a sidecar or leftover writer that replaced the content produces a mismatch and falls through), and (6) `runner_exit == 0` (#1214 round 5 / P1(a) — the digest is accumulated over bytes-as-written so a SIGKILL'd prefix digests correctly; only a clean exit proves the reviewer finished, so a signalled or nonzero exit is an INCOMPLETE delivery, never terminal). "Non-blank" is a TEXT judgment (#1214 round 3 / P2(a)): the log is decoded as UTF-8 (lenient) and `str.strip` — Unicode-aware, unlike `bytes.strip` — decides, so a capture of only U+2003 is blank; an empty, missing, or whitespace-only log is the "present but empty capture" case the P1 was filed for — a reviewer that died before producing output leaves an exit integer but no recoverable verdict. A capture that fails any check falls through to the liveness probe and classifies `runner-absent`, not `terminal`; #1214 round 5 / P2: a rejected capture carries its REASON into the status detail ("capture rejected: <reason>") rather than the misleading "runner exit never observed", so a digest mismatch and a signalled exit point at different diagnostics. The pair is written beside the `.prompt.md`/`.launch.json` and shares their `<branch>-r<round>-<digest16>` stem; the `.runner.log` is RESERVED with `O_CREAT|O_EXCL|O_NOFOLLOW` by `_launch_detached` BEFORE the reviewer forks (#1214 round 5 / P1(b) — so an EEXIST is reported through the launch handshake and no reviewer runs, rather than silently swallowed after the fork); the reserved fd is passed to the supervisor (no reopen, no create→write TOCTOU window); the exit record uses a temp-file-then-rename so a torn write cannot read as a fake observation. A coordinator finds a completed review's output by reading the `.launch.json`'s `runner_log` field (the path is recorded in the launch record), and its outcome from the sibling `.runner.exit.json` | `test_dispatch_lane.py` |
