...
| `.dreamwork/docs/plans/` | Active feature plans, alphabetical (answers-page, artifact-templates, composer-row, daemon-mode, dreamhub-stage1, filebytes-range, goal-hierarchies, heartbeat-into-monitor, hub-public-auth, lan-bind, note-reply-threading-254, parallel-architecture, question-instruction-options, review-essential-marks, task-store-schema, task-transition-boundary, tasks-page, ud-dreamtask, ud-dreamwork-github, user-event-journal, user-event-journal-implementation, version-and-upgrade, watch-client-extraction) | Prune when features fully land; `lint.py` fails the row against the directory |
