...
- **A gate that runs only targeted pytest never sees a lint-only contract, and a piped exit code is not an exit code** (2026-07-30, briefs #545/#460). Two coordinator brief commits (454f297c, d0becdde) left master lint-red for ~50 minutes: the #398 obligation check is a substring match for the literal `.dreamwork/handoffs.md`, both briefs wrote "handoffs.md" bare, and the gates in that window ran `pytest -k handoffs` and `-k burndown or provenance` — targeted subsets that deselect `TestBriefHandoffObligation` entirely. A lane's worktree report surfaced it. Two procedure fixes: **every coordinator commit that is not itself gated by a full lint run gets `python3 -m pytest test_lint.py -q` before the push of the day ends — the brief-file class of contract is invisible to every targeted -k.** And **never gate a `&&` chain on `pytest … | tail` — the pipeline's exit code is tail's; the commit after it ran on a red suite.** (The red was real, not flaky: `test_this_repo_passes_its_own_linter` snapshots HEAD, and HEAD was genuinely red until 81b836df.)

- **A conflict resolver that greps for three marker forms misses the fourth** (2026-07-30, #548 merge gate). The repo's merge style is diff3, which writes FOUR markers: `<<<<<<<`, `|||||||` (base), `=======`, `>>>>>>>`. The coordinator's python resolver split on `<<<` / `===` / `>>>` only, so the `||||||| e2acedf5` base marker rode into the merged handoffs.md and was COMMITTED — and all 397 lint tests passed with it in place, because the fold grammar check never looked for markers at all (filed #554; the #508 gate hit the same class and its lesson stayed prose). Two rules: **after ANY hand resolution, grep for all four forms — `grep -nE '^(<{7}|>{7}|\|{7}|={7}$)' <file>` — before `git add`; the base marker is the one every ad-hoc resolver forgets because it only exists in diff3 style.** **CORRECTED 2026-07-31 (lane-673tick, measured): the `=` arm needs the `$` and the other three must not have it.** The three named markers are each followed by a branch or base name, so they cannot be anchored; `=======` stands alone. An unanchored `={7}` therefore matches any rule-of-equals divider of seven characters or more — `test_chain_golden.py` false-positives under it, and this entry shipped the unanchored form for a day. Verified both ways: a 9-equals divider matches the old regex and not the corrected one, while a bare `=======` matches both. The lesson inside the lesson is that **a regex published as a safety check is itself a check and needs the same both-directions proof as any other** — this one was only ever tested against text that should match. And the companion the repo already teaches: a lesson that does not become a check is a lesson waiting to be re-learned — the second occurrence is the signal to write the check, not the third.

- **A committed receipt is not evidence that a route did its job — a refusal commits one too** (2026-07-31, #586 lane, measured not argued). Every registered write route commits a receipt in `do_POST` regardless of the handler's outcome, and answers 202-on / 200-off either way. So for any route whose payload can be REFUSED, E2Shadow's two nets — the status-list assertion and the receipt-count assertion — both pass on the refusal path, and the route is exercised hollowly. The lane proved it by constructing the green red-run deliberately rather than reasoning about it: with `/chat-reply`'s chat id bogus and its post-condition deleted, all 15 tests passed. The fix is a BEHAVIOURAL post-condition — seed a real chat through the one production writer (`apply_chat_turn`, the same call the handler makes), assert the precondition, and assert afterwards that the reply text actually landed in the transcript. **Any new write route that can refuse needs the same, or its registration proves only that it was reachable.** Two companions worth carrying: the route's own suspect list is now filed (#594 — `/decide` deliberately reaches its `domain_invalid` refusal and asserts nothing behavioural, which is documented as intentional, exactly how the `/chat-reply` gap would also have looked); and the brief predicted the WRONG assertion would catch the missing-POST injection — the status-list assertion fires first, and no edit to `run_all_routes` alone can isolate the receipt count, so **a brief that names an expected failure site should also ask which assertion actually caught it.**

- **Folding is not merging, and the ledger cannot tell the difference** (2026-07-31, found by branch audit, not by any check). `#590`'s lane landed a 377-line backlog re-rank, appended its hand-off, and the coordinator folded it into the ledger citing the sha — the whole protocol executed correctly. The branch was never merged. So `tasks.md` said "landed `44cd9ca1`", `doc-map.md` carried a row for the document, and `.dreamwork/docs/backlog-rerank-2026-07-31.md` **did not exist on master**. Nothing was red: the fold cites a sha, and a sha in a hand-off is satisfied by a commit on ANY branch — `git cat-file` finds it, `git log --all --grep` finds it, and the ledger's own `sweep` examines commit subjects since the last fold commit, which a reachable-but-unmerged commit still supplies. This is the sibling of the same day's "merging is not shipping" (a verified fix sat unmerged into the deployed artefact for 11 minutes because the merge was mistaken for the deploy): **each step in land -> merge -> fold -> deploy has its own evidence, and satisfying one is never evidence for another.** The cheap standing check is a branch audit — for every branch, `git rev-list --count master..<branch>`; anything non-zero is either live work, a cherry-picked lane (verify the content is actually on master, do not assume), or a gap. Run it when folding a batch, because the ledger will never raise it. (Two other lanes' branches audited the same way that day were genuinely cherry-picked and their content verified present — so a non-zero count is a question, not a verdict.) **STATE-AT-DISCOVERY, and the entry needed this clause added the same day: it was FIXED on discovery — merged at `010f0093`, 2026-07-31 18:48 — so `44cd9ca1` is an ancestor of master today and the 377-line document is there.** A reader who checks now finds the claim false and reasonably concludes the lesson is wrong; lane-671sweep did exactly that, verified both facts, and reported the entry as a factual error in its brief. It was right about the present and wrong about the history, and the fault is the entry's for describing a repaired condition in a tense that reads as current. **So: a lesson about a defect that was fixed must name the fix, or its own evidence refutes it the moment someone checks** — which is the more general failure, because a lesson nobody can verify is one nobody will trust twice.

- **A red for the wrong reason is indistinguishable from a red for the right one in a `-q` summary** (2026-07-31, #659 lane, caught by accident). Constructing a false-green case, the lane sliced `read_text_full(path)[:N]` *before* the `if raw is None` check — so on an absent file the reader raised `TypeError` instead of truncating, and the test went red for a crash rather than for the defect under test. It looked like a successful red-proof. It was only caught because an unrelated test also failed and prompted a second look. **The pass/fail count is not the evidence; the failure MESSAGE is.** A red-proof is only discharged when the failure you observed is the DISCRIMINATING one — the assertion you predicted, failing the way you predicted — and the cheap mechanical version of that discipline is to grep the run for the expected `AssertionError` text rather than reading the summary line. This is the injection-side twin of the standing rule that a green red-run is a finding: a green red-run says the check cannot see the defect, and a red-for-the-wrong-reason says the same thing while looking like success, which makes it the more dangerous of the two. (Companion from the same day: the #653 lane's first red-proof of its reproducibility fix stayed green because `cwd=tmp` neutralised the injection, so the defect was never actually reintroduced — the injection must be proven to have landed before its red means anything.)

- **A tool that answers out of a store which did not resolve manufactures the confident wrong citation the brief exists to prevent** (2026-07-31, #667, measured by lane-659attractor). `python3 dev/ledger.py get <id>` run from a lane worktree returned `#NNN not found` for every id: `ledger.sqlite3` is gitignored (#294) so it cannot travel, and the `tasks.md` that does travel is the #458 migration shim. That message does not read as "you invoked the tool wrong" — it reads as "that task is not in the ledger", and the brief's OWN next sentence then routes the lane to `tasks.md.deprecated` ("not in the ledger does not mean not real") to cite a stale entry with confidence. Four required reads, four false not-founds, in one lane. The enumeration mattered more than the headline: `get`/`list`/`count` answered falsely and silently; `counts`/`fold`/`file`/`note` refused but blamed #440 prose corruption, an unrelated and alarming cause; `reviews`/`groom` named "markdown mode" but not a fix; and `sweep` printed **"nothing to review (this ran — see the examined count above)"** while examining zero ledger entries. So: **when a reader can be pointed at the wrong store, the refusal has to name the working invocation, not just the cause** — a lane handed only a cause re-runs the invocation that just failed. In a brief, spell the flag out with the MAIN checkout's absolute path (`--ledger <main-checkout>/.dreamwork/tasks.md`); "from the repo root" is read by a lane told to stay in its worktree as *its* root. This is #611's rule (*a check that examined nothing must not read as passing*) applied to a READER rather than a check, and it takes #611's shape too — one gate at the single dispatch point, never a near-identical refusal pasted into each verb.

- **A brief's evidence claim must name the assertion that would fail — three measured instances are one failure in different clothes, and the fourth is the one the brief did not name** (`#672`, from the #655 review). A brief exists to prevent false confidence; an evidence claim that cannot be wrong in a way anyone notices manufactures exactly that. (1) A commit *count*: the #655 brief said the lane was *"one commit ahead of master"* when it was one ahead of its branch point and **32 behind master**, and the merge ERRORed on `client/views.js` exactly as the count hid — fixed by `just build-client` in `6dd7127f`. Two shas cannot rot into a comforting number; that rule is in SKILL.md. (2) A *guard* named as evidence: both the author's brief and the coordinator's said *"run the status browser guard"*, but `dev/capture/status.mjs` has no assertion about the drain-depth feature and its fixture has no journal, so **PASS was guaranteed before the work started** — the author reported it as evidence and it was true and uninformative. This is the lesson *"A trace field that is collected but never asserted is a hollow check wearing a thorough header"* (the header's claim-list is not the assertion-list) applied to briefs. (3) A *"reuse X, do not reimplement it"* demand: the #655 brief forbade copying the drain's query into `watch.py` and never asked for a check that the drain's projection is what runs — the reuse was done correctly and was entirely unguarded, proven when a hand-rolled `SELECT COUNT(*)` without the kind filter passed all four of the author's tests while answering 4 where the drain lists 3. The unifying shape for all three: **the evidence a brief cites must be falsifiable — the sha pair that cannot drift, the assertion that would fail, or the check that binds the reuse.** The pattern catches the instance the brief never named (item 4 below is a different species and lives with it).
- **Tooling companion (fish), same family as the fish-glob entry at line 3098: `grep --include=*.py` fails under fish with *"no matches found"* — fish glob-expands the flag's value.** Quoting fixes it: `grep --include='*.py'`. Every lane here runs fish, so every lane wastes a call on it once. Landed here rather than in SKILL.md boilerplate because it is a shell gotcha, not a brief-evidence rule, and the established home for shell gotchas in this repo is this file (the coordinator, who writes grep commands into briefs, reads it; lanes do not — `#400`).

- **A check that disagrees with data you can see is a question about the PATH between them, and a claim about evidence is not evidence** (2026-07-31, `#696`/`#124`, and I got it wrong twice before measuring). `dev/ledger.py sweep` kept flagging `#124` as an entry git names a landing for that the entry does not cite. The entry *did* cite it — I had written the note myself at 20:32, and the note's own words were **"CITING THE MERGE SHA SO THE DELIBERATE OPEN STATE STOPS READING AS AN OVERSIGHT"**. It did not contain the sha. A sentence asserting that evidence is present read, to its author, as the evidence being present; this is the lesson *"A trace field that is collected but never asserted is a hollow check wearing a thorough header"* (the header's claim-list is not the assertion-list) turned on a ledger note instead of a check docstring, and the author is exactly who it fools. **When you write "citing X", paste X.** Then, having added the sha, sweep still flagged it — and I hypothesised twice (the suppression tests a full `%H` against a body carrying an 8-char abbreviation) and was about to file that as the cause. Both hypotheses were wrong: `dev/ledger.py:500` builds the list with `%h`, already abbreviated. **The measurement that settled it was one command** — print what the parser actually returns for that entry — and it should have been the first act, not the third. The real cause was in neither the check nor the data but in the projection between them: `ledger_parse.ledger_entries` (`:63-66`) ends an entry at the first column-0 line that is not an entry head (deliberately, so the prose summaries under `## Recently landed` never join an entry), while multi-paragraph bodies reach the store **unindented** — so the parse stops at the first prose paragraph. Measured across the live store: **584 tasks with a body, 66 truncate, 242,682 characters invisible**, the ids contiguous `#636`-`#695`. **CORRECTED BY SELF-REVIEW 40 minutes later, and the correction matters twice over.** I first illustrated the claim with `#636`/`#655`/`#671`/`#695`, asserting the parsed entry was "the head line alone" — but those are LANDED, so they are not in the `## Open` projection at all and I had verified nothing about them; the illustration was picked from a store-side scan without checking it reached the thing the checks read. Re-measured over OPEN entries, where it counts: **77 open entries retain under half their store body, 142,249 characters lost**, worst `#606` at 171 of 14,013 and `#630` at 145 of 9,867 — both genuinely reduced to the head line alone, so the claim was right about the mechanism and wrong about its evidence. And the re-measurement exposed the part I had missed entirely: for `#630` the column-0 prose sits IMMEDIATELY AFTER THE HEAD, so the parser stops there and the three correctly-indented `· ` note lines BELOW it are lost as well. **The loss is not the malformed line — it is everything after it, however well-formed.** Two consumers confirmed producing false results off the starved input (`sweep`, and `lint.py`'s `check_cited_shas`), and the scope was itself measured rather than assumed: of the seven entries `check_cited_shas` warns about, exactly **one** is truncation fallout and six genuinely do not cite their shas — so the fix clears one WARN, and claiming it clears the list would have been the same error one level up. The transferable rule is the first sentence: **a check and its input disagree in three places, not two — the check, the data, and the projection that carries one to the other — and the projection is the one nobody instruments.**

- **"By construction" is a claim about a mechanism — go and look at what constructs it** (2026-07-31, `#404`'s premise, five failure modes measured in one evening). `#404` makes `dev/ledger.py sweep` the loop's **primary landing-discovery route**, and its stated justification is that a lane cannot land work without committing and the commit convention puts the id in the subject *by construction*. Nothing constructs it. Measured, in the order they were found, each by a different investigation and each fixed locally: (1) `#686` — the lane committed nothing at all, so there was no subject to read; (2) `#693` — the lane wrote the id and `git`'s `commit.cleanup` **stripped** it, because a `#NNN` subject is a comment line under `strip`/`default`; (3) `#696` — the id was written and the **ledger projection truncated the entry** before the correlation could see it; (4) `#705` — no brief ever *instructed* a lane to write the id, absent from `briefs/boilerplate.md` entirely and only declarative at `SKILL.md:387`; (5) `#707` — the id **is** written, in the form the whole repo uses, and `SWEEP_SUBJECT` cannot match it: it is anchored and requires parens, so `Merge #688: …` and `#704: skip …` are both MISSED while `design(#700): …` matches. **Every merge commit this loop has ever made — the commits that record that a lane landed — is invisible to the tool whose job is discovering landings.** `#671` had already measured the scale (*1697 commits name an id in a form sweep cannot attribute*) and nothing acted on it, because a measurement filed against one task is not a question asked of the premise. The evidence is blunt: across five merges in one evening `sweep` said *"nothing to review"* every time, and the first landing it ever reported came after a commit deliberately written `fix(#707,#705):`. **The self-implicating half is the useful half.** I filed `#705` to close mode 4 and wrote a standing rule telling every lane to begin subjects with `#NNN:` — *citing sweep as the reason* — which is precisely the shape mode 5 makes invisible; three lanes were dispatched under it before a lane measured the rule against the actual regex and reported it. So the fix for one failure of the premise made the premise falser, and did so while quoting the premise. Two transferable rules. **First: when a justification contains "by construction", that is the place to look, because the phrase is doing the work the mechanism is supposed to do — it reads as proof and it is a promise.** Second, and this is the sibling of `#654`'s three-times-patched truncating default: **five separate investigations each found a mechanism and repaired it; not one asked whether the thing they were repairing had ever worked.** Each mode was cheap to find *once suspected*. What was expensive was that nothing prompted the suspicion, and a premise stated as self-evident is exactly what suppresses it.

- **Shell companion to the fish-glob entry (line 3098) and the `--include` entry above: a double-quoted `-m`/`--note` argument command-substitutes its backticks, and the tool stores what the shell handed it** (2026-07-31, coordinator, twice in forty minutes). Writing a ledger note containing `` `pending` `` under double quotes produced `zsh: command not found: pending` on stderr and stored the sentence with the word **missing**; the same mistake then ate `` `pending | tail -3` `` from a merge commit message. Both were accepted with a clean exit — the only signal was one stderr line scrolling past under the tool's own success output. This matters here more than in most repos because every ledger note and commit message this loop writes is dense with backticked code references, so the failure mode is not occasional, it is *correlated with writing carefully*. **Use a heredoc (`-F -` for commits, `<<'PY'` for python) or single quotes for any message containing backticks**, and note the shape: it is the same class as the entry it sits beside — a channel that silently drops content and reports success. The coordinator wrote a lesson about that class an hour before making the mistake for the second time.

- **A green reading is evidence only if you know what produced it — three times in one evening I credited the wrong mechanism** (2026-08-01, coordinator, all three self-caught but only one before it reached the ledger). The shape is constant: *the observation was real and the attribution was wrong*, so nothing looked like an error. (1) **A step recorded the success of a step that never ran.** `git merge --no-ff <branch> -F -` fails — `git merge` does not accept `-F -`, unlike `git commit` — but the same command block then ran `SHA=$(git rev-parse --short HEAD)`, captured the *unrelated* commit still at HEAD, and wrote a ledger note saying `#708` LANDED at `d9e2b7b5`, a commit whose subject is `docs(#683): …`. The `fold` ran too. The only thing that saved the work was `git branch -d` refusing to delete an unmerged branch — a safety I did not design and was not relying on. **A command that records an outcome must read the exit status of the command that produced it, not the state that happens to be lying around afterwards.** (2) **I asserted a false-green trap into a brief from the shape of a change, without reading the assertions.** `#713`'s brief warned its lane that substituting bare `consume` for `consume --through` would be green-and-wrong; the test in fact closes that path *twice* (`_drain`'s `cur == through` post-condition and the body's `expected == 1`), both failing loudly. `#586`'s trap is real; asserting it applies to a particular test without reading that test is how a brief teaches a lane something false — and a lane reads the brief as the coordinator's measurement. (3) **I confirmed a feature by watching my own input come back.** After merging `#716` (discovery for `status_sync`'s `dreamers`) I ran `just status-sync`, saw `already in sync (181 open, 4 live)` and four correct lanes in `status.json`, and reported discovery working end-to-end. I had **hand-written those four entries minutes earlier**. What I was watching was `live_lanes` correctly *pruning* my planted list — the half that already worked before `#716`. The new half contributed nothing: `discover_lanes(Path("."))` returns `[]`, because `wt_root` is built by string concatenation and `"./.worktrees/"` never prefix-matches the absolute path `readlink` returns, and `--target` defaults to `"."`, which is the invocation the loop runs every tick (filed `#720`). **The discriminating test for "does X populate Y" is to empty Y first** — not to look at Y and find it plausible. The unifying rule, and it is the cheap one: **for any green signal, name the mechanism you believe produced it, then ask what else could have.** In all three cases a single question would have done it — did the merge exit zero, what do the assertions say, is the field populated from nothing — and in all three the reason it went unasked is that the reading agreed with what I expected. This is the sibling of the same file's *a red for the wrong reason is indistinguishable from a red for the right one* (line 3306): that entry is about greens and reds being misattributed inside a test run; this one is about the same misattribution one level out, where the thing producing the green is not even the thing under test.

- **File disjointness is not interface disjointness, and `git merge` succeeding is evidence about TEXT** (2026-08-01, coordinator, and the lane I ignored had written the prediction down). Every brief this loop dispatches carries an off-limits list of FILES, and that discipline works: parallel lanes have not produced a merge conflict in dozens of dispatches. It prevented the wrong failure. `#606` landed `dev/guard_preflight.py`, which calls `status_sync.discover_lanes`; twenty minutes later `#675` landed `status_sync.py`, changing that function's return from a 2-tuple to a 3-tuple. The files were disjoint, so both merges were clean, so nothing was red — and the preflight's lane count silently degraded to `?` because a bare `except Exception: return None` caught the `ValueError` and rendered a hard interface break as a soft "I could not see" (`#136`). `#675`'s lane updated `status_sync.py:696` and all seven `test_status_sync.py` call sites, correctly and completely; it could not update a file that did not exist when its worktree was created. **The self-implicating part: `#606`'s lane named this exact hazard in its out-of-scope section, and I merged `#675` on top without reading it.** A lane's out-of-scope section is the one place it records what it could see and could not reach, which makes it the highest-value paragraph in the report and the one a coordinator in a hurry skips. Two rules, both cheap. **Before merging lane B, grep for callers of every symbol whose SIGNATURE it changed** — `grep -rn '<symbol>' *.py dev/*.py` costs one call and would have caught this in full; a clean merge tells you the texts do not overlap, never that the semantics compose. **And when parallel lanes touch modules that import each other, the second merge is the risky one even though the first is where the change lives** — the breakage always lands in the file nobody edited, which is exactly where nobody looks. (Companion, same evening, same family as the *name the mechanism that produced the green* entry above: I then found `dev/reaper.py`'s `is_dead_lane` using the same `cwd.endswith(" (deleted)")` predicate that had just mislabelled the coordinator's own process, and filed it P1 asserting the phantom list was a loaded gun — a reaper might act on it. I escalated on a RESEMBLANCE. The dry-run was one command away and refuted me: `parse_cmdline` requires a `watch.py` basename **and** a server flag, so the reaper only ever enumerates watch.py servers and structurally cannot see a `claude` process, on top of protecting self/init/never-kill/the deployed dashboard. Corrected to P2. **Check the CONSUMER before pricing the risk of a classifier** — a shared predicate is safe or dangerous according to what acts on it, and that is a fact you read, not one you infer from the predicate's shape.)

- **A brief that says "I ALREADY MEASURED" is an instruction not to check, and mine was wrong twice over** (2026-08-01, `#734`, caught by the lane I told not to look). I wrote into `#734`'s brief, under a heading reading *"I ALREADY MEASURED BOTH PROPERTIES — do not repeat this, build on it"*, that `reprioritise` and `unblock` refuse with **exit 0**. They exit 2 and always had. **The mechanical error:** I ran `python3 dev/ledger.py … 2>&1 | tail -3` and then `echo "exit=$?"` — and `$?` after a pipeline is the LAST command's status, so I measured `tail`, which succeeds unconditionally. **The second error, found while correcting the first:** re-run without the pipe, my fixture returned `rc=1` for `reprioritise`, `unblock`, and the `get` control alike — because a copied `tasks.md` in a temp dir is **not a linked worktree**, so the `#667` gate never fires and all three take the ordinary not-found path. My fixture never reproduced the condition I claimed to be measuring. Two measurements, two different causes, one confident brief. The lane's own sentence is the transferable rule: ***"The brief's 'I ALREADY MEASURED' framing made this harder to question, not easier. A measurement is only worth quoting if it names the commit it was taken against"*** — and mine named only "a throwaway copy of a worktree shim", with no sha and no timestamp. `git blame` on the two refusal paths settled it in seconds, which is the measure of how cheap the check was that my framing discouraged. **So: quote a measurement with the sha it was taken against and the fixture that produced it, or do not present it as settled** — and never pair it with "do not repeat this", which converts a helpful shortcut into an authority claim. The redeeming half, which the lane was fair enough to state: the task was still worth doing, because the gate genuinely did not check exit codes at all, so a future verb that refused correctly but exited 0 would have sailed through; the diagnosis was wrong and the hardening was real. **The pattern this belongs to is the evening's most useful finding and it is not about any one bug: three separate lanes corrected the coordinator's live filings in one session** — `#726`'s lstrip lane produced the audit that refuted a P1 I had just filed at the wrong severity, `#607`'s lane measured its own task's premise false (the boilerplate already satisfied it; the defect was in the brief heads I write), and `#734`'s lane refuted the measurement I had explicitly told it to skip. Each correction arrived because the lane looked at something adjacent to its assignment and said so. That is the direct argument for `#589` making the dogfood section standing rather than hoped-for, and it is also the argument against the brief voice I had drifted into: a coordinator who writes with too much authority gets lanes that implement the brief instead of the task.

- **A citation's blast radius is set by where it lives, not by how wrong it is** (2026-08-01, `#748`, found by `#739`'s lane in its dogfood section). `briefs/boilerplate.md:131` reads *"**Volume** (`#612`): land your change as the fewest lines that carry the meaning."* The principle is good. `#612` is *"The #381 fold-prompt WARN quotes the ENTIRE hand-off body verbatim, so one 4KB hand-off dominates the whole lint report"* — a P2 polish task about lint output volume, thematically adjacent and stating no scope rule at all. A single wrong id, in the one file `#400` established that **every lane physically reads**. So it self-replicated: `lane-710:233`, `lane-725:81`, `lane-733:106` and `lane-641:129` each quote `#612` for the principle in their own reports, and I repeated it in every brief I wrote that session — none of us resolved it, because a citation in the boilerplate reads as already-vetted. **The same typo in a single lane's brief costs one lane one lookup; in the boilerplate it becomes the loop's folklore.** It is `#671` one level up: a citation that points at nothing relevant still READS as authority, and the lane that follows it lands on an unrelated entry and learns that citations here are decorative — which is the expensive part, because the boilerplate's other citations are load-bearing. Two rules. **Resolve every id in any text that will be COPIED, and re-resolve when you inherit it** — the cost is one `get` per id, once, against a file that is read hundreds of times. **And prefer stating a principle in its own words over citing an id for it**: the sentence at `:131` is self-contained and needed no authority, so the citation added only a way to be wrong. (The finding also belongs to the pattern in the entry above — a fourth lane correcting a coordinator's live text from adjacent territory, and the second one produced by `#589`'s standing dogfood obligation rather than by luck. The obligation is paying.)

- **A lesson's ILLUSTRATION is not its authority, and the mix-up hardens into tool output where everyone reads it** (2026-08-01, `#752`, found by `lane-cx-737dogfood` as dogfood — the second citation-blast-radius finding in one night, after `#748`). Every brief I wrote cited *"`#590`: a non-zero count is a QUESTION, not a verdict."* `#590` is titled *"Re-rank the open backlog against his 2026-07-31 focus"* — a backlog re-ranking chore — and states no such principle; its nearest line says its own audit numbers "are a recommendation". **The phrase is real.** It lives at `lessons.md:3302`, the branch-audit lesson, and it is about `git rev-list --count master..<branch>`: a non-zero count means live work, a cherry-picked lane, or a gap. `#590`'s lane appears in that lesson **as its example** — its branch was the unmerged one. So the citation named the specimen and not the finding, which is a subtler failure than `#748`'s (a plain wrong id) because **the entry it points at is genuinely present in the lesson's text** — it just is not the thing that says the thing. The propagation chain is the part worth keeping, because it shows the hardening: (1) `lessons.md:3302` states the rule and uses `#590` to illustrate it; (2) `#676`'s body writes *"`#590`'s rule applies: a non-zero count is a QUESTION, not a verdict"* — reasonable shorthand for "the rule whose example is `#590`", which reads as attribution; (3) `dev/ledger.py:1016` now **prints** `a + is a question, not a verdict (#590, #676)` on every `reach`, i.e. on every fold, so the coordinator re-reads the miscitation nightly; (4) my briefs copied it from the tool. **A citation that reaches PRINTED TOOL OUTPUT is worse than one in a body**, because it is re-read on a schedule and arrives with the authority of the tool rather than of an author. Note the asymmetry that makes the diagnosis precise: `#676` is *defensible* (its body literally contains the sentence, and it is the task that applied the rule) while `#590` is not — so "both cited ids are wrong" would itself have been a widening (`#707`). The rule: **cite the text that carries the evidence — `lessons.md:<line>` — not the task that happens to appear inside it**; and when a lesson is stated with an example, the example's id is the last thing to reach for. A lane sent to `#590` finds a backlog re-rank and learns that citations here are decorative, which is the expensive part.
...
- **A citation must carry its own evidence, and a line number carries none — the fix
  ...
  The filing-time blast radius was measured, not estimated: four lane reports, two merge
  commit messages, most of a night's briefs, and — the part that matters — **three entries
  inside `lessons.md` itself**, then recorded as `#505`, `#672` and `#696`. Re-measurement
  at implementation found that `#505` no longer carried it, while this lesson's specimen
  had become the third literal occurrence; the live authority citations were `#672` and
  `#696`. A wrong citation inside the lessons file is re-copied by everyone who reads it
  correctly.
...
- **A guard's message must name a mode the guard can actually detect, and the way to know is to construct that mode and watch it fail.** `#596`'s route-table extractor guarded its brace walk with `self.assertGreater(j, ob, "TITLES table has no closing brace")`. The sentence looks careful, but the arithmetic says the opposite: on an unterminated table the walk runs to `j == len(router)`, which is still greater than the opening-brace offset `ob`, so the assertion **passes in exactly the mode its message names**. A runnable reconstruction against `"const TITLES = {"` confirmed the unterminated input passed. This is the assertion-sized form of red-proof: do not review a guard by reading its message; construct the named failure and require that exact guard to go red. The failure it prevents is worse than an absent guard because the explanatory text lends authority to decoration. I measured three possible static proxies across 129 tracked Python files before adding one: ordering assertions with an absolute message word (`no`, `never`, `missing`, `empty`, `unterminated`) matched 12 sites, direct length/end-bound comparisons matched 22, and a broad message-identifier mismatch matched 52. The first two sets were legitimate guards, while the third was dominated by prose-vs-expression false attribution; narrowing to the historical spelling would miss the same defect with `"brace scan finished"`. So no mechanical check landed: *"the message names a mode the assertion cannot detect"* is semantic, and a noisy proxy would teach reviewers to ignore the report. The durable rule is the one-run experiment: **make the mode named by the message happen, watch the guard itself fail, and treat a green run as proof that either the guard or the experiment is wrong — never as coverage.** Evidence: `#651`, measured 2026-08-01; the exact bad assertion passed on unterminated input, the absolute-word proxy named it, and the same decoration with a non-absolute message passed outside the proxy.

- **Dispatch structure can make a base true, but word lists cannot make prose consistent.**
  `#763` measured the live brief corpus before choosing a coordinator check: none of 218
  briefs carried a 40-hex base sha, while a broad conditional-word proxy named all 218 and
  an imperative-word proxy named 211. A generated base can be the same sha used to create a
  worktree and is therefore true by construction; a linter that merely finds a hex string
  can prove at most that it resolves, never that it was the branch point. Self-contradiction
  is more semantic still: a proxy noisy on nearly every healthy brief teaches the coordinator
...
- **What a subprocess received is its argv, not the file you meant to hand it — and a dispatch carrying no rules at all looks exactly like a healthy one** (2026-08-01, `#768`, mine, caught by rereading my own quoting rather than by any instrument). I dispatched a lane with `ccc -y @agent "\$(cat prompt.txt)"`. The backslash escaped the substitution, so the runner received the literal 24 characters `$(cat /tmp/.../p766.txt)` as its **entire** prompt: no *never merge, never push*, no *no `attn`*, no live-ledger prohibition, no port restrictions, no thread limit. A model handed a shell fragment does not decline to act — it improvises. It ran unconstrained for about a minute. No damage, but that was luck plus a short window, and I verified it rather than assuming: master unmoved, worktree clean, ledger counts intact, nothing pushed. **Every instrument said normal.** The dispatch exited 0, the process started, the runner count incremented, `status.json` listed a healthy lane — which is `#136` at the one place it is most expensive, because the failure is invisible precisely when the constraints that would have contained it are the thing that went missing. Two transferable rules. First, **verify the delivered prompt, not the intended one**: `/proc/<pid>/cmdline` is the only authoritative record of what the runner actually got, and one `grep` for a standing rule against the string you are about to pass costs nothing (`grep -c 'Never merge, never push'` before launch). Second, **the strong half of the fix is on the lane, not the coordinator**: a lane whose prompt does not contain its rules should REFUSE, because that inverts the trust — the constraint stops depending on the coordinator getting shell quoting right on every single dispatch, forever. The bug itself came from mixing an interpolating heredoc (wanted, to inject a base sha) with a command substitution in the same block, then over-escaping defensively against the very interpolation I had just enabled; the fix is a quoted heredoc with a `__PLACEHOLDER__` substituted afterwards. And note the timing: this arrived minutes after a lane correctly refused to build a coordinator-side dispatch guard on measured grounds. That refusal was right about the guard it examined and this is not a counterexample to it — but *something* on this path was needed, and the incident is what says which half.

- **A check that fires on a healthy input is worse than no check — its message names a failure on every run, so the reader learns to dismiss the failure it exists to catch** (2026-08-01, `#786`). This principle has been applied correctly all session — #763's first refusal rested on it, and every dispatch-time check candidate that would red on ordinary brief prose was rejected on exactly these grounds. It was UNHOMED: I had been citing it as "#755 — a check that fires on a healthy input" in 19 ephemeral session prompts, but #755's real subject is queued_dispatches rot, and the phrase appears nowhere in the ledger or this file. A lane (`cx-783cover`) checked the citation instead of taking it, reported the mismatch, and #786 was filed. **This entry is its HOME** (option 2 of #786's remedy: write the lesson so the citation becomes true). The evidence: #763 measured that a broad conditional-word proxy names all 218 briefs and an imperative-word proxy names 211 — a check noisy on nearly every healthy brief teaches the coordinator to ignore it, and `lint.py`'s `check_placeholder_citations` (which fires two WARNs on the healthy live file) is the documented instance. The companion is the entry below: a signal that always fires is one nobody reads.
- **A signal that always fires is one nobody reads — a WARN that appears on every healthy run is decoration, not a warning** (2026-08-01, `#786`, companion to the entry above). This principle was cited as "#136 — a signal that always fires is one nobody reads" in 30 ephemeral prompts, but #136's real subject is *"A questions.md that parses to nothing must say so"*. #136's entry does carry a RELATED idea (an exemption that blesses the morning failure), but the specific "always fires → nobody reads" wording is the broader principle and had no home. **This entry is its HOME.** The evidence is `#766`'s finding (`untracked=1` printed on every reap, read as routine every time, never as the warning it was) and `#755`'s instance (a check that fires two warnings on the healthy file). The durable form: a warning that cannot be quieted by compliance is a warning that will be muted, and a muted warning is worse than none — so design the check to be SILENT on the healthy path and LOUD only on the fault, or it trains the reader to skip the row that matters.
