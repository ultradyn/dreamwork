# BRIEF — #763: the coordinator half, whose blocker is now gone and whose measurement has moved

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-763coord
Branch: cx-763coord
Base sha: c109ed8e
...
Lane-owns: dev/dispatch_lane.py, test_dispatch_lane.py, and a coordinator-checklist file if you
create one. `briefs/boilerplate.md` ONLY if your mechanism requires it — another lane may touch its
always-run paragraph this tick, so keep your edits elsewhere in the file.

**Task:** `python3 dev/ledger.py get 763 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` —
read the entry and **all** its notes. There are eleven recorded instances and they are the
specification.

## The lane half is landed. This is the coordinator half, and it was REFUSED once.

`briefs/boilerplate.md` already tells lanes what to do when a brief and the standing contract
disagree (merged `1d21a3be`). That half is structural and done. **This half is about stopping me
shipping the disagreement in the first place.**

It was refused before, on two stated grounds. **Both have since expired, and that is why it is back:**

1. *"The brief is never persisted at all — you cannot check what you do not keep."* **#766 landed.**
   `dev/dispatch_lane.py` now writes every brief to `.dreamwork/docs/briefs/<id>-<lane>.md` with a
   `.sha256` integrity receipt, and refuses a receiptless file.
2. *"Every proxy produces #755 false reds — 0/218 briefs carry a 40-hex sha; the broad proxy names
   218/218."* **Re-measured by me at `8a96783c` on 2026-08-01:** 239 briefs total, 20 carrying an
   explicit `Base sha:` line. Naively that is 219 false reds — the old refusal exactly. **Scoped to
   governed briefs** (`INTEGRITY_START_TASK = 766`, already shipped in `dispatch_lane.py`) it is
   **13 briefs, 12 carrying a base sha, and the single miss is `766-cx-766b.md`** — the brief that
   established the watermark, i.e. the boundary case.

**Re-derive both numbers yourself before building on them.** I am telling you how I measured
(count briefs under `.dreamwork/docs/briefs/`, grep for `Base sha:`, partition on the watermark) so
you can check the method, not just inherit the answer. This matters more than usual: **one hour ago
a brief of mine asserted a bisect result as established and instructed the lane not to re-derive it,
and the result was wrong.** That is instance eleven, below.

## The eleven instances are three species, and the third is new today

- **1–3** — the brief contradicts an **external** authority (the standing contract) or carries a
  rotting fact (a commit distance; an absent base sha).
- **4, 7** — the brief contradicts **itself**: a conditional deliverable stated as an unconditional
  imperative, then softened three paragraphs later.
- **5–6** — the brief's **lists are derived from the wrong source**: a free-list fenced by what other
  lanes own rather than by what the task needs; a verification list that omits the caller-fallout set,
  so the lane's bar cannot predict the coordinator's gate.
- **8, 11** — the brief asserts a **fact about the world** as a requirement. #8 required a lane to
  reproduce a phenomenon I had never run (it did not reproduce). **#11, from today:** my #781 brief
  said *"the bisect is already done; do not redo it"* and named a first-bad revision that the
  evidence clears. The lane caught it **only** because the standing direction-2 requirement made it
  check the healthy tree anyway. Its dogfood names the counterfactual exactly: a lane that obeyed
  that line would have patched the wrong seam and never known.

The rule instance 11 implies, which I have adopted by hand and want made structural: **a brief may
state a measurement, but must never instruct a lane NOT to re-derive one** — and anything presented
as established carries *how it was measured*, so the lane can audit the method rather than trust the
number.

## What to build — and the trap that killed the first attempt

The obvious mechanical form is a **dispatch-time check inside `dev/dispatch_lane.py`**: it already
validates the standing contract (`validate_prompt(prompt, contract, coordinator_inbox)`), already
refuses `rc=2` when the boilerplate is missing or altered, and already knows the watermark. Adding a
base-sha requirement there is a small edit to a proven seam, and — unlike a lint — it fires **before**
the lane starts, which was the previous attempt's other objection.

**The trap:** the refusal was right the first time. A check that fires on healthy briefs is worse
than no check (#755), and the 218-false-red measurement is what made it right. Your job is to
confirm the watermark scoping genuinely removes that, **or to find that it does not and say so**.
A second refusal with fresh evidence is an acceptable and valuable outcome — do not build something
weak to avoid returning empty-handed.

Judge the candidate forms with IGC — `/home/xertrov/.claude-p/skills/ud-dreamwork/igc-method.md`.
The entry itself suggests the answer is probably **both** a lane-facing rule (landed) and a
coordinator-facing checklist, since the two audiences read different files.

**Scope:** the base-sha check is the one with a clean measurement behind it. The other ten instances
are largely judgement failures that no dispatch-time check can catch — **do not try to mechanise
"the brief contradicts itself"**, that is a semantic detector with #707's false-positive problem.
If a checklist is the only honest home for those, write the checklist and say why the check stops
where it does.

## Red-proof

- **Direction 1:** a brief missing its base sha must be refused, naming that specific defect — not a
  generic validation failure. Break the check and watch the test red on the discriminating message.
- **Direction 2 — mandatory here, because the first attempt died on exactly this:** run your check
  across the **whole existing brief corpus** and report how many healthy briefs it fires on. If the
  answer is not "the explicable boundary case only", the check is not ready. The previous attempt
  also constructed the shape-vs-resolution false-green — 40 `f`s passes a shape test and resolves to
  no commit; a real sha passes both shape and resolution while not being the branch point. **Reuse
  that construction**; a check that only validates the *shape* of a sha proves nothing about whether
  it is the base.
...
## Handle `dispatch_lane.py` carefully — it is the loop's only dispatch route

It ends in `os.execvp(runner[0], [*runner, prompt])` (`:393`), executing from the **main checkout**;
the worktree is communicated to the lane only through the brief text. A change that makes dispatch
refuse wrongly does not degrade a feature — it stops the loop launching lanes at all (#755). Three
other lanes are live right now and I will be dispatching more against this file's *merged* form, so
prove the healthy path still dispatches, not merely that the unhealthy one refuses.

## Lessons selected for this task

- **#755 — a check that fires on a healthy input is worse than no check.** The refusal that killed
  attempt one; your direction-2 requirement.
- **#764 — a citation must carry its own evidence.** The parent principle: a brief's claims must
  come with what backs them.
- **#671 — a check that examined nothing must not read as passing.** A validator that skips
  historical briefs must say it skipped them.
- **#707 — every widening multiplies false attribution.** Why the other ten instances are not
  mechanisable.
- **#440 — one supported way.** If the base sha becomes required, there must be one way to state it.

## Run these, explicitly

```
just pytest test_dispatch_lane.py
...
- **Three other lanes are live.** `cx-783cover` owns `dev/capture/*` and is binding the guard ports —
  do not run browser guards. `glm-782catch` owns `dreamwork_db/core.py` and will be *running*
  `test_dispatch_lane.py` against the degraded path — do not be surprised, and do not coordinate
  with it directly; route anything through me. `glm-780registry` owns `dev/repo_wide_guards.py` and
  may touch `briefs/boilerplate.md`'s always-run paragraph.
