# BRIEF — #772: what a persisted brief must contain, now that it finally lands in the corpus

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-772hdr
Branch: cx-772hdr
Base sha: 911b6ab7
...
Lane-owns: dev/dispatch_lane.py, test_dispatch_lane.py, lint.py, test_lint.py, briefs/boilerplate.md


**Task:** `python3 dev/ledger.py get 772 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry in full. It carries the collision, the measurement, and a second finding about the check itself that is probably the more interesting half.

**This was held behind `#770` on purpose and `#770` has now landed.** Briefs persist to the main checkout's corpus again, resolved via `git rev-parse --path-format=absolute --git-common-dir`. So the artifact reliably reaches the corpus it is checked against, and the question this task exists to answer is finally answerable.

## The collision

A brief persisted by `#766`'s route carries my dispatch header, which names `.worktrees/<lane>`. That puts it in `#405`'s scope: **a worktree-naming brief must give an ABSOLUTE `.../inbox.md` path**, so a lane cannot be told a relative inbox and report into its own copy where the coordinator never reads it. When I committed five such briefs I got five ERRORs.

**`#766` could not have anticipated this**: its only artifact was a bootstrap reconstruction whose header was reduced to a bare `Branch:` line, so it never contained `.worktrees/` and never entered `#405`'s scope. The narrowing I flagged as a defect at that gate is also what hid this.

**The artifact cannot be edited to fix it.** It is byte-exact with a sha256 receipt taken at dispatch; adding a line breaks the receipt, which is the integrity mechanism `#766` exists to provide. So the fix belongs in the dispatch header — mine, and cheap to change — and cannot be retrofitted to briefs already sent.

## What I have already changed, so you do not redo it

Every header I dispatched today carries an absolute inbox line. It works: lanes appended to the main checkout's inbox, not their own copies. **But it introduced a defect of its own, which is now part of your scope.**

My header says *"append your **hand-off line**"* to `inbox.md`. The standing contract says *"DO NOT append a **hand-off line** to `.dreamwork/handoffs.md`"*. Same phrase, two files. Today `#645`'s lane appended and `#771`'s lane refused — and `#771` was right to refuse under the contract's ambiguity rule, and said so. **One instruction, two correct-but-opposite readings, is `#440` violated in my own header.** Fix the wording so it cannot be read as the forbidden act, and check whether the contract's prohibition should name its file more loudly.

## The second finding, and it may be the real one

`check_brief_worktree_abs_inbox`'s docstring says *"Untracked briefs are skipped (mid-write)"*. **That is not what the code does**, measured:

- five briefs present and untracked → lint clean at 1 warning;
- committed → 5 ERRORs;
- commit reverted, files untracked again → **still 5 ERRORs**;
- files removed from the corpus directory → green.

`git ls-files` returned nothing for them at both the green and the red readings. What changed is HISTORY: the add-commit survives the revert, so **the scope classifier reads git while the content check reads the disk**, and the two disagree after a commit-then-revert.

**Decide whether that split is a bug or intended, and say which** — do not assume it follows from the header fix. A coordinator who trusts that docstring does what I did: runs lint before committing, reads clean, commits, and goes red at the gate. That is a pre-commit green that is not evidence, which is `#671`'s family.

## Direction 2

Whatever the header gains, construct the brief that **satisfies the check and still fails the purpose**: an absolute path ending in `inbox.md` that points somewhere no coordinator reads — another worktree, a stale path, `/tmp`. `#405`'s regex accepts a leading `/` and a basename ending in `inbox.md`, so it will pass. Show what your answer decides, and if it cannot tell a real inbox from a well-formed fake, say so plainly rather than letting the check's name imply more than it proves (`#651`).

Second: if you make the header satisfy `#405` **by construction**, then the check can never fire on a real dispatch again. Say what still makes it worth running — a check that cannot fail is `#671` wearing a different hat, and the honest answer might be that its value is now only against hand-written briefs.

## Constraints

- **`#405`, `#440`, `#671`, `#136`, `#702`, `#755`** as usual.
- **Do NOT break the receipt.** The persisted brief is byte-exact by design; anything you change must change what is dispatched, not what is stored afterwards.
- **Do NOT rewrite historical briefs.** The already-sent ones cannot be retrofitted and must not be edited to make a check pass — that would be falsifying the audit record `#766` exists to create.
- `dev/dispatch_lane.py` just changed under `#770`; read it as it is now rather than as any note describes it.
- Do NOT rewrite other lanes' reports.
...
- `just pytest test_lint.py test_dispatch_lane.py <anything else you touched>` — the one supported form. **Do not run the bare full suite**; I own the single merged-tree sweep.
- **Exercise the real route and commit a persisted brief in a scratch clone** to show whether `#405` now passes by construction. This task exists because the failure was only visible at a merge gate; a report that reasons about the check instead of running it repeats that.
- `python3 dev/redproof.py` for injections; snapshot the FIXED file lane-privately via `dev/lane_scratch.py snap`, restore by `cp`, verify with `cmp` — never `git checkout` (`#349`, `#608`). Leave the registry clean and **do not leave a committed injection in your branch history**.
- No browser guards, no port binding, nothing near `:35110` / `:35113`.

## Report

`.dreamwork/lane-772-report.md`, with a DOGFOOD REPORT section. Lead with whether `#405`'s scope/content split is a bug or intended, and your evidence — that is the finding with reach beyond this one check.
