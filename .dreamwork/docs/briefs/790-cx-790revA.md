# BRIEF — #790 REVIEW LENS A: correctness and regression risk across this tick's six merges

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-790revA
Branch: cx-790revA
Base sha: beddf975
...
Lane-owns: NOTHING. **This is a read-only review lane.** Write no production code, change no
tests. Your deliverable is a report appended to the coordinator inbox. If you find something worth
fixing, describe it precisely enough that I can dispatch a fix — do not fix it.

## What to review

Six merges landed in the last hour, each reviewed by me once and gated together. A second
independent pass is exactly the check my single pass cannot be.

```
beddf975 #784  judge a browser-guard revision, or refuse to
8d60bbae #670  MCP browser's shared screenshot root
5bba38bf #786  home two unhomed principles + citation audit tool
f8515ec1 #785  advisory keys on available memory, not swap-used
8e3bfc10 #701  refuse a piped stdout that can SIGPIPE the runner
d45d964f #689  record which libc statx resolved through
```

`git show <sha>` for each; `git diff dc739001..beddf975` for the whole set.

## Your lens: does the code do what the merge message claims, and what does it break?

Rank by **consequence**, not by count. Specifically hunt:

1. **`#701` is the highest-stakes change in the set.** `dev/dispatch_lane.py` ends in `os.execvp` on
   the loop's **only** dispatch route. `validate_stdout()` now refuses a FIFO stdout. I verified
   three healthy shapes launch (regular-file redirect, tty, bare). **Find a fourth shape I did not
   test.** A wrong refusal does not degrade a feature — it stops every lane launching. Consider:
   stdout as a socket, a closed fd, a character device, `/dev/null`, a tty under `script`, a
   redirect to a file on a full disk.
2. **`#689` widened nothing but shifted 12 lines**, which rotted 52 line-pinned citation anchors and
   reddened the gate (owned by `cx-789anchor`, do not touch). Ask whether anything ELSE in the repo
   pins `watch.py` line numbers and is not covered by that test.
3. **`#785`** changed a trigger from swap-used to `MemAvailable < 8 GiB`. Ask what happens on a
   machine where `MemAvailable` is absent or nonsensical, and whether the advisory can now be silent
   in a state that genuinely warrants noise.
4. **`#786`** added `dev/citation_audit.py`. It resolves ids through the store. Ask what it does when
   the store is locked, absent, or mid-migration.
5. **`#670`** and **`#784`** add tooling that shells out and creates worktrees. Ask about cleanup on
   failure paths.

## The standard I hold

A finding must carry **a concrete failure scenario**: inputs or state → wrong output or crash. "This
could be fragile" is not a finding. If you cannot construct the failure, say so and downgrade it —
**reporting an unconfirmed suspicion as confirmed is worse than not reporting it** (#651, #702).

Verify before you report: run the code, construct the input. A red under load is not a finding until
you have re-run it quiet — this box runs many lanes and **#666/#606/#471 measured that guards return
WRONG answers under load.** My standing rule: *a green under load I accept; a red under load I do not.*
