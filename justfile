...
guards port="39899":
    #!/usr/bin/env bash
    set -uo pipefail
    DEFAULT_GUARDS="headertravel reflow qacard docktarget noteprop oneinput regroup popbg typing wisp states dismiss confirmation thread status health pushhealth dashboard identity motion morph morphhold prominence qsec submitlog indicator draft subslog history plugcmd qorder revieworder reviewsplit serving gitrow burndown provenance answers runmode hfit"
