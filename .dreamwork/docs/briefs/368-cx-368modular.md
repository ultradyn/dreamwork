# BRIEF — #368: make "modular" and "fast to start" into numbers before refactoring anything

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-368modular
Branch: cx-368modular
Base sha: beddf975
...
Lane-owns: `.dreamwork/docs/plans/modularity-and-startup.md` (create it), a startup-benchmark tool
under `dev/` (your naming), and its tests. **NOT `watch.py`** (`glm-646policy` owns it this tick).
**Do NOT begin the refactor itself** — see the scope gate below.

**Task:** `python3 dev/ledger.py get 368` and `get 124` — read both and ALL notes. They are the same
subject from two directions and **Max owes a decision between them that I have been unable to put to
him well.** Your real deliverable is to make that decision *askable*.

## His three asks, and why the third is the one to build

Verbatim (2026-07-28 02:46, via the dashboard):

> *"after the cli, we should refactor the large python files into a proper modular codebase that's
> reusable and more easily tested. also, re cli, we can write a core in python that we can later
> replace with something written in a faster-to-start compiled language. We can measure how fast it
> is to start too and like return 'version' as a benchmark."*

The ledger note already records the insight: **the third ask makes the first two checkable.** A
startup benchmark turns "modular" and "fast to start" from taste into numbers. So:

**BUILD:** the startup benchmark. `--version`-style entry, measured properly (many runs,
distribution not a single sample, cold vs warm, and state what you controlled for). This is real code
and it is the part that can land now without a ruling.

**DO NOT BUILD:** the refactor. `watch.py` is ~6270 lines and the client extraction has already moved
the CSS/JS out to `client/`, so the premise "watch.py is a 15k-line monolith" is **stale** — verify
its true size and composition yourself and report the corrected number. A refactor started on a stale
premise is worse than none.

## What to write instead of the refactor

An options document that makes Max's decision concrete: what the module boundaries would be, what
each buys, what each costs, and **what the benchmark says about the startup claim** — with a
recommendation. `#645` is migrating questions to the DB in a parallel lane and `dreamwork_db/` is the
one already-modular seam; read it, because it is the existence proof of the pattern being argued for.

**Judge rival boundaries with IGC** — worktree-local `./igc-method.md`.

## Red-proof

- **Direction 1:** the benchmark must fail loudly when it measures nothing — break the timed path and
  show it reds rather than reporting a suspiciously fast number.
- **Direction 2, the one that matters for a benchmark:** construct the case where it reports a
  plausible number that is **wrong** — a warm page cache read as cold, an import cached between runs,
  a measurement dominated by interpreter startup rather than by the code under test. A benchmark that
  cannot distinguish these is decoration. Name what yours does not control for (#702).
