# BRIEF — #800: the screenshot helper answers about the wrong process

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-800root
Branch: cx-800root
Base sha: 7ecb732a
...
Lane-owns: `dev/mcp_screenshot_root.py`, `test_mcp_screenshot_root.py`,
`briefs/boilerplate.md` (the MCP-screenshot paragraph ONLY, if your fix changes what lanes must run)

**Task:** `python3 dev/ledger.py get 800 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`

## Measured against the live server today

#670 landed this helper to tell a lane where the Playwright MCP browser will write screenshots. Its
own central finding is that **the MCP server is ONE PER SESSION**, launched by the harness, with its
output directory derived from `process.cwd()` **at launch time**.

The helper does not do that. It reports the **caller's** cwd:

- Live server: PID 1328852, cwd `/home/xertrov/.llm-general/skills/ud-dreamwork` (the main checkout).
- Helper invoked from a lane: reported `.../.worktrees/cx-790revA/.playwright-mcp` — and described
  that lane's own worktree as *"a stranger's tree"*.

**State → wrong output:** a lane runs the default command the boilerplate tells it to run, takes a
screenshot, copies from the reported path, gets no file. The screenshot was in the main checkout the
whole time. The helper answers *"where would MY cwd put screenshots"* when the question is
*"where will the SHARED server put them"*.

Supplying the coordinator cwd explicitly gives the right answer, but nothing tells a lane to do that.

**Re-derive this before fixing.** Find the live `@playwright/mcp` process and read its actual cwd
(`/proc/<pid>/cwd`); run the helper from your worktree; compare. If it does not reproduce, that is a
finding and I want it more than a fix.

## Also: it claims a fallback it does not implement

Its docstring says it mirrors upstream's `outputDir()`, which falls back to `tmpdir()` when the cwd is
a system directory or not writable. The helper omits that, so it can name a path the server would
never use — a message naming behaviour it does not have (#651).

## The honesty requirement

The helper's value is telling a lane the truth about a process it does not own. If the server's cwd
cannot be determined reliably (no such process, several candidates, unreadable `/proc`), it must
**say so** rather than falling back to the caller's cwd and presenting it with the same confidence —
that fallback is what produced this bug. `#611`/`#702`: absence must not read as an answer.

## Red-proof

- **Direction 1:** break the resolution and watch it red naming that the reported root is not the
  server's, not merely that a path differed.
- **Direction 2, mandatory:** construct the case where the helper looks right and is wrong — the
  obvious one is a lane whose cwd *happens to equal* the server's, so caller-cwd and server-cwd
  coincide and a broken resolver is indistinguishable from a working one. Your fixture must not have
  that property, or it passes vacuously (that is the exact shape that let #670 through).
## Standing bars
