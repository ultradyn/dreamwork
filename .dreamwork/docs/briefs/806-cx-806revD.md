# BRIEF — #806 REVIEW LENS D: what breaks only because two changes landed the same day?

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-806revD
Branch: cx-806revD
Base sha: aadf579f
...
**Task:** `python3 dev/ledger.py get 806`.

## Your lens: CROSS-CHANGE INTERACTION — and nothing has ever run this one here

Sixteen changes merged into master today. **Each was gated individually, against the tree as it stood
at its own merge.** No pass has asked what breaks *only because two of them landed together*. That is
your entire subject, and it is a class my gate structurally cannot catch: every change is correct
alone, so every individual gate is green, and the composite defect is invisible to all of them.

**A worked example from today, so you know the shape is real and not theoretical:**

`#689` inserted 12 lines at `watch.py:3353`. Alone: correct, gated, green. But
`test_reanchor_citations.py` pinned 74 `(path, line, symbol)` anchors, and **52 of them silently
stopped pointing at their evidence.** The citations still *read* fine. Nothing about `#689`'s diff
mentions them. It took a red merge gate to surface it, and `#789` then fixed the anchor *form* so
lines derive from symbols. That is one instance. **Today's sixteen merges are a much larger surface,
and `#646` is editing `watch.py` again right now.**

Sub-classes to hunt deliberately:

1. **Line-pinned or offset-pinned references across a file another change edited.** `#801` is already
   filed for 106 more rotted `watch.py` citations across 34 files — do not re-report those. Ask
   instead: *what else is pinned by position rather than by identity?* Fixture line numbers, `sed`
   ranges, AST line-span resolution (`dev/styleguide_audit.py` resolves constant spans this way),
   slice indices into a parsed file.
2. **Two changes that each altered the same contract from different sides.** A writer changed and a
   reader changed, both correctly, but for different versions of the format. `#645` collapsed two
   rival `StoreSpec` builders into one composer the same day several callers moved.
3. **A test fixture whose realism one change quietly destroyed.** A fixture built to look like
   production drifts when production moves; the test stays green and stops meaning anything. This is
   the highest-value sub-class because it produces **silent** false greens, and it is exactly what
   `#796` found one file over (equal tracked/on-disk counts made a broken check indistinguishable
   from a working one).
4. **Ordering assumptions between changes.** Something that works because A merged before B and would
   not if the order reversed — a latent trap for any rebase or revert.
5. **A guard registry, doc-map, or list that one change added a member to and another change's
   count-based check now reads differently.** `#794` landed *today* on exactly this: a count fell
   while a new warning appeared.

**Method note:** do not read the sixteen diffs in isolation and then try to imagine interactions. Work
the other way — pick the *shared surfaces* first (`watch.py`, `lint.py`, `briefs/boilerplate.md`,
`dev/citation_audit.py`, `.dreamwork/docs/doc-map.md`, the guard registry, the test fixtures), list
which of today's merges touched each, and for every surface with two or more touchers, ask what the
second one assumed about the first. That ordering is the difference between a lens and a re-read.

## What is NOT your lens

Not single-change correctness — lens A did that and found four defects; you will only re-find them.
If a change is wrong *on its own*, that is out of scope: note it in one line at the end under
"outside lens" and move on.

## Lessons selected for this task

Cite these by **content**, not my wording — verify each before relying on it.

- **`#764`** — the fix for miscited task ids was to cite `lessons.md:<line>`, and line citations rot
  too. The parent principle of your sub-class 1.
- **`#671`** — a check that examined nothing must not read as passing. Sub-class 3's failure mode.
...
- **`#611`** — a check whose absence reads as a pass. (Verify: this line is `#611`'s, not `#671`'s —
  I have attributed it wrongly before.)
