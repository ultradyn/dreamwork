# BRIEF — #777 increment 1: the check, and the classification it needs to be safe

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/glm-777cite
Branch: glm-777cite
Base sha: 2f57cd8e
...
Lane-owns: lint.py, test_lint.py

**Task:** `python3 dev/ledger.py get 777 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — the entry carries the full measurement, the two populations, and the honest ceiling. Read it first.

## What this increment is, and what it is NOT

**IS:** a lint check that a `<file>:<line>` citation in a **living** doc names a line that exists, plus the living/historical classification that makes it safe.

**IS NOT:** re-anchoring the 335 dangling citations. That is increment 2, it is much larger, and doing it now would bury the check in a thousand-line diff. **Do not fix the citations.** Your deliverable is the check plus a report of exactly what it finds.

## The measurement, reproduce it

`watch.py` is **6255** lines. Across the repo's markdown, excluding worktrees: **335** `watch.py:N` citations with N > 6255, in **48** files; **265** in range. #397's client extraction moved ~9,300 lines of CSS/JS into `client/`, and every citation written before it now points past EOF.

Worst offenders: `render-architecture.md` 44, `handoffs.md` 35, `question-updated-wake.md` 27, `findings/514-wake-semantics-audit.md` 25, `user-event-journal-implementation.md` 20, `delivery-modes.md` 20, `findings/519-exactly-once-audit.md` 17, `inbox.md` 15, `migration-backup/tasks.md` 15, `posture-autonomy-axis.md` 13, `tasks-page.md` 11, `filebytes-range.md` 11.

**Generalise past `watch.py`.** The bug is "a citation outlived the file it names", and any tracked file can shrink. Build the check for `<tracked-file>:<line>` generally, then report which files besides `watch.py` actually have dangling citations today — that count is a finding either way, including if it is zero.

## The classification is the hard half, and it is what makes this safe

Two populations, and **only one is a defect**:

- **LIVING** design/plan docs still consulted — `render-architecture.md`, `delivery-modes.md`, `user-event-journal-implementation.md`, `posture-autonomy-axis.md`, `tasks-page.md`, `filebytes-range.md`, `question-updated-wake.md`. A dangling citation here is a real defect: a lane sent to read the doc lands nowhere, or reconstructs what it thinks was meant.
- **HISTORICAL** append-only records — `handoffs.md`, `inbox.md`, `migration-backup/tasks.md`, lane reports, and `findings/*` that describe a past state. **These were CORRECT WHEN WRITTEN.** Flagging them is wrong and "fixing" them would falsify a record. The check must stay silent on them (#755).

Use an **explicit allowlist of historical paths**, not a heuristic — a heuristic will misclassify the `findings/*` docs, which is exactly where judgement is needed. **Anything you cannot confidently place, report as unclassified rather than guessing which bucket it belongs in (#702).** I expect the `findings/*` directory to be genuinely mixed and I am not pre-deciding it for you; tell me what you found and what rule you used.

## Red-proof, both directions

- **Direction 1:** point a living-doc citation at a line past EOF and watch the check red on a **discriminating** message — one naming the file, the cited line, and the actual line count. "Citation error" is not enough to act on.
- **Direction 2 is the one that decides whether this ships.** Run the finished check against the repo as it stands and prove it stays **silent on every historical doc** — 35 dangling citations in `handoffs.md` alone must produce zero output. A check that flags an append-only record has made the repo worse, because the only available "fix" is to falsify history. Then construct a healthy living doc whose citations all resolve and prove it passes.
- Run `python3 dev/lessons_index.py --act red-proof` before your first injection.
- **Never `git checkout` to restore an injection** (#349) — snapshot lane-privately via `dev/lane_scratch.py snap`, restore by `cp`, verify with `cmp`. Snapshot the FIXED file, not the pre-fix one (#608).

## Name it for what it proves, not what you wish it proved

This catches **past-EOF only**. A citation that still lands inside the file but on the **wrong line** — which the extraction also produced in bulk — is undetectable this way. Do not name the check, its message, or its OK row as though it verifies citations are *correct*; it verifies they are *in range*. An OK row reading "citations verified" would be a lie of exactly the kind #651 names, and it would stop anyone looking harder.

## Expect the check to fail its own repo on first run

There are 335 dangling citations today, so a check that reds on living docs will red the gate immediately. **That is the correct behaviour and you must not weaken the check to get green.** Decide and state which you built: an ERROR that blocks (and therefore requires increment 2 before it can land enabled), or a WARN that raises the bar count and lets the fix land incrementally. **My recommendation is WARN with an exact count**, so the check lands now and increment 2 drives the number to zero — but argue it yourself; you have the counts and I want your reasoning, not my conclusion.

Whichever you choose, say clearly in your report what the **new worktree bar** becomes, because I gate every merge against it and a bar that silently moved is #756.

## Lessons selected for this task

- **#755 — a check that fires on a healthy input is worse than no check.** Historical docs are healthy input.
- **#702 — report what you cannot classify.** The `findings/*` mixed bucket.
- **#651 — a name must not imply more than it proves.** In-range is not correct.
- **#707 — every widening multiplies false attribution.** Why the allowlist is explicit.
- **#756 — never quote a stale bar.** Including the ones below.

## Bars

**Measured in your worktree at dispatch, 2026-08-01, base `2f57cd8e`:** worktree lint **5 warnings**; `test_lint.py` collects **552**. Re-measure and quote what *you* got; if we disagree, your numbers win and the disagreement is worth reporting.

- Count tests by collecting, not by grepping `def test_`.
- `just test` is the gate. Limit to 2 threads. Machine load is ~22 from unrelated work — if something is flaky on timing, say so rather than retrying until green.
