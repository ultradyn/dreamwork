# BRIEF — #825: decide whether briefs are files or records, before the answer gets decided by accident

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-825briefs
Branch: cx-825briefs
Base sha: 3a7539f9
...
Lane-owns: a NEW design doc under `.dreamwork/docs/plans/`, and `.dreamwork/docs/doc-map.md` to
register it. **This is an ANALYSIS-AND-RECOMMENDATION task — do NOT change `.gitignore`, do NOT
migrate anything, do NOT touch `dev/dispatch_lane.py`.** The deliverable is a decision document with
a recommendation I can act on. **Do NOT touch** `watch.py`, `test_watch.py`, `client/*`,
`dev/status_sync.py`, `bin/ud-dw-chat` — other lanes hold those.

**Task:** `python3 dev/ledger.py get 825 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`
**Note the `--ledger` form**; a bare `get` refuses from a worktree.

## His question, verbatim (2026-08-01 15:17)

> *"figure out whether `.dreamwork/docs/briefs/` should be gitignored or not, and whether these briefs
> should get stored in the db. We might launch subagents via api / dreamwork cli in future (eg so we
> have just like 'role' and 'prompt' required params for subagents, and they are launched via ccc or
> direct cli call or similar by the dreamhub directly, though task set by dreamworker). We should
> consider this when considering if briefs/ should be gitignored."*

## Context he may not have, and it cuts against the naive answer

**The corpus was untracked for two days and it cost us.** That blindness produced a false conclusion
(`#786`'s) and an `AUDIT IS INCOMPLETE` banner that stood for two days before anyone noticed the
corpus was simply not in git. It is now **fully tracked, 279/279**, and `dev/dispatch_lane.py`
enforces integrity receipts from `INTEGRITY_START_TASK = 766` onward — `#807` fixed a hole where a
receipt whose brief was present but below that id escaped governance.

**So the status quo is TRACKED, deliberately and recently.** Any recommendation to gitignore must
argue against that history, not around it. Read `#807` and `#786` before you form a view.

## The real question is his second one, and it dissolves the first

He is describing a future where **dreamhub launches subagents directly from `(role, prompt)`**, with
the task set by the dreamworker. In that world a brief is not a file at all — it is a **record with
an integrity receipt**, and "should the directory be gitignored" stops being a meaningful question.

**Answer the record-vs-file question first.** Then the gitignore answer follows from it rather than
being chosen independently and later contradicted. `DREAMWORK.md`'s **one fact, one home on disk**
philosophy is the governing constraint: a brief stored in both git and the DB is two homes for one
fact unless one is explicitly derived.

## What I need from you

1. **What a brief actually IS today** — measured, not assumed: who writes it, who reads it, what
   enforces it (`dev/dispatch_lane.py`'s receipt path), and what would break if it moved.
2. **A recommendation with its reasoning** — tracked-file, DB-record, or a staged path between them.
   **Judge with IGC** using the worktree-local `./igc-method.md`.
3. **What the `(role, prompt)` API needs from the storage decision**, so we do not build the wrong
   thing now and pay to undo it.
4. **What is genuinely open and needs HIS ruling**, stated as a question I can put to him directly.

**Do not manufacture a recommendation you do not believe.** "The status quo is right and here is why"
is a complete and valuable answer; I will not read it as failure. What I cannot use is a survey with
no position.

## Red-proof

This is a design document, so the honesty check is different but not absent:

- **Every factual claim tagged VERIFIED (measured, cited by `file:line` or command output) or
  INFERRED (judgement).** A design doc that mixes the two is how a guess becomes a premise — and I
  have made that exact error three times today and been caught by three separate lanes.
- **State what would falsify your recommendation.** A recommendation with no failure condition is a
  preference wearing an argument's clothes.
- If you add any check or guard, it gets both directions like any other: break it, watch it red on a
  discriminating message, and construct the input where the thing checked is broken but the check
  still passes.

## Lessons selected for this task

Cite by **content**, not my wording — verify each before relying on it.

- **`#786`** — a false measurement licensed a wrong conclusion. The corpus blindness IS this incident.
- **`#807`** — the receipt-governance hole; the integrity model you are reasoning about.
- **`#763`** — read the owning decision before calling it a defect. The tracked state is deliberate.
- **`#440`** — one supported way. Two storage homes for one brief is the thing to avoid.
- **`#764`** — a document that ships a false measurement teaches readers its measurements are
  decorative. Your VERIFIED/INFERRED tagging is the antidote.

## Run these, explicitly

```
python3 dev/repo_wide_guards.py list          # 3 nodes now — use what it prints, not a remembered number
just pytest $(python3 dev/repo_wide_guards.py list)
python3 lint.py | tail -1
python3 lint.py | grep doc-map                # the doc-map is TEST-ENFORCED (test_lint.py::TestDocMapPlans)
```

Registering a new plans doc changes the doc-map row — run that grep **before and after**.
