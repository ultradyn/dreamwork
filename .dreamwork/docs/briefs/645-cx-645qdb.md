# BRIEF — #645: migrate questions to the DB with a principled, reusable Python DB API

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-645qdb
Branch: cx-645qdb
Base sha: beddf975
...
Lane-owns: `dreamwork_db/` (except `core.py` — read it, extend only if you justify it),
`test_dreamwork_db_*.py`, a questions CLI wherever you decide it lives, and its tests.
**NOT `watch.py`** (`glm-646policy` owns it this tick). **NOT `.dreamwork/questions.md` itself** —
that is the human's and coordinator's channel; a lane never writes it.

**Task:** `python3 dev/ledger.py get 645 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry and ALL notes. This has landed increments before and **increment 9 was REVERTED**; find out why before designing, because the reason is the constraint.

## Read the existing ladder before adding to it

`dreamwork_db/core.py` already carries a classified error ladder — `Busy`, `Corrupt`,
`SchemaMismatch`, and (landed today, #782) `ConstraintViolation`, which exists specifically to
distinguish *the store failing* from *the store correctly rejecting caller data*. `#681` built
validation into `ledger_write` precisely so callers validate before the write rather than catching a
constraint violation as control flow. **Follow that pattern; do not catch your way around it.**

`#667` is why you resolve through the store rather than the text shim: `.dreamwork/tasks.md` is a
migration stub and `ledger_parse.store_records(dw_dir)` is the real read.

## The reusable API is the point, not the questions table

The title says "principled, reusable". A migration that lands a questions table plus bespoke helpers
has done the easy half. State plainly what makes the API reusable and demonstrate it — ideally by
showing the questions path and one existing path expressed through the same seam.

**Judge rival shapes with IGC** — the worktree-local `./igc-method.md`.

## Red-proof

- **Direction 1:** break a write path and watch it red on the **discriminating** message — the wrong
  *class* (`ConstraintViolation` vs `Busy` vs raw sqlite), not merely that an exception occurred.
- **Direction 2, mandatory:** a locked store must still behave as `Busy` and must still reach
  `dispatch_lane`'s `DID NOT RUN; launch allowed` path. That path runs immediately before
  `os.execvp` on the loop's **only** dispatch route and lanes are live right now — a change that
  makes it refuse wrongly does not degrade a feature, it stops the loop launching lanes at all.
- **Direction 2b:** show an `OperationalError` that is neither busy nor schema-shaped still becomes
  the honest unclassified `DatabaseError` and is not swept into a named class.
- **Never open the live `.dreamwork/ledger.sqlite3` for writing** and never run mutating ledger verbs
  against the live ledger — use a fixture via `--ledger`.
