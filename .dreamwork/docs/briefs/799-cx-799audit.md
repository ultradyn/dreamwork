# BRIEF — #799: the audit tool advertises a flag that does not exist, and crashes on three store states it should classify

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-799audit
Branch: cx-799audit
Base sha: aadf579f
...
Lane-owns: `dev/citation_audit.py`, `test_citation_audit.py`.

**Task:** `python3 dev/ledger.py get 799`. Read the entry in full.

## Read this before you read the entry: one third of it is already fixed

`#799` was filed an hour ago against `dev/citation_audit.py` at a base that no longer exists. **`#796`
landed twelve minutes ago** (merge `aadf579f`, your base) and it fixed the entry's headline claim.

The entry says a lane's default invocation reports `examined 161, UNRESOLVABLE: 161`. **I re-ran it
from a fresh worktree at your exact base just now:**

```
corpus: 219 tracked / 273 on disk (AUDIT IS INCOMPLETE — untracked briefs not visible)
citation_audit: examined 381 citation(s)
  UNRESOLVABLE:     2
  NO_RELATIONSHIP:  3
  UNCLASSIFIABLE:   376
```

Two, not 161. `_default_dw_dir()` now reaches the main store from a worktree. **Do not re-fix that
half, and do not report it as fixed by you.** I am telling you this because I have twice this session
dispatched a lane against a defect that had already been closed, and the cost each time was a lane's
whole run. **Re-derive the numbers above yourself before you build** — if they differ from mine, say
so and trust yours.

## What is genuinely still live — this is your scope

### 1. The documented recovery flag does not exist (`#651`)

The module's usage prose tells the reader to pass `--ledger PATH`. `argparse` implements only
`--dw-dir` and rejects `--ledger` at exit 2. So the tool names a remedy its own reader cannot use.
`#796`'s lane saw this and correctly declined to widen its fix — it is yours.

**Decide which is the truth and make the other match**: either the docs should say `--dw-dir`, or the
flag should exist under both names. One supported way (`#440`) — do not ship a doc that describes an
alias you did not build.

### 2. Three store states crash instead of classifying

An exclusive SQLite lock raises uncaught `dreamwork_db.core.Busy`; a half-migrated schema raises
uncaught `SchemaMismatch`; a missing store file silently converts a **VALID** citation into
`UNRESOLVABLE`. That last one is the worst of the three, because it is the *silent* one: a store fault
is reported as evidence that a task id does not exist.

`#782` built the classified error ladder (`Busy` / `Corrupt` / `SchemaMismatch` / `ConstraintViolation`)
for exactly this. Read it before designing — the ladder exists so callers can tell a store fault from
a data fact, and this caller currently cannot.

### 3. A question I want argued, not silently answered

**376 of 381 citations classify as `UNCLASSIFIABLE` — 98.7%.** I do not know whether that is correct.
It may be honest (most citations genuinely are not task-id references), or it may be `#671` in a new
costume: a bucket so large that the tool's confident output conveys nothing. **Work out which, and say
so with evidence.** If the category is doing real work, show what a healthy run looks like and why the
number is high. If it is a catch-all that hides misses, that is a finding worth more than the two
fixes above and I want it named even if you do not fix it in this lane.

Do not widen scope to fix it without saying so first — report it and I will file it.

## Red-proof

- **Direction 1:** for each of the three store states, inject the real fault (lock the store, hand it a
  mismatched schema, remove the file) and watch your classification red on the **discriminating**
  message — it must name *which* store fault occurred, not merely that the audit failed. A missing
  store must not be reportable as `UNRESOLVABLE` at any severity.
- **Direction 2, mandatory:** construct the input where the check is broken but still passes. The
  specific shape to beat here: a fixture whose store is **healthy** makes a broken fault-classifier
  indistinguishable from a working one — every path returns the same answer. That is precisely the
  vacuous shape `#796` was filed for one file over. Show the vacuous version failing to catch a
  sabotage, then show yours catching it.
- **The boilerplate's red-proof paragraph changed this hour** (`#795`): you must now **name the
  production seam you break — path plus symbol or branch** — and editing a test's assertion is not a
  direction-1 proof. Read the current paragraph, do not work from memory of it.
...
Cite these by **content**, not by my wording — I have several times this session attributed a
principle to an id whose entry does not contain it. Verify each before relying on it.

- **`#651`** — a guard whose message names a failure mode it cannot detect. `--ledger` is the pure
  form: a message naming a remedy that does not exist.
- **`#671`** — a check that examined nothing must not read as passing. Your section-3 question.
- **`#782`** — the classified store-error ladder, and why a caller must not flatten it.
- **`#440`** — one supported way. Docs and argparse must not describe two.
- **`#611` / `#685`** — gitignored state cannot travel into a worktree. The mechanism behind the half
  that `#796` already fixed; you need it to avoid re-breaking that fix.

## Run these, explicitly

```
just pytest test_citation_audit.py
...
a verification list derived from the files you edited rather than from what they reach is `#763`
instance six.

## Standing bars

**Master is GREEN as of your base** — a full `just pytest` at `7ecb732a` read 2789 passed / 0 failed,
and the one failure that followed (`test_lint.py::TestDocMapPlans`) was closed by `#794`, which is in
your base. A gate run over your exact base is in flight as I write this; if it reds I will tell you.
**Quote your own failure list explicitly rather than saying "same as baseline".**

- Main-checkout lint now reads **1** (it was 2; `#794` removed the doc-map row this hour). Your
  worktree reads **5**. Read the bar with `python3 lint.py | tail -1` — the count is in the
  **trailer** and the rows are **indented**, so `grep -c '^WARN'` returns a false **0**.
- **Compare the complete WARN ROW SET, not the trailer count** (`#794`, landed this hour): the count
  can fall while a new warning appears. Quote the rows before and after.
...
- Rebase onto master before you finish and resolve conflicts yourself. Master is moving fast today.
