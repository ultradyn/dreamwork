# BRIEF — #818: a chat message is a turn in a conversation, and it waits 4 minutes

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-818chatwake
Branch: cx-818chatwake
Base sha: 05f7f902
...
Lane-owns: `watch.py`, `test_watch.py`, `.dreamwork/docs/plans/delivery-modes.md`.

**Task:** `python3 dev/ledger.py get 818 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`
Read the whole note — it contains the measurement and the two candidate fixes.

## What was measured, today, live

Max sent a chat at **15:04**. I did not see it until **15:08**, on the tick drain. He then asked me
directly: *"are chats not delivered to you immediately?"* The answer is no, and here is the seam:

```python
# watch.py:4627
def emits_wake(kind, target):
    if kind in PREEMPT_KINDS:          # watch.py:4615 — ("do-now", "do-next")
        return True
    return delivery_mode(target) == DELIVERY_DEFAULT   # "instant"
```

Delivery posture is `batched`. So a `/command` with `kind=chat` commits its receipt (durable —
**nothing is lost**, and that half is working correctly) but emits **no** `watch-events.log` line, so
it reaches the coordinator only on the next heartbeat drain.

**The bug is the classification, not the mechanism.** Batching was designed for the *ambiguous* class
— ideas, notes, anything not addressed to *now*. A chat is a turn in a dialogue with a human who is
sitting there waiting for a reply. That is the definition of the pre-empt class.

## The decision I am NOT pre-making

Two routes, and I want the second one argued rather than assumed:

- **(a) Add `"chat"` to `PREEMPT_KINDS`.** One-line, matches the existing ceremony, and is honest:
  chat *is* a pre-empt kind. Cost: it becomes unconditional — he cannot turn it off even if a chat
  thread gets chatty during deep work.
- **(b) Give chat its own posture axis** (e.g. `chat-delivery: instant|batched`), so he chooses.
  Cost: a fifth-and-a-half axis on a surface `#445` deliberately held to five, and `#440`'s
  one-supported-way pressure cuts against a second delivery knob.

**I lean to (a) and I am genuinely unsure.** `#445` Q3 ruled the axes orthogonal and resisted
widening `run-mode`; that argues against (b). But he has twice now asked for *explicit control* over
things this loop made implicit (see `#646`'s save/reset button). **Judge with IGC** using the
worktree-local `./igc-method.md` and state which you chose and why.

Whichever you pick: `/chat-reply` is a **route**, not a kind — check whether it needs the same
treatment. A reply from him on an existing thread is at least as conversational as a new chat, and
right now `emits_wake` receives route paths for the `/ask`, `/answer`, `/comment` family. Say
explicitly what you found and what you did about it.

## Red-proof

- **Direction 1:** break the production seam — name it as path plus symbol or branch — and watch the
  test red on a **discriminating** message. The failure must say that a chat did not wake the loop,
  not merely that a function returned False.
- **Direction 2, MANDATORY, and this is the exact shape that already bit us:** a test that asserts
  *the receipt committed* proves **nothing** about the wake line — the receipt was always committing;
  that is precisely why this defect survived. Construct the input where the receipt is present and
  correct and the wake line is absent, and show your test catching it. **Assert on
  `watch-events.log` content, not on receipt existence.** `#814` closed this same false-green shape
  in the neighbouring route family today; read its landing before you start.
...
Cite by **content**, not my wording — verify each before relying on it. I have miscited an id three
times today and three separate lanes caught me, so please do check.

- **`#671`** — a check that examined nothing must not read as passing. A receipt-existence assertion
  standing in for a wake-line assertion is exactly this.
- **`#651`** — a guard's message must name a mode the guard can actually detect.
- **`#440`** — one supported way; this is the strongest argument against route (b).
- **`#519` / `#528`** — the wake line is the interrupt, the receipt is the record; and why the drain
  must run on every tick regardless of mode. Your change must not weaken either half.
- **`#445`** — the orthogonal-axes ruling, and its explicit reluctance to widen the posture surface.
...
**Master is GREEN as of `05f7f902`** on the targeted files (`24 passed` across
`test_reconcile_submissions.py test_user_events_http.py`; a full gate is in flight). Do not treat
that as reassurance — `#809` demonstrated today that two mutations survive their whole relevant file.

- Main-checkout lint reads **1** — the `lessons.md` near-duplicate, which is **human-gated and must
  stay**. A worktree reads ~5. `python3 lint.py | tail -1`; the count is in the **trailer** and rows
  are **indented**, so `grep -c '^WARN'` returns a false **0**.
...
- Rebase onto master before you finish and resolve conflicts yourself. Master moved 28 times today.
