# BRIEF — #657: he asked for this by name, and the chat surface is where he now talks to me

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-657chat2
Branch: cx-657chat2
Base sha: 05f7f902
...
Lane-owns: `client/views.js`, `client/router.js`, `client/style.css`, `client/dist`, and a NEW browser
guard under `dev/capture/`. **You do NOT own `test_watch.py` or `watch.py`** — `cx-818chatwake` is in
both right now. If you need a Python test, put it in a new file.

**Task:** `python3 dev/ledger.py get 657 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`
— read the entry and all notes. **Note the `--ledger` form**; a bare `get` refuses from a worktree.

## He asked for this directly, twice over

At 14:59 today, into the dashboard composer: *"we have a task about topic chats formatting on
dashboard, it needs to be investigated and fixed. find the task and delegate to subagent."* This is
that task. A previous lane was dispatched on it and died having committed **nothing** — so nothing
has been done and you are starting clean.

The entry names two defects:

1. **Two chat entries render with no linebreak between them** — the list runs them together.
2. **A just-submitted pending chat shows the wrong turn count.**

## Bug 2 is the one that will fool you, and I am telling you the trap in advance

**Bug 2 is transient by nature.** It appears in the window between submitting and the state settling.
**A test written against the settled state passes against completely broken code** — it waits, the
count corrects itself, and the assertion is green. That is a vacuous test, and this loop demonstrated
today (`#809`) that two such mutations survive their entire relevant test file undetected.

So: **your test must observe the pending window**, not the resolved one. If you conclude the transient
cannot be observed deterministically, say so explicitly and argue for what you did instead — an
argued limitation is a fine deliverable; a settled-state assertion presented as covering bug 2 is not.

## Why this surface matters more than its priority suggests

The chat surface is now a live channel between Max and this loop — he opened a thread on it today and
is waiting for replies there. Rendering defects on it are not cosmetic; they are noise on the wire.
Treat it accordingly.

## Red-proof

- **Direction 1:** break the production seam — **name it as path plus symbol or branch** — and watch
  your check red on a **discriminating** message. For bug 1 the failure must name that the entries
  were not separated, not merely that some HTML differed.
- **Direction 2, mandatory:** construct the case where the fix **looks** applied but is not. For bug 1
  the obvious false-green is a check that asserts a separator element *exists in the markup* while the
  CSS collapses it to zero height — visually unchanged, structurally "fixed". For bug 2 it is the
  settled-state trap above. Show the naive check failing to catch the sabotage, then yours catching it.
...
Read `transitions.md` before changing anything that animates.
...
- **`#671`** — a check that examined nothing must not read as passing. The settled-state test is this.
- **`#651`** — a guard's message must name a mode it can actually detect.
- **`#440`** — one supported way; reuse the existing chat-render path rather than adding a second.
- **`#702`** — a count assertion passes when membership changed but length did not. Bug 2 is a count.
- **`#809`** — landed today: two mutations survived their whole relevant file. Your direction-2 is why.
...
Browser guards bind **39890–39899**, hub **39880–39889**. `#666` measured they return WRONG answers
under load — **if a guard reds, re-run it quiet before believing it**; if you could not run them, say
so rather than reporting a UI change as visually verified when it was not.

## Bars

**Master is GREEN as of `05f7f902`** on targeted files; a full gate is in flight. Do not treat green
as reassurance — see `#809` above.

- Main-checkout lint reads **1**; the remaining WARN is the `lessons.md` near-duplicate, **human-gated
  and must stay**. A worktree reads ~5. `python3 lint.py | tail -1` — the count is in the **trailer**
  and rows are **indented**, so `grep -c '^WARN'` returns a false **0**.
