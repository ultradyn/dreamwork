# BRIEF — #631 increment 6: the cold SessionService, still dark

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/glm-631i6
Branch: glm-631i6
Base sha: 4a569f24
...
Lane-owns: session_log/, test_session_log_*.py


**Task:** `python3 dev/ledger.py get 631 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry and all its notes. Increments 1–5 are landed.

**Design:** `.dreamwork/docs/plans/session-log-view.md`, **§12 increment 6**, which specifies this increment including its injection. That spec is authoritative; this brief adds only what it does not say.

> **Cold `SessionService`, still dark.** Add one thread-safe service that resolves ids through the catalogue, runs the scanner, owns the in-memory per-session event ring/cursor, builds a snapshot skeleton and parses a bounded peek from a registered source range. It has no file-notification thread and no HTTP caller yet.

**Files:** new `session_log/service.py` and `test_session_log_service.py`. The service calls `session_source` and the adapter; its test is its only caller.

**Explicitly later:** wakeups, retirement, HTTP, and the derived store. That is increment 7 onward.
...
Increment 5 established this one level down: the wire `CatalogEntry` carries an opaque `session_id` and no path at all, and confinement resolves links **first**, then confines the resolved path against the resolved root. **Read `session_source.catalogue` before you design the service** — the service must not reopen the hole the catalogue closed, and the way that happens is a convenience overload that accepts "an id or a path".

**Impossible, not merely rejected.** A validated string is still a string; an id that can only ever be resolved through the catalogue is a different shape. Say which one you built.

## The other two hazards, neither of which the spec spells out

**Thread safety is claimed in the spec and is the hardest thing here to prove.** "Thread-safe" asserted in a docstring is worth nothing. Either exercise it concurrently, or state plainly what your tests do and do not establish about it (`#651`). A single-threaded test suite over a service documented as thread-safe is precisely a name implying more than it proves.

**A bounded peek is a bound that can be wrong in two directions.** Too small silently truncates; too large is the thing the bound exists to prevent. And a peek over a *registered source range* is an offset pair — which is where off-by-one hides. `#645`'s increment 7 lane found exactly this shape and closed it: a 1-byte-short span produced text that looked correct and passed a naive equality check, but failed a fresh re-derivation of the expected length. **Use that route, not a naive comparison.**

## Direction 2

The snapshot skeleton and the event ring **agree with each other trivially if both are built from one scan in one call.** Construct the case where the cursor has advanced but the snapshot has not, and show what the service reports. If snapshot and cursor cannot disagree in your design, say so and explain why that is a property rather than an untested assumption.

Second: a service that resolves an id which no longer exists on disk — the session was deleted between catalogue and read — must not render identically to one that resolved an empty session (`#136`). Both are plausible today; only one is a fault.

## What increments 1–5 established that binds you

- The node/state/event vocabularies are **closed**; `session_log.model` raises `ModelError` on an unknown kind. Do not widen them.
- The three classifier outcomes — a node, `suppressed`, `unclassifiable` — must stay distinguishable through the service (`#702`).
- `scan_incremental` with its `Frontier` is **the one supported way** to resume. Do not add a second.
- The recorded `agent_session` is the only active identity; newest-mtime is never promoted.
- The wire shape is exact: `{ev, node}`, and `ref.len` is spelled `len`.

## Scope

A **~15–20 minute landing**. Increment 5 came in at +21 collected tests. If it grows past a coherent landing, stop at a committable point and say what remains.
...
- Real Claude Code JSONL transcripts live under `~/.claude-p/projects/`. **Read them to check assumptions — do not copy one into the repo**; they contain session content.
...
- `just pytest test_session_log_service.py test_session_source.py <anything else you touched>` — the one supported form. **Do not run the bare full suite**; I own the single merged-tree sweep.
- **Count tests by collecting, not by grepping `def test_`.** Tell me the collected count before and after.
...
`.dreamwork/lane-631i6-report.md`, **and commit it** — a lane left its report untracked today and it would have died with the worktree. Lead with whether a client-supplied path is impossible or merely rejected at your API boundary.
