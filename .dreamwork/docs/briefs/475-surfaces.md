# Brief — #475 · four surface guards, one of which never judged

Repo: `ud-dreamwork`. Worktree: **`.worktrees/surfaces`**, branch **`wt/surfaces`**. Do not push, do not merge.
**Never use `attn` under any circumstances** — the coordinator is the only party that notifies the human.

Lane-owns: dev/capture/health.mjs, dev/capture/serving.mjs, dev/capture/answers.mjs, dev/capture/burndownmock.mjs

## Your four

```
health       | a refused write says so, instead of nothing at all
serving      | a page behind HEAD says so, and by how much
answers      | tick trigger accepted
burndownmock | the guard threw before finishing its checks
```

**`burndownmock` is the different one and do it FIRST.** It is one of the two guards `#471`'s new accounting
named as having **never judged** — it printed no genuine verdict and died before its first assertion, so it
gated nothing. The other such guard turned out to be a real cross-lane break (`#436`'s ask gate locking a
guard fixture out of `review_artifact.py`, fixed `43036f2`), so **read the crash, not the exit code** — the
error text names the cause. Note also that `#417` merged into `watch.py` tonight and changed the burndown
panel substantially (c3 cap weight, a c4 figure line, a `.bdtip` hover), so a mock built against the previous
panel is a live hypothesis.

**`health` is about the same `#263` E5 contract that has already bitten one guard.** A refused write is
**`202` with `rejected` in the body**, not `4xx`; `_reject(reason_code, detail)` and a closed
`REJECTION_REASONS` set three wide. `#474` found `identity` asserting `400` and failing since E5 unnoticed.
Check whether `health` is the same mistake — and whether the *page* genuinely surfaces a refusal, because
"says so, instead of nothing at all" is a claim about what he can SEE, and if the page has gone silent about
refusals that is a real defect and the more serious answer.

**`serving`** is a version/staleness surface (a page behind HEAD saying so, and by how much). Note the skill
version and `skill_identity` moved this session — `/summary.json` now projects a `skill_version` scalar
(`#275` Q5). Ask whether the guard reads a shape that changed.

**`answers`** fails on `tick trigger accepted`. Do not assume that is a write failure: I verified the write
path works. Find what "accepted" means to this guard and what it reads to decide.
...
starting, prefix every line `[surfaces]`, and create + watch
`/home/xertrov/.cache/agent-comms/ud-dreamwork/surfaces-inbox.md` so I can steer you mid-task.
...
- `python3 lint.py --target .` clean; run your guards with an explicit port, e.g.
  `DREAMWORK_GUARDS="..." DREAMWORK_HUB_GUARDS= just guards 39893`. **Do not run the full `just test`** —
...
- **Work only inside `.worktrees/surfaces`.** Verify cwd and branch before every write.
...
`.worktrees/surfaces` (state cwd and branch), edited no `watch.py` and no `justfile`, left nothing listening,
