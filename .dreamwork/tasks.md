...
- **#205** — [plan: `docs/plans/heartbeat-into-monitor.md` — ezfb's
  ...
  **CEILING**: 4.75m sits under the prompt-cache TTL, which is why the
  ...
  · relates #180, #200, #203
- **#204** — [adoption path exists: #166's handler takes a LIST of
  surfaces (9ed526f), so each peek is one list entry away from the
  shared gesture; #166's red-first run is also this bug's direct
  evidence — six motion checks red on the native toggle while every
  end-state check stayed green] The four plain `expand()` peeks still snap · P3 · task ·
