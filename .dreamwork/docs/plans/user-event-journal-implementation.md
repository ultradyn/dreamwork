...
- `watch.py` — `def log_submission(target, path, body, nbytes, truncated=False)`.
- `watch.py` — the single call site, in `do_POST`, before dispatch.
- `watch.py` — `except OSError: pass`. This is #262's whole bug: the
  witness can fail silently and the request is still dispatched and acknowledged.
- `watch.py` — `body = self.rfile.read(min(nbytes, MAX_BODY))`, and:
...
- `watch.py` — six POST routes: `/answer`, `/ask`, `/comment`,
  `/command`, `/tint`, `/run-mode`.
- `watch.py` — `_handle_command` validates `kind`, validates non-empty
...
- `watch.py` — `_send` hardcodes `self.send_response(200)`.
...
- `watch.py` — `ANSWER_LOCK = threading.Lock()`, used at `8430`, `8453`,
...
watch.py     # _handle_answer
watch.py     # _handle_comment
```
...
exclusion is a `threading.Lock` (`watch.py`), and a threaded version of
...
`watch.py` does today) must make this test fail.
...
appears), verified by the lane. The plan measurement (`watch.py`) and the
...
*Red line:* the `self.send_error(400)` removal in `_read_json` (`watch.py`)
...
- **Zero task state is mutated at HTTP time** (its F3(a)). Independently
  re-measured above at `watch.py`. So increment 22's `202` promises
...
- **`/answer` and `/comment` truncate `questions.md` in place** (`watch.py`,
  `watch.py`) — no temp, no rename, no `fsync`, while `/ask` next to them uses
...
- **No short-read check in `do_POST`** (`watch.py`, M22) — an interrupted
...
- **`_send` cannot express a status code** (`watch.py`) — a small structural
