...
watch.py:5489-5502; initial render `posturePicker` watch.py:5760-5768):
...
- **Both ambient states: replaced by a 'remind' link-btn** — a
  ...
  `all N reviews →` line at watch.py:4063; spare, lowercase-leaning
  voice). Label: `remind`.
...
- **The cooldown/confirmation state lives in module-scope JS**, exactly
  like `postArmTimer`/`postArmUntil` (watch.py:5504-5520) — never read
