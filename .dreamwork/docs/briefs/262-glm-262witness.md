# BRIEF — #262: does the accepted-but-unwitnessed path still exist after the 202 cutover?

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/glm-262witness
Branch: glm-262witness
Base sha: e699d4f6
...
Lane-owns: watch.py, test_watch.py, user_events/, test_user_events*.py

**Task:** `python3 dev/ledger.py get 262 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry and **especially its newest note**, which unblocked it today and states the tension you must resolve. Then `get 263` (the event model this depended on) and `get 199` (the rule that says the opposite).

**#262 is P0 and was blocked on `#263`'s event model. I discharged that blocker today** — `user_events/` is built and wired (`watch.py:52` imports `Envelope, open_journal`; the 202-after-receipt cutover is live at `:2789`, `:4998`, `:5114`). #263's remainder is UI (E6, lane G), not the event model.

## Your first increment is a MEASUREMENT, not a fix

**Do not assume the defect survives.** This brief exists because I made exactly the opposite mistake this morning on a sibling task: I briefed a lane that three live entries were "a real finding about live data", when a fix landed hours earlier had already made that false. The lane then built against a premise that was not true. That cost is what this instruction is preventing.

Here is the tension, stated fairly, with both sides quoted:

- **#262 claims:** `log_submission()` catches and suppresses `OSError`, so a process can dispatch and acknowledge a request whose server witness was never persisted.
- **`log_submission`'s own docstring disagrees, deliberately** (`watch.py:4741`): *"It CANNOT RAISE. A logging failure must never be why his answer was refused, so every error is swallowed — including the ones that are the caller's fault."* That is `#199`'s argued property. It is not an oversight, and "just make it raise" would regress a rule that exists to protect his input.
- **#263 plausibly dissolves the conflict** by *moving the witness*: the receipt commits **before** the 202 (`watch.py:2789` — *"the receipt already committed, so this never refuses the 202"*). If that holds on every write route, `log_submission` is a best-effort **secondary** record and its failure no longer implies an unwitnessed acknowledgement.

**So the question you answer first is: on today's tree, is there any write route that can return a success status to the browser without a durable receipt having been committed?** Enumerate the write routes — do not sample them — and say which ones you checked and how. A route you could not classify is reported as unclassified, never silently counted as safe (#702).

Three outcomes, and all three are acceptable results for this increment:

1. **The path is closed.** Then say so with the evidence, name the commit that closed it, and propose folding #262. A P0 that no longer describes reality should not survive on reputation.
2. **The path is open.** Then this is a live P0 — build the red-first coverage the entry names, smallest increment first.
3. **Partly.** Name exactly which routes are covered and which are not. This is the most likely answer and the most valuable one.

## What the 202 cutover does NOT address, whichever way the above lands

The entry names a second harm that the receipt-before-202 change does not touch: **multiple same-target `watch` processes split receipt history.** Two processes serving the same `--target` each hold their own journal, so a receipt id minted by one is unknown to the other, and a replay or a status lookup can honestly report "no such receipt" for a request that was durably accepted. Check whether anything today detects or prevents that. If your measurement says routes are fine but this is not handled, **that** is the finding, and it is worth its own task rather than being folded into a route audit.

## Discipline

- **Red-proof both directions** if you write a check. Direction 1: inject the failure the check exists to catch and watch it red on a *discriminating* message. Direction 2: construct an input where the acknowledged request genuinely has no durable witness but your check still passes — a durability check that cannot fail is the exact shape of the bug (#671).
- Run `python3 dev/lessons_index.py --act red-proof` before your first injection.
- **Never `git checkout` to restore an injection** (#349). Snapshot lane-privately with `dev/lane_scratch.py snap`, restore by `cp`, verify with `cmp`. Snapshot the FIXED file, not the pre-fix one (#608).

## Lessons selected for this task — do not go hunting in lessons.md

- **#671 — a check that examined nothing must not read as passing.** A durability check that passes because it found no routes is the failure, not the proof.
- **#136 — "nothing needs you" and "the channel is broken" must not render identically.** This is the exact family the bug lives in: a 200 that means "witnessed" and a 200 that means "logging silently failed" are the same bytes to the browser.
- **#702 — report what you cannot classify.** An unclassified write route is reported, never assumed safe.
- **#199 — his words on disk before anything may refuse them.** The rule you must not regress while fixing this.
- **#651 — a name must not imply more than it proves.** "Durably witnessed" must mean the receipt is committed, not that a function was called.

## Ports and safety

- **Do NOT touch `:35110` (the deployed dashboard) or `:35113` (dev).** Bind an ephemeral port for any server you start, and kill it when done.
- Browser guards bind 39890–39899 and hub 39880–39889 — another lane may hold them. Check before binding; if they are taken, say so rather than waiting.

## Bars

- Worktree lint bar is **5 warnings**, not 1. Measure it at your base and quote what you measured — do not trust this number or any number in a note (#756).
- Count tests by collecting, not by grepping `def test_`.
- `just test` is the gate. Limit to 2 threads.
- `watch.py` is ~15k lines and other work wants it — keep your diff tight and **rebase onto master before you finish**, resolving conflicts yourself.
- Do not use `attn`.
