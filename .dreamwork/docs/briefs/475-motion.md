# Brief — #475 · the four motion guards

Repo: `ud-dreamwork`. Worktree: **`.worktrees/motion`**, branch **`wt/motion`**. Do not push, do not merge.
**Never use `attn` under any circumstances** — the coordinator is the only party that notifies the human.

Lane-owns: dev/capture/oneinput.mjs, dev/capture/wisp.mjs, dev/capture/qsec.mjs, dev/capture/posture.mjs

## Your four

```
oneinput | the indicator LANDS on first paint (it does not tween in)
oneinput | reduced-motion: the indicator JUMPS, it does not slide
wisp     | normal: the intensity fades in and OUT — a breath, not a sweep
qsec     | opening: ...and it travels there, rather than teleporting (0 of 76 part-way)
qsec     | opening: the section really grows (else its height check is vacuous) (17 -> 0, 17px)
qsec     | opening: the section itself grows continuously rather than in one step (0 of 76 part-way)
qsec     | opening: the revealed body eases in rather than blinking on
posture  | arm bar drain visited mid frames: 0 (between())
posture  | posture file written
posture  | file carries pace: steady
posture  | file carries asking: inform
posture  | file carries a non-negative delegation integer
```

**Two readings of this list are already available and both are worth testing.**

**`posture`'s five are probably ONE failure.** The head is a motion sample; the four file assertions cascade
because the guard never finishes arming the 10s bar, so nothing is ever written. I verified separately that
the production write works — a direct `POST /posture` writes the file correctly — so **do not go looking for a
write bug**. Find out why the arm bar's drain samples zero mid-frames.

**`qsec`'s `0 of 76 part-way` is not frame starvation.** Seventy-six samples with none part-way says the
transition is not running at all, and `the section really grows … (17 -> 0, 17px)` says the section
**collapsed** rather than grew. Read those two together before theorising: a section that ends at 0 height
cannot show intermediate heights, so the second may be the cause of the first and the other two may cascade
from it.

**Read `transitions.md` first — its opening section is *how to check*, and it is not optional.** An end-state
assertion cannot fail on a motion bug and neither can "did it move"; these guards sample intermediate frames
for that reason. `#414`'s frame-rate-free helpers in `dev/capture/dom.mjs` (`between()`, part-way counting)
are the shared instrument — **read** that file, and if the helper itself is the defect, say so with the diff
rather than working around it in four guards.

Reduced-motion parity is part of every verdict here, not a follow-up.
...
starting, prefix every line `[motion]`, and create + watch
`/home/xertrov/.cache/agent-comms/ud-dreamwork/motion-inbox.md` so I can steer you mid-task.
...
- `python3 lint.py --target .` clean; run your guards with an explicit port, e.g.
  `DREAMWORK_GUARDS="..." DREAMWORK_HUB_GUARDS= just guards 39892`. **Do not run the full `just test`** —
...
- **Work only inside `.worktrees/motion`.** Verify cwd and branch before every write.
...
`.worktrees/motion` (state cwd and branch), edited no `watch.py` and no `justfile`, left nothing listening,
