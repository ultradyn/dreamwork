# BRIEF — #798 + #802: the revision judge can be talked into PASS by a comment

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-798magic
Branch: cx-798magic
Base sha: 7ecb732a
...
Lane-owns: `dev/bisect_guard.py`, `test_bisect_guard.py`,
`.dreamwork/docs/bisecting-browser-guards.md`

**Task:** `python3 dev/ledger.py get 798` then `get 802` — two findings in one file, both from the
same independent review. Fix them together.

## #798 — the tool's central claim does not hold

#784 exists because my #781 "bisect" never ran the revision it named. Its whole purpose is to
distinguish **a judged verdict** from **an unjudged one**. An independent reviewer defeated that:

- **State:** a real temporary git revision carrying every required file, whose `justfile` contains
  the strings `guard-execution`, `already held` and `is serving` **only inside a comment**, and whose
  `guards` recipe performs no browser assertion at all — it just prints `PASS qroll`.
- **Result:** `inspect_revision_tree()` accepts it (substring checks), and `judge_revision()` returns
  `PASS — historical guard-execution gate confirmed a judged pass`. **No guard judged anything.**

This recreates the exact confident-unmeasured-boundary failure the tool was built to prevent, which
is why it is P1 rather than a nitpick.

A substring search over a justfile establishes that some bytes are present, never that a guard RAN.
Weigh at least: parse the recipe **body** rather than the file; require a machine-readable verdict
from the guard itself; or **refuse to judge** revisions whose gate cannot be established — exit 125
already exists for precisely that, and refusing is the honest outcome the tool already models.
**Judge with IGC** (worktree-local `./igc-method.md`). An argued *"this cannot be made sound against
arbitrary historical justfiles, and the procedure must say so"* is acceptable — the doc already
qualifies its limits and #702 is the governing principle.

## #802 — it leaks locked worktree metadata that cannot be pruned

- **State:** inside a temporary revision, `git worktree lock . --reason probe-lock` is held.
- **Result:** the `finally` block's single force-removal FAILS, **its status is ignored**, and
  `shutil.rmtree` deletes the directory anyway. `judge_revision()` returns PASS while
  `git worktree list --porcelain` still lists the missing detached path as `locked probe-lock`.
  **Even `git worktree prune --dry-run` will not prune it.** Repeats accumulate permanently.

This matters beyond tidiness: `git worktree list` is how I read my own fleet and how `status_sync`
derives lane state. Entries naming paths that do not exist make that instrument lie — it already
reports phantom entries, and this adds a class that cannot be cleaned up at all.

Report the leak rather than swallowing it (#702), and **do not mask a genuine failure by deleting the
evidence.**

## Red-proof

- **Direction 1:** for each fix, break it and watch it red on the **discriminating** message —
  for #798 that the revision was accepted without a judged guard; for #802 that a registry entry
  survived.
- **Direction 2, mandatory:** construct a revision your new check reports as handled while it is
  genuinely unjudged. If your mechanism is structural (parsing the recipe body), a recipe that
  *calls* a judged guard through one indirection is the obvious escape — show what happens and name
  what you do not close.
- **Do NOT create worktrees under `.worktrees/`** — twelve lanes live there and `git worktree` is
  repo-global. Use `tmp_path`, and clean up; a stray entry makes my fleet view lie, which is #802.
