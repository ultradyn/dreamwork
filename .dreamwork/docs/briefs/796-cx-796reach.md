# BRIEF — #796: the truncation guard I merged an hour ago does not fire where it was needed

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-796reach
Branch: cx-796reach
Base sha: a564bd80
...
Lane-owns: `dev/citation_audit.py`, `test_citation_audit.py`, and the restore-teaching dogfood
guard's test (find it: `TestBriefLaneScratch::test_this_repo_has_restore_teaching_briefs`).
**NOT `dev/redproof.py`** — `cx-795proof` owns it this tick and every live lane is using it.

**Task:** `python3 dev/ledger.py get 796 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`

## I merged this believing it was closed. It is not.

`#788` landed a corpus-coverage check so that an audit which saw 87% of the brief corpus could not
read like one that saw all of it. It works in the **main checkout** and is silent in a **worktree** —
and a lane only ever runs in a worktree, which is exactly where `#786` was defeated.

Measured by `cx-791revB`, same interpreter, two targets:

```
worktree default : corpus: 219 tracked / 219 on disk                                  <- equal, NO warning
main checkout    : corpus: 219 tracked / 264 on disk (AUDIT IS INCOMPLETE ...)        <- fires correctly
```

The worktree is not lying: its `briefs_dir` genuinely holds 219 tracked files and nothing else, so
`tracked == on_disk` is **true** there and the check is honestly silent. The 45 untracked briefs are
invisible to it. **The check cannot see the thing it exists to warn about** (`#611`: absence reads as
a pass).

**Re-derive both numbers yourself** — run the tool from your worktree and from the main checkout and
quote what you got. The counts have been moving all afternoon as lanes land, so my numbers are stale
by construction; the *relationship* is the finding, not the integers.

## Why `#788`'s own test could not catch it

Its split fixture builds a `briefs_dir` containing **both** tracked and untracked files. The real
lane's `briefs_dir` **cannot see the main checkout's untracked files at all**. The fixture models a
shape that does not occur.

The reviewer named the assertion that would catch it: **a linked-worktree fixture** — create a real
linked worktree, add an untracked brief to the main checkout, invoke the worktree-local tool on its
default corpus, and require it to either resolve the main checkout's on-disk corpus **or**
refuse/report the main-vs-worktree split. That is a genuinely different fixture shape from anything
this repo currently builds, and building it is the deliverable.

**Which of those two behaviours is right is yours to judge** (IGC, worktree-local `./igc-method.md`):
resolving the main checkout's corpus makes the tool useful in a lane but couples it to a path outside
the worktree; reporting the split is honest and leaves the lane unable to audit. Argue it.

## The sibling instance, same shape, already live

`TestBriefLaneScratch::test_this_repo_has_restore_teaching_briefs` measures **86** restore-teaching
briefs in the main checkout and **43** in a worktree, and **passes anyway**, because its only
completeness proxy is `>= 10`. A floor that loose accepts a corpus missing half its subjects. Either
tighten it to something that can detect the truncation, or state why a loose floor is right there —
both are acceptable, silently leaving it is not.

## Red-proof

- **Direction 1:** break your new check and watch it red naming the **specific** condition — which
  corpus was truncated and by how much — not a generic assertion failure.
- **Direction 2, mandatory, and note this task IS a direction-2 escape from the last one:** construct
  the case where your fix looks applied but is not. Obvious candidate: a worktree whose untracked
  count happens to equal the main checkout's, so the split reconciles by coincidence. Another: a tool
  invoked with an explicit `--corpus` path that bypasses your default-resolution fix entirely. Show
  what happens and name what you do not close (`#702`).
- Run `python3 dev/redproof.py` for snapshot/restore — **use it, do not modify it** (`cx-795proof`
  owns it). `python3 dev/lessons_index.py --act red-proof` before any injection.
- **Never `git checkout` to restore an injection** (`#349`); snapshot the FIXED file (`#608`).

## Do not create linked worktrees under .worktrees/

Eight lanes live there and `git worktree` operations are repo-global. Build test worktrees under your
own `tmp_path` / scratch, and clean them up — a stray linked worktree makes my `git worktree list`
lie about the fleet.
