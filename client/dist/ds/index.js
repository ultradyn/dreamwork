...
var DreamworkDesign = (() => {
  ...
  function taskRefModel(task, stale) {
    ...
    const origin = ["human", "loop", "unknown"].includes(task.origin) ? task.origin : "unknown";
  ...
  async function showTaskRef(anchor) {
    ...
    try {
      ...
      paintTaskRef(panel, cached ? taskRefModel(cached.task, true) : { kind: "unavailable" });
    }
  ...
  function hideTaskRef(doc) {
    ...
    panel.hidden = true;
    doc.querySelectorAll("a.taskref[data-touch-ready]").forEach((a) => delete a.dataset.touchReady);
  }
  function linkTaskRefText(node) {
    ...
    for (const part of parts) {
      if (part.id == null) {
        frag.appendChild(node.ownerDocument.createTextNode(part.text));
        continue;
      }
  ...
  function observeTaskRefs(doc, reviewHtml = false) {
    ...
    const bindFrame = (frame) => {
      const load = () => {
        try {
          observeTaskRefs(frame.contentDocument, true);
        } catch (e) {
        }
      };
    ...
    const observer = new doc.defaultView.MutationObserver((records) => records.forEach((rec) => rec.addedNodes.forEach((node) => {
    ...
    doc.addEventListener("mouseout", (e) => {
      const a = e.target.closest && e.target.closest("a.taskref");
      if (a && !a.contains(e.relatedTarget)) hideTaskRef(doc);
    });
    ...
    doc.addEventListener("focusout", (e) => {
      const a = e.target.closest && e.target.closest("a.taskref");
      if (a) hideTaskRef(doc);
    });
    doc.addEventListener("keydown", (e) => {
      if (e.key === "Escape") hideTaskRef(doc);
    });
  ...
  function taskTriageRow(task, selected) {
    ...
    const link = `<a class="goalhandle"${current ? ' aria-current="page"' : ""}` + (current ? ` style="border-left:2px solid var(--accent);padding-left:.7rem"` : "") + ` href="/tasks2?t=${task.id}"><span class="goalstate ${escA(task.state || "unknown")}">#${task.id}</span><span class="goaltitle">${esc(task.title || "untitled task")}</span><span class="goalprogress">${esc(task.priority || "priority unknown")}</span><span class="goalmeta">${esc(facets || "details unknown")}</span></a>`;
    return `<div class="taskrow" data-task-row="${task.id}">${link}<button class="tasksteer" type="button" data-do-next="${task.id}" aria-label="steer the loop at #${task.id}">do next</button></div>`;
  }
  ...
  addEventListener("click", (e) => {
    ...
    btn.disabled = true;
    postCommand("do-next", "#" + btn.getAttribute("data-do-next")).finally(() => {
      btn.disabled = false;
    });
  });
