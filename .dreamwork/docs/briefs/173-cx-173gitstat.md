# BRIEF — #173: live git status that can NEVER take index.lock

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-173gitstat
Branch: cx-173gitstat
Base sha: dbbab263
...
Lane-owns: a new git-status module under `dev/` (your naming) and its tests.
**`watch.py` ONLY if the surface genuinely requires it** — say so in your report first; another
lane's client work is pending merge and I want that boundary explicit.

**Task:** `python3 dev/ledger.py get 173 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md`

## The lock constraint is a known INJURY on this machine, not a preference

Max's `CLAUDE.md` carries an active mitigation dated 2026-07-10: background `git status` was taking
the real `.git/index.lock` in every repo, racing his interactive git and orphaning locks when killed.
The mitigations in place are a fish-prompt override adding `--no-optional-locks`, `GIT_OPTIONAL_LOCKS=0`
in every Claude session's env, and a `git-lock-watch` systemd service logging lock creators to
`~/.cache/git-lock-watch/log`. On 2026-07-27 that watcher attributed recurring orphans in THIS repo to
a 500ms-timeout `git status --porcelain` in a status-line extension.

**So a polling git-status feature is precisely the thing that caused his injury.** Build it as though
a regression here costs him his interactive git, because it did once.

Requirements the entry already fixes:
- `--no-optional-locks` on **every** invocation, and `GIT_OPTIONAL_LOCKS=0` in the server's env. Both,
  not either — belt and braces, because the env var is inheritable and the flag is explicit.
- **Read-only commands only.**
- **A guard asserting the lock never appears during a poll.** This is the deliverable that makes the
  rest checkable.
- Three cadences by design: status 5–15s; PR much slower; CI slower still and only when a PR exists
  and is not draft. PR/CI go through `ud-dreamwork-github`, which already owns `gh` — do not shell
  `gh` yourself (#440).

## Red-proof — the guard is the whole point, so red-proof it hardest

- **Direction 1:** break the `--no-optional-locks` discipline (drop the flag from one call site) and
  the guard must red **naming that the lock appeared**, not merely that a command failed.
- **Direction 2, mandatory and subtle:** a lock-watching guard is trivially green when the poll never
  ran, when the poll was too fast to observe, or when the watcher samples on a slower cadence than
  the lock's lifetime. **Construct the case where the lock IS taken and your guard still passes**,
  then show what you did about it. A sampling watcher that can miss a 3ms lock is decoration — say
  what sampling interval yours needs, and how you established it, or state plainly that it cannot
  guarantee detection (#671, #702). An honest boundary beats an implied guarantee.
- Also prove the **healthy** path: a normal poll must not fire the guard, or it is worse than no
  guard (that principle now has a home in `lessons.md` via #786 — cite it by content).
- Run `python3 dev/redproof.py` for the snapshot/restore protocol; `python3 dev/lessons_index.py
  ...
  and note the irony that this task is about git commands specifically. Snapshot the FIXED file (#608).

## Do NOT test against the live repo's index

Other lanes are committing in this repo right now and Max may be using his interactive git. **Use a
scratch repo you create under your worktree** (`git init` a temp dir) for anything that could
contend. A test that races the real `.git` is the defect reproducing itself.
