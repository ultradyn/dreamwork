...
`0 total` render with a quiet empty-state line (`none yet` — the same
dim-line idiom reviews and answers use). The section's presence is the
channel's health made visible (a section that vanishes is indistinguishable
from a broken one). This REMOVES an appear/vanish — a section that is
always present has no arrival/departure to animate, the smallest possible
motion story. The reviews panel stays quiet-when-empty by deliberate
contrast (he has not asked to change it).
