# BRIEF — #645 increment 7: the lossless parser unit, dry-run only

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/glm-645i7
Branch: glm-645i7
Base sha: 911b6ab7
...
Lane-owns: dreamwork_db/, ledger_parse.py, and the new parser-unit files plus their tests


**Task:** `python3 dev/ledger.py get 645 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry **and all its notes**. Increments 1–6 are landed; their notes carry the precedents and my recorded brief errors. Do not re-derive them.

**Design:** `.dreamwork/docs/cx-645-db-api-design.md`, the numbered landing sequence at its tail. Increment 7 reads:

> **Lossless parser unit.** Build dry-run manifest/source-span coverage with the heading, wrapped-title, multi-answer, missing-date and unclassifiable fixtures; print the live denominators, write nothing.

## "Write nothing" is the whole discipline, and it is load-bearing here

Increments 1–6 were dark by construction: new code nothing called. **This one is different — it READS the live `questions.md`** to print denominators, and that is the first time this task family touches production data at all. So the containment rule changes shape: not *"nothing imports it"* but *"it opens the real file and must not be able to write to it."*

Make that structural rather than remembered. A dry-run that writes nothing **because the code path happens not to be taken** is one edit away from writing; a dry-run that cannot write is a property. Say which one you built.

- Do NOT touch the live `.dreamwork/ledger.sqlite3`, `questions.md`, `status.json`, or `answers.md`.
- Do NOT import anything. That is increment 8 and it is already sequenced.

## The five fixtures are the specification, and they are named for a reason

`heading`, `wrapped-title`, `multi-answer`, `missing-date`, `unclassifiable`. Each is a real shape that has bitten this repo:

- **heading** — a markdown heading inside a task BODY truncated an entire section and hid the five newest tasks from sweep and three lint checks, growing by one every time anything was filed (`#753`). Found by printing what the parser actually returns instead of hypothesising.
- **wrapped-title** and **missing-date** — the parser's shape assumptions.
- **multi-answer** — one question, several answers.
- **unclassifiable** — the `#702` case, and the one that decides whether this increment is worth anything.

**A parser that silently drops what it cannot classify produces a manifest that looks complete and is not.** The denominators exist precisely so a reader can see the gap. Report the unclassifiable count as a first-class number, never as absence.

## Direction 2 — where this increment is won

**A "lossless" claim over a corpus you also parsed is circular**: if the manifest is built by the same parser whose losslessness it certifies, it will agree with itself on every input it mishandles. That is `#759` — hold the SUBJECT fixed and vary the INTERPRETER. Establish coverage against something the parser did not produce: raw byte spans, a line/char count, a checksum over the source, an independent scan. **Say what your route is and why it is independent.**

Second, and concrete: `100% coverage` printed over a corpus of **zero** parsed entries is indistinguishable from perfect coverage of a real one (`#671`). Every denominator you print must carry its numerator and its population; a percentage alone is not a report. If the live file yields nothing, that must read as a failure to examine, not as success.

Third: source spans are exactly where an off-by-one hides. A span that is one byte short still round-trips through most checks. Construct the case where the span is wrong but the extracted text still looks right, and show what your check decides.

## Precedents from increments 1–6 that bind you

- **The version fact is `meta.schema_version`**, not `PRAGMA user_version`.
- New code **must route through `dreamwork_db.core`** — `test_no_raw_connect.py` is live and will red a raw `sqlite3.connect` in a production source.
- Repositories return the exact compatibility DTO shapes the facades already returned, not `sqlite3.Row`.
- **A parity proof must load its "before" state from a frozen source**, not from the code doing the work.
- **Find what PINS what you changed, not only what calls it.** Increment 6 was reverted once because `SCHEMA_VERSION` seeded a golden literal in a file that `migrate.py` does not mention — and its re-land found a SECOND pin transitively encoding the same constant. If you touch a constant, search for the VALUE, not just the symbol.

## Constraints

- **`#702`: report what you cannot classify; never drop it.** Governing rule for this increment.
- **`#671`: a check that examined nothing must not read as passing.**
- **`#136`: "parsed and complete" and "could not determine coverage" must not render identically.**
- **`#755`: silent on healthy input.** **`#440`: one supported way** to produce the manifest.
- Do NOT rewrite historical briefs or other lanes' reports.

## Verification

- `python3 lint.py` from the worktree — the lane bar was **5 warnings** at `7270566e`; master has moved several merges since, so **re-measure at your own base and state the number**.
- `just pytest <files you touched, plus caller fallout>` and **`just pytest test_no_raw_connect.py`** — the one supported form. **Do not run the bare full suite**; I own the single merged-tree sweep.
- **Count tests by collecting, not by grepping `def test_`** — parametrisation multiplies, and I have mis-predicted a gate delta this way. Tell me the collected count before and after.
- Prefer an exact pytest node id over a `-k` expression containing spaces.
- `python3 dev/redproof.py` for injections; snapshot the FIXED file lane-privately via `dev/lane_scratch.py snap`, restore by `cp`, verify with `cmp` — never `git checkout` (`#349`, `#608`). Leave the registry clean and **do not leave a committed injection in your branch history** — a lane had to tell me to squash its branch today for exactly that.
- No browser guards, no port binding, nothing near `:35110` / `:35113`.

## Report

`.dreamwork/lane-645i7-report.md`, with a DOGFOOD REPORT section. **Lead with the live denominators** — the real numbers off the real file, including how many entries you could not classify. That number is the deliverable; the manifest machinery is how you got it.
