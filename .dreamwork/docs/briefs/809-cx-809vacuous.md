# BRIEF — #809 REVIEW LENS E: how many of my 2806 tests can actually fail?

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-809vacuous
Branch: cx-809vacuous
Base sha: 7f978b35
...
Lane-owns: nothing in production. You are a READ lens; you may write a report file on your branch.
Export `DREAMWORK_LANE_ROLE=reviewer`.

**Task:** `python3 dev/ledger.py get 809`.

## Why you exist

**I merge on a green gate. The gate is these 2806 tests. Nothing has ever asked how many of them can
actually fail.**

That is not a rhetorical worry. **Three defects filed today were exactly this class,** and each was
found by accident, by a lens hunting something else:

- **`#796`** — a corpus-coverage check that could not fire in a worktree, because tracked and on-disk
  counts were **equal** there. Its test passed for a reason unrelated to the thing it checked.
- **`#795`** — a red proof satisfied by editing a **test's own expected value**. The test went red,
  the tool certified it, and nothing about the production code had been exercised.
- **`#798`** — a guard that returned PASS from magic words in a **comment** plus an `echo`.

Three in one day, all incidental. **You are the systematic pass**, and the question is deliberately
blunt: *if I deleted the behaviour this test names, would this test notice?*

## Method — and the method is most of the deliverable

**2806 is far too many to treat exhaustively, and I am not asking you to.** I am asking you to state
a sampling method, justify it, apply it, and report what it did and did not cover. **"The ones that
looked suspicious" is not a method** — it cannot be audited, it cannot be repeated, and it silently
reports its own blind spot as an all-clear (`#671`).

Some leads. They are leads, not a menu; a better method argued is a better deliverable:

1. **Mutate and re-run.** The direct instrument: delete or invert a production branch, run the tests
   that name it, see if any red. Highest confidence per test, slowest. Best spent on a *chosen* few.
2. **Read for structural tells at scale.** Cheap to apply broadly: a test whose only assertion is
   `assert result is not None`; one that asserts a status code without reading back what changed
   (`#671`); one whose fixture makes both branches identical (`#796`'s exact shape); one that asserts
   a count where membership is the property (`#702`); one whose `except` swallows the failure it
   exists to catch.
3. **Follow the defect history.** Today's three all sit in *guard and tooling* tests — code that
   checks other code. That is a hypothesis about where the density is; **test it rather than assume
   it**, and say whether it held.
4. **Look for tests that never execute at all** — skipped, deselected, collected but unreachable, or
   in a file no runner names. A test that does not run is the limit case, and `just pytest` reports
   it as neither a pass nor a failure.

**Prefer depth over breadth if you must choose.** Ten tests where you *demonstrated* the vacuity by
mutation are worth vastly more to me than four hundred flagged by reading. A flagged-by-reading list
is a hypothesis; a mutation is evidence.

## Report shape

For each finding: **the test's node id, what it appears to check, what it actually checks, and the
mutation that proves the gap** — a production edit that should have reddened it and did not. Without
that last part it is a suspicion, which is fine, but **label it as one.** Keep suspicions and
demonstrations in separate lists; do not blend them into one number.

Then, separately and importantly: **your coverage statement.** How many tests you examined, by what
method, and what you did not look at. An honest *"I mutation-tested 14 and read 300 of 2806, in these
areas, and here is what I never touched"* is a complete report. A number that implies whole-suite
coverage when you sampled is the very defect you were sent to find, committed by the report about it.

**If the suite is in better shape than I fear, say so plainly and show the evidence.** A well-argued
"I tried to break these and could not" is a genuinely valuable result and I will not read it as
failure. Do not manufacture findings to justify the dispatch — I act on what you file, and a false
positive costs a whole lane.

## Rules for your mutations

- **Never `git checkout` to restore an injection** (`#349`). Use `python3 dev/redproof.py` — it owns
  the snapshot/restore protocol. Snapshot the file as it is, mutate, restore by the tool, verify.
- Run `python3 dev/lessons_index.py --act red-proof` before your first injection.
- **`redproof.py check` must be clean before you report, and quote it.** It refuses if any injection
  is left unrestored — and you will be making many, so this is your real safety net, not a formality.
- **You are in a worktree; everything you mutate must be in it.** Never touch the main checkout.
- **Do not commit any mutation.** Restore before every commit.

## Lessons selected for this task

Cite these by **content**, not my wording — verify each before relying on it.

- **`#671`** — a check that examined nothing must not read as passing. Your central principle.
- **`#611`** — a check whose absence reads as a pass. (Verify: this line is `#611`'s, not `#671`'s.
  I have attributed it wrongly before and a lane caught me.)
- **`#702`** — a count-only comparison passes when membership changed but length did not.
- **`#651`** — a guard whose message names a failure mode it cannot detect.
- **`#786`** — a check that fires on a healthy input is worse than no check. The mirror of your lens,
  and worth holding so you do not recommend making tests brittle as the cure.

## Run these, explicitly

```
just pytest --collect-only -q | tail -3
python3 dev/repo_wide_guards.py list
```

**Do not run browser guards.** Load is ~39 and `#666` measured they return WRONG answers under load;
a red you cannot trust is worse than no run. Say in your report that you did not run them.

## Bars

**Master is GREEN**: a full `just pytest` at `aadf579f` read **2806 passed, 0 failed** in 421s. Four
merges have landed since and a gate over them is in flight. That green is the thing you are auditing —
do not treat it as reassurance.
