# BRIEF — #787 (application) + #660 (a ruling that outgrew its own premise)

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-787cite
Branch: cx-787cite
Base sha: beddf975
...
Lane-owns: `.dreamwork/docs/research/contextual-review-annotations.md`,
`.dreamwork/docs/plans/orchestrator-posture.md`, `.dreamwork/docs/plans/posture-autonomy-axis.md`

**Two tasks, both doc-only, different files.** Do #787 first — it is mechanical and fully specified.
`python3 dev/ledger.py get 787` then `get 660`, and read every note.

## #787 — application, not judgement. The ruling is already made.

In `.dreamwork/docs/research/contextual-review-annotations.md`:
- `:20` `(.dreamwork/tasks.md:41-48)` → `(ledger #256)`
- `:26` `(.dreamwork/tasks.md:66-76)` → `(ledger #253)`
- `:26` `(.dreamwork/tasks.md:58-65)` → `(ledger #254)`

Each id is already named in the prose immediately before its parenthetical. **Verify each resolves
via `dev/ledger.py get <id>` to a title matching what the sentence claims** rather than trusting me —
if an id has been retitled so the prose no longer matches, that is a finding about the PROSE, worth
reporting rather than silently repairing.

Why the form changes: `tasks.md` is a 5-line stub since the E3 cutover, so those line ranges are
dead. The ruling (#777) is that a citation **into the ledger** is an id citation; a citation **into a
living doc** stays a line citation under `check_citation_range`. The same doc carries live, valid
line citations into `.dreamwork/review/threaded-topic-chats.html` and `file-formats.md` — **leave
those alone.**

**EXPECTED RESULT: `python3 lint.py | tail -1` drops from 2 warnings to 1** in the main checkout.
The surviving row is `lessons.md:580`'s near-duplicate of `:622`, which is **permanently
human-gated** — its own text says merging is Max's call. **DO NOT drive the bar to 0**; a lane that
does has pruned a lesson he reserved to himself. If you find yourself at 0, you have deleted
something you should not have.

## #660 — the transferable half, which is the actual work

`orchestrator-posture.md:234` and `posture-autonomy-axis.md:277` state the #445/#342
**widen-not-sibling** ruling unconditionally: widen `posture`, never a sibling file, never a second
route, never a second arm. But the quoted justification is **conditional** — *"a widening lets the
closed-set discipline already guarding pace/asking guard this for free"* — and that premise is
**false by construction for a free-text field**, which inherits no closed set.

The lane framing to carry: **a standing ruling whose premise is narrower than its statement will be
either wrongly obeyed or quietly broken by the next person who meets the gap** — and neither outcome
is visible. This is #638 seen from the other end (there a statement outlived its decision; here a
statement outgrew its reason).

**Minimum:** amend those two lines to state the scope the ruling actually earned (closed-set axes),
citing #650 as the case that found the edge. **Then judge with IGC** (worktree-local
`./igc-method.md`) whether the general fix — *whatever records a ruling records the premise it rests
on* — is worth landing here or belongs as its own filed task. An argued "file it, don't build it" is
a fine answer; say why.

**Live relevance:** `glm-646policy` is building the free-text subagent-policy control RIGHT NOW —
the exact case this premise gap governs. Your amendment lands into a live question, so be precise.

## Red-proof (doc lane form)

- **Direction 1:** confirm each citation you change actually resolves after the change, and that
  breaking one is detected — run `lint.py` before and after and quote both trailers.
- **Direction 2:** show a citation that is *dead but shaped correctly* is not made to look alive by
  your edit. An id citation that resolves to a retitled entry is exactly that case — the shape is
  fine and the meaning has rotted (#764). Say what your change does and does not guarantee.
