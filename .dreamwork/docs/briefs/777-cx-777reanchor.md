# BRIEF — #777 increment 2: re-anchor by symbol, because a line number that lands in range still proves nothing

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-777reanchor
Branch: cx-777reanchor
Base sha: e195f901
...
Lane-owns: .dreamwork/docs/**/*.md, dev/reanchor_citations.py, test_reanchor_citations.py, lint.py (allowlist membership ONLY — see the constraint below), test_lint.py

**Task:** `python3 dev/ledger.py get 777 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry and its notes.

## Where this sits

Increment 1 landed `check_citation_range` in `lint.py` (`e195f901`): every `<file>:<line>`
citation in a **living** doc is resolved against the tracked file it names, and a line past
that file's EOF is a WARN. Historical append-only records stay silent via
`HISTORICAL_DOC_PATHS` / `HISTORICAL_DOC_PREFIXES` (`lint.py:3889`).

It shipped as WARN rather than ERROR precisely so this increment could exist: the check
reports the damage, increment 2 repairs it. **You are increment 2.**

**Correct my filing note before you start.** The #777 entry says *335 dangling across 48
files*; increment 1's lane measured *305 across 46* and was right — my figure counted
untracked files. Neither is the number that matters here. What the landed check reports is
**184 dangling citations across 20 living docs**; the rest are inside the historical
allowlist and are correct-when-written. 184/20 is your baseline.

## The cause is one event, and it tells you the method

`watch.py` was ~15,666 lines. The client extraction (#397) moved 10,517 lines of CSS/JS out
into `client/` and `watch.py` is now **6255**. Every `watch.py:1xxxx` citation is a
pre-extraction line number. I measured the targets across tracked docs: **301 watch.py, 6
ledger_store.py, 5 ledger_write.py, 5 `.dreamwork/tasks.md`.**

The important property: **these citations name their subject in adjacent prose.**

```
`tick()` (`watch.py:9105`) snapshots state, calls `buildCurrent()` (`watch.py:6430`)
`append_human_question` (watch.py:6524) builds the title
Verified: `read_bytes` at `watch.py:6752` is a bare `f.read()`.
`emits_wake` seam — one `if` around the `log_event` call at `watch.py:13513`.
```

I measured this too: **108 of the 184 have a backticked identifier on the same line**; the
other 76 name their subject in the surrounding sentence or paragraph. So the re-anchor is
resolvable **by symbol**, and that is the only route that produces a citation worth having.

**There is exactly one way to re-anchor and it is by symbol, never by arithmetic** (#440).
No uniform offset exists — the extraction removed interleaved blocks, not a prefix. A
citation whose line number you adjusted until it stopped exceeding EOF is worse than the
dangling one you started with: the dangling one announces itself, and the clamped one lies
silently. That is the whole failure mode of this increment.

## What to build

`dev/reanchor_citations.py` — **proposes, never writes.** For each dangling citation in a
living doc: extract the symbol the prose names, locate its **definition** in today's tree
(it may now be in `client/router.js`, `client/views.js`, `client/style.css`, … or still in
`watch.py` at a new line), and emit a proposal: `<doc>:<line>  <old> -> <new>  (symbol, confidence)`.

Propose-only is a design constraint, not timidity. A tool that rewrites docs on a symbol
lookup it got wrong corrupts prose silently and at scale, and the corruption is
indistinguishable from correct output. You apply reviewed proposals; the tool never does.

Then apply the reviewed proposals to **the three largest docs** — `plans/render-architecture.md`
(44), `plans/question-updated-wake.md` (27), `plans/delivery-modes.md` (20) — **91 of 184, a
majority.** Increment 3 takes the rest with the tool you built. Do not attempt all 20 docs;
the cap is ~15-20 minutes of work and reviewing 184 proposals is not that.

## The false-green, named, because it is the one that will bite

A citation that is **in range but points at the wrong line** is invisible to
`check_citation_range` — increment 1 named this limit explicitly and it is now your problem.
Two concrete ways your tool manufactures one:

1. **Clamping.** Any proposal whose "resolution" is arithmetic on the old number is this bug.
2. **Mention-vs-definition.** Searching for `` `tick` `` finds the call sites, the comment
   that discusses it, and the string literal — as well as the definition. A proposal that
   lands on a mention satisfies the check, reads as repaired, and points at nothing. Your
   tool must distinguish a definition from a mention, or **report the ambiguity rather than
   picking** (#702).

So the property you are actually asserting is not *"the line is in range"* — the landed
check already gets that. It is **"the line contains the thing the prose names"** (#651, #764).
Assert that in your tests over the docs you repair.

## The allowlist is a claim about a document, not a way to clear a row

You own `lint.py` for **allowlist membership only** — adding a path to
`HISTORICAL_DOC_PATHS` / `HISTORICAL_DOC_PREFIXES`. Do not touch the check's logic; do not
relax it; do not add a suppression mechanism.

Some `plans/*.md` may genuinely be historical: a plan whose work has shipped is a record of
what was intended, not a live description. Reclassifying one is legitimate. But it is a
**claim** — argue it per-doc with evidence (the plan landed; the doc says so; nobody
consults it as current), and **report allowlisted-vs-re-anchored as two separate counts.**

A lane that drove the bar to zero by allowlisting 20 docs would have proven nothing and
broken the check for every future reader (#755, #671). I would rather see 91 re-anchored and
93 still red than 184 silenced.

## Report what you cannot re-anchor — that is a finding, not a leftover

Already visible from here, so I am naming them rather than letting you discover them as
failures:

- **`.dreamwork/tasks.md:66`** and its 4 siblings. Since the E3 sqlite cutover `tasks.md` is
  a **5-line stub**; the ledger is `.dreamwork/ledger.sqlite3`. There is no line 66 to point
  at and there never will be. The right repair is probably to cite the ledger **entry id**
  rather than a line in a retired file — but that is a change of citation *form*, so
  **propose it, do not land it unilaterally.**
- **`ledger_store.py` / `ledger_write.py`** (11 rows): those files still exist but shrank.
  Same symbol method applies.

Any citation whose named symbol no longer exists anywhere is a real finding: the doc
describes something that was deleted. Say so; do not invent a target.

## Red-proof

- **Direction 1:** take a citation your tool resolved correctly, move or rename the symbol it
  points at, and confirm the tool reports *cannot resolve* rather than proposing a nearby
  line. The discriminating output is the refusal, not a diff.
- **Direction 2 (the false-green):** construct a doc whose citation is in range and whose
  named symbol is absent from that line — the mention-vs-definition case above. Show what
  your assertion catches that `check_citation_range` alone does not. If your tests cannot
  tell those two inputs apart, you have built the clamping bug with extra steps.
...
- **Never `git checkout` to restore an injection** (#349) — snapshot lane-privately via
  `dev/lane_scratch.py snap` (or `dev/redproof.py`, which subsumes it and supplies a history
  scan), restore by `cp`, verify with `cmp`. Snapshot the FIXED file, not the pre-fix one (#608).

## Lessons selected for this task

- **#764 — a citation must carry its own evidence.** The parent principle of #777 entirely.
- **#651 — a name must not imply more than it proves.** "Re-anchored" must mean *points at
  the named thing*, not *no longer exceeds EOF*.
- **#671 — a check that examined nothing must not read as passing.** A cleared row that was
  cleared by clamping or allowlisting.
- **#702 — report what you cannot classify.** The unre-anchorable classes above, and every
  ambiguous symbol resolution.
- **#440 — one supported way.** Re-anchor by symbol. Arithmetic on the old line number is
  the second door.
- **#755 — a check that fires on a healthy input is worse than no check.** Why the allowlist
  must not become a silencer.

## Run these, explicitly

Your targeted subset is `test_lint.py` plus your own `test_reanchor_citations.py`.

```
just pytest test_lint.py test_reanchor_citations.py
```

The boilerplate's targeted-subset rule applies — do not run the whole tree; the coordinator
owns the full merged-tree sweep.

**One guard no owned-files list will point you at** (#778 is the structural gap; until it is
fixed briefs name these by hand): if you add a file under `dev/`, `test_no_raw_connect.py`
governs it repo-wide — run it if `reanchor_citations.py` touches any store. It should not
need to; if you find yourself wanting a DB connection here, stop and report why.

## Bars

**Measured in your worktree at dispatch, 2026-08-01, base `e195f901`:** worktree lint
**25 warnings**; **20 citation-range rows**; **184 dangling citations**; `test_lint.py`
collects **563**. Re-measure and quote what *you* got.

Target for this increment: 184 → ~93, and 20 rows → 17. If your numbers differ, say so
rather than reconciling them to mine.

- Count tests by collecting, not by grepping `def test_`.
- Another lane (`glm-645i9b`) is running the full browser-guard suite right now and owns
  `dreamwork_db/`, `dev/ledger.py`, `test_ledger_cli.py`. Do not touch those; if `just test`
  ports look busy, that is why — you do not need browser guards for this work.
- Machine load is elevated from that lane — if something is flaky on timing, say so rather
  than retrying until green. Limit to 2 threads.
- Do not bind `:35110` or `:35113`.
- Rebase onto master before you finish and resolve conflicts yourself; master may move under
  you (`glm-645i9b` is due to land).
- Do not use `attn`.
