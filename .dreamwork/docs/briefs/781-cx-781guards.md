# BRIEF — #781: three guards red for 30 hours, and two of them never judged at all

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-781guards
Branch: cx-781guards
Base sha: 8a96783c
...
Lane-owns: dev/capture/qroll.mjs, dev/capture/bdinput.mjs, dev/capture/selectkeep.mjs, client/

**Task:** `python3 dev/ledger.py get 781 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` — read the entry and its notes. The bisect is already done; do not redo it.

## What is established, so you can start from it

I bisected this before dispatching:

- **PASS** at `947531af` — the parent of the client extraction.
- **FAIL** at `2730557f` — `refactor(#397): extract the client out of watch.py into client/`.
- Red ever since: **~30 hours.**

**It is not load.** The lane that first hit it reasonably attributed it to the #471/#666 load
phenomenon and correctly did not retry to green. I re-ran on a quiet machine where the preflight
itself certified the run — `guard preflight: OK [load 20.86 (1.3x cores) on 16 cores, 0 ccc lane(s)]
— guards should judge honestly` — and got the same failures, then byte-identical failures at a sha
predating today's merges.
...
## The distinction that should drive your whole approach

**These are two different bugs and the output says so.**

```
FAIL qroll (exit 1)
      FAIL the roll SURVIVES THE RELOAD (his standing rule: never lose UI state on an autoreload)
      FAIL a fresh tab opens with the persisted roll already applied
      FAIL unrolling in one tab unrolls the other (kept in sync like other ui state)
FAIL bdinput (exit 124)
      FAIL the guard threw before finishing its checks: Error: page.evaluate: TypeError:
           Cannot read properties of undefined (reading 'map')
FAIL selectkeep (exit 1)
      FAIL the guard threw before finishing its checks
```

- **`qroll` JUDGED and FAILED.** Three named behavioural assertions. That is a real regression in
  the UI, and one of them is a rule **Max himself stated**: never lose UI state on an autoreload.
- **`bdinput` and `selectkeep` DID NOT JUDGE.** "Threw before finishing its checks" is the #471
  did-not-judge sentinel. We do **not** currently know whether their subjects are broken — only
  that the guards died. **Making them stop throwing is not the same as making them pass**, and if
  you fix the throw and they then fail, that is a *second* finding, not a regression you caused.
  Report the two stages separately.

## What I already ruled out, so you do not spend the time

The obvious hypothesis is stale source-pinning: `#397`'s own plan predicted it, saying
*"a minority pin source text — e.g. `dev/capture/bdinput.mjs:81-86` asserts literal strings exist
in `HEAD:watch.py`. Those need re-pointing at `client/*.js`."*

**That was done correctly.** `bdinput.mjs:92` already reads `CAP_SRC_REL = 'client/views.js'` and
its docstring explains the move. So bdinput's failure is **not** a stale pin — the `TypeError` is
raised inside `page.evaluate`, i.e. in the running page, not in a source read.

That points at **actual client behaviour**, which matters because the extraction's headline claim
was that *the served page is byte-identical before and after*. If that claim held, behaviour could
not have changed. So one of these is true and **establishing which is the most valuable thing you
can report**:

1. the byte-identity check was never run, or
2. it was run and is vacuous (it passed without examining what it claimed to — #671), or
3. it held, and the break is in something the guards do that the served page does not exercise.

Do not skip past this to a patch. A guard that starts passing because you adjusted the guard, when
the behaviour is genuinely broken, is strictly worse than the red you started with (#755, #651).

## Do NOT deploy, and do not treat this as user-facing yet

`python3 dev/deploy_state.py` reports the deployed dashboard is a **STALE SNAPSHOT** differing on
`watch.py`, `client/command.js`, `client/router.js`, `client/views.js` and the `client/dist/`
bundle. Max is looking at older code, so his live dashboard is **not** currently affected. The
operational consequence runs the other way: **deploying right now would push this regression to
him.** Deploy is not yours to run in any case — it is outward-facing and his call — but do not
recommend it, and say so if your work would otherwise imply it.

## If your fix touches `client/`

`client/dist/` is committed, so `python3 lint.py` goes ERROR and names the file if you change a
`client/*.js` source without rebuilding. Run `just build-client` and commit the rebuilt bundle in
the same commit as the source change.

## Red-proof

- **Direction 1:** once each guard is green, inject the behaviour it guards (break the roll's
  persistence; break whatever bdinput and selectkeep actually assert) and confirm each reds **on
  its own named assertion**, not merely by throwing. A guard whose only failure mode is a crash is
  a guard that cannot tell you what broke — which is the state two of these are in right now.
- **Direction 2:** confirm each guard **passes on the healthy tree** and that you have not made it
  pass by weakening an assertion. State explicitly, per guard, what it asserts now versus what it
  asserted at `947531af` — if any assertion is weaker, that is a reduction in coverage and it must
  be argued, not slipped in (#755).
...
- **#671 — a check that examined nothing must not read as passing.** Two of these guards did not
  judge; and the extraction's byte-identity check may be the same shape one level up.
- **#755 — a check that fires on a healthy input is worse than no check.** And its converse here:
  do not make a guard pass by softening it.
- **#651 — a name must not imply more than it proves.** "Guard green" must mean the behaviour holds,
  not that the guard stopped crashing.
- **#702 — report what you cannot classify.** If you cannot determine whether a subject is broken
  or only its guard is, say so rather than picking.
- **#136 — a signal that always fires is one nobody reads.** Thirty hours of reds read as load is
  exactly how this survived; whatever you land should make the next one distinguishable.
...
The last line is the always-run repo-wide set that #778 landed an hour ago; the boilerplate below
names it. It costs ~0.3s.

**You are the one lane that SHOULD bind guard ports.** The fleet is otherwise quiet and the ports
39890-39899 are yours. Do not bind `:35110` or `:35113`.

## Bars

**Measured in your worktree at dispatch, 2026-08-01, base `8a96783c` (gated, pytest green):**
worktree lint **22 warnings**; full tree collects **2713**. The three guards are **RED** — that is
your starting state, not a bar to preserve. Re-measure and quote what *you* got.
