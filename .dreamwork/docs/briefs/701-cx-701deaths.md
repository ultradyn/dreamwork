# BRIEF — #701: four lanes died with no error in any log, and I have since reproduced the mechanism

Worktree: /home/xertrov/.llm-general/skills/ud-dreamwork/.worktrees/cx-701deaths
Branch: cx-701deaths
Base sha: dc739001
...
**Task:** `python3 dev/ledger.py get 701 --ledger /home/xertrov/.llm-general/skills/ud-dreamwork/.dreamwork/tasks.md` —
read the entry AND its newest note, which is the lead.

## A cause is now known, because I caused it by accident

The entry says *"terminated mid-flight with no error in any log — cause unknown."* On 2026-08-01 I
ran `just dispatch-lane <prompt> @agent | head -3` for two dispatches. `head` exits after three
lines and closes the pipe; the writer takes **SIGPIPE** and dies. Both lanes were killed seconds
after the runner banner printed. My earlier successful dispatches that day had no pipe — that is the
whole difference.

**Why it fits the entry's evidence exactly:** SIGPIPE leaves no log line *by construction*. "Died
with no error in any log" is not a mystery when the cause is a signal — it is the expected
observable. Any monitoring that looks for an error message finds nothing and correctly reports
nothing.

**What this does NOT establish, and you must not overclaim it:** I know the mechanism, not that
those four died this way. It also explains only a death seconds after launch, not one late in a
lane's work. Treat it as a strong lead (#702).

## The fix worth having makes the trap unavailable rather than known

`dispatch_lane.py` already refuses on a bad contract (`rc=2`, clean, nothing launched) and, as of
today, on a base sha that is not the branch point. The natural addition: **detect that stdout is not
a terminal / is a pipe, and refuse or warn before `os.execvp`.**

**Weigh this properly rather than just building it.** Legitimate non-tty dispatches exist — I
routinely run `just dispatch-lane ... > file.log 2>&1` in the background, and every dispatch in
today's fleet was launched that way. A check that cannot tell a *redirect to a file* from a *pipe to
a short-lived reader* would refuse every background dispatch I make, which would stop the loop
dispatching entirely. `os.isatty` alone does not distinguish them; `os.fstat(1).st_mode` and
`stat.S_ISFIFO` do. Measure before choosing.

**A second, cheaper option worth pricing:** ignore SIGPIPE (or restore `SIG_DFL` deliberately) in the
dispatcher so the writer survives a closed reader. That treats the symptom generally rather than
detecting one caller shape. It may be strictly better. Argue it either way.

Judge with IGC — use the **worktree-local `./igc-method.md`**, not any copy under `~/.claude-p`.

## Handle this file with care — it is the loop's only dispatch route

It ends in `os.execvp(runner[0], [*runner, prompt])`, executing from the MAIN checkout. A change
that refuses wrongly does not degrade a feature; it stops lanes launching at all. **Five other lanes
are live right now and I will be dispatching more against this file's merged form.**

## Red-proof

- **Direction 1:** reproduce the death — dispatch through a pipe to a short reader and show the
  runner dies; then show your fix prevents it, with the test naming the specific case.
- **Direction 2, mandatory:** prove every HEALTHY dispatch shape still launches — a tty, a redirect
  to a file, and a background redirect (the shape the whole fleet uses). A refusal that fires on
  those is far worse than the bug.

## Lessons selected for this task

- **A check that fires on a healthy input is worse than no check** — the governing constraint here.
  (Its number is under audit as #786; treat the principle as the instruction.)
- **Report what you cannot classify** (`#702`) — the mechanism is not proof about those four deaths.
- **One supported way** (`#440`) — if dispatch grows a refusal, there must be one way to say "yes,
  I meant to redirect".
